...
//	eb f0 7c ab 50 d2 8a b3 	7a 
	...
	public static void load() {
		...
		try {
			...
			뚫어방어사용 = Boolean.parseBoolean(serverSettings.getProperty("dduldef",
					"true"));
			유체이탈방어사용 = Boolean.parseBoolean(serverSettings.getProperty(
					"ucedef", "true"));
			...
			ACCOUNT_PASSWORD = Boolean.parseBoolean(serverSettings.getProperty(
					"AccountPassword", "false"));//패스워드 암호화 삭제 소스 0813 추가
			...
			LOGGING_TIME = Integer.parseInt(serverSettings.getProperty(
					"LoggingTime", "120"));
			...
			AUTO_CHECK = Boolean.parseBoolean(serverSettings.getProperty(
					"AutoCheck", "false"));

			WAR_TIME_AUTO_SETTING = Boolean.parseBoolean(serverSettings
					.getProperty("WarTimeAutoSetting", "false"));

			GameServer.신규지원_경험치지급단 = Boolean.parseBoolean(serverSettings
					.getProperty("NewvieBonus", "false"));
		...
		/*try {
			...
			fishrate = Double.parseDouble(rateSettings.getProperty("fishexp",
					"112500"));

			RATE_LAWFUL = Double.parseDouble(rateSettings.getProperty(
					"RateLawful", "1.0"));
			...
			RATE_ROBOT_TIME = Integer.parseInt(rateSettings.getProperty(
					"RateRobotTime", "12")); // 무인PC(쿠우)
			...
			RATE_DREAM = Integer.parseInt(rateSettings.getProperty("Ratedream",
					"1")); // 만월의정기이벤트
			...
			MAX_WEAPON = Integer.parseInt(rateSettings.getProperty("MaxWeapon",
					"12"));

			MAX_ARMOR = Integer.parseInt(rateSettings.getProperty("MaxArmor",
					"10"));

			MAX_WEAPON1 = Integer.parseInt(rateSettings.getProperty(
					"MaxWeapon1", "12"));// 마족

			MAX_LEVEL = Integer.parseInt(rateSettings.getProperty("Maxlevel",
					"12"));// 신규보호렙제
			...
			FEATHER_TIME = Integer.parseInt(rateSettings.getProperty(
					"FeatherTime", "10"));

			FEATHER_NUMBER = Integer.parseInt(rateSettings.getProperty(
					"FeatherNumber", "10"));

			CLAN_NUMBER = Integer.parseInt(rateSettings.getProperty(
					"ClanNumber", "10"));

			CASTLE_NUMBER = Integer.parseInt(rateSettings.getProperty(
					"CastleNumber", "50"));
			...
			생명의나뭇잎 = Integer.parseInt(rateSettings.getProperty("Leafitem",
					"100"));
			영생의빛 = Integer.parseInt(rateSettings.getProperty("Eternalitem",
					"100"));
			...
			RATE_7_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_7_Dmg_Rate", "1.5"));
			RATE_8_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_8_Dmg_Rate", "1.5"));
			RATE_9_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_9_Dmg_Rate", "2.0"));
			RATE_10_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_10_Dmg_Rate", "2.0"));
			RATE_11_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_11_Dmg_Rate", "2.0"));
			RATE_12_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_12_Dmg_Rate", "2.0"));
			RATE_13_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_13_Dmg_Rate", "2.0"));
			RATE_14_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_14_Dmg_Rate", "2.0"));
			RATE_15_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_15_Dmg_Rate", "2.0"));
			RATE_16_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_16_Dmg_Rate", "2.5"));
			RATE_17_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_17_Dmg_Rate", "2.5"));
			RATE_18_DMG_RATE = Double.parseDouble(rateSettings.getProperty(
					"Rate_18_Dmg_Rate", "2.5"));

			RATE_7_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_7_Dmg_Per", "5"));
			RATE_8_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_8_Dmg_Per", "10"));
			RATE_9_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_9_Dmg_Per", "20"));
			RATE_10_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_10_Dmg_Per", "30"));
			RATE_11_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_11_Dmg_Per", "40"));
			RATE_12_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_12_Dmg_Per", "50"));
			RATE_13_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_13_Dmg_Per", "60"));
			RATE_14_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_14_Dmg_Per", "70"));
			RATE_15_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_15_Dmg_Per", "80"));
			RATE_16_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_16_Dmg_Per", "90"));
			RATE_17_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_17_Dmg_Per", "90"));
			RATE_18_DMG_PER = Integer.parseInt(rateSettings.getProperty(
					"Rate_18_Dmg_Per", "100"));
		...
		try {
			...
			신규레벨 = Short.parseShort(altSettings.getProperty(
					"biginnerlev", "56"));
			...
			ALT_PRIVATE_WAREHOUSE_LEVEL = Integer.parseInt(altSettings
					.getProperty("PrivateWarehouseLevel", "5"));
			...
			Big_Praseant = Boolean.parseBoolean(altSettings.getProperty(
					"Bigprasenat", "false"));
			
			드래곤성장의물약이벤트 = Boolean.parseBoolean(altSettings.getProperty(
					"dragonpotion", "false"));
			
			ALT_FEATURE = Boolean.parseBoolean(altSettings.getProperty(
					"FeatureEvent", "true"));
			...
			DELETE_DB_DAYS = Integer.parseInt(altSettings.getProperty(
					"DeleteDBDAY", "14"));
			
			MAX_버모스_DUNGEON_LEVEL = Byte.parseByte(altSettings.getProperty(
					"MaxEarthDragonDungeon", "99"));

			MAX_후오스_DUNGEON_LEVEL = Byte.parseByte(altSettings.getProperty(
					"MaxWaterDragonDungeon", "99"));
			
			용레이드레벨 = Byte.parseByte(altSettings.getProperty(
					"Dragonraid", "80"));
			
			발라카스레이드레벨 = Byte.parseByte(altSettings.getProperty(
					"DragonraidVala", "85"));

			MIN_상아탑_DUNGEON_LEVEL = Byte.parseByte(altSettings.getProperty(
					"MinIvoryTowerDungeon", "99"));

			자동방지_퀴즈 = toKor(altSettings.getProperty("Anti_Auto_Quiz",
					"이번차에도감사합니다현재는자동체크중이니불편하시더라도자동아님을입력해주세요"));
			// System.out.println("자동방지 퀴즈 : "+자동방지_퀴즈);
			폰인증체크퀴즈 = toKor(altSettings.getProperty("Phone_Auto_Quiz",
					"오토 판별을위해 핸드폰 인증을 해주세요."));

			폰인증멘트 = toKor(altSettings.getProperty("Phone_Ment",
					"본인핸드폰을 이용해 010-1234-5678 로 전화주시면 인증을 도와 드립니다."));

			자동방지_답 = toKor(altSettings.getProperty("Anti_Auto_Answer", "자동아님"));
			// System.out.println("자동방지 답 : "+자동방지_답);

			Event_Box = Boolean.parseBoolean(altSettings.getProperty(
					"Event_box_ok", "false"));

			ALT_PRIVATE_SHOP_LEVEL = Integer.parseInt(altSettings.getProperty(
					"PrivateShopLevel", "0"));

			EVENT_DRAGON_DIAMOND_CHANCE = Integer.parseInt(altSettings
					.getProperty("DragonDiamondChance", "0"));
			EVENT_DRAGON_DIAMOND_CHANCE2 = Integer.parseInt(altSettings
					.getProperty("DragonDiamondChance2", "0"));

			드상드랍_드레이크_찬스 = Integer.parseInt(altSettings.getProperty(
					"DragonDiamondChance3", "0"));

			아르카_Event = Boolean.parseBoolean(altSettings.getProperty(
					"150409Event", "false"));
			드상드랍_아르카_찬스 = Integer.parseInt(altSettings.getProperty(
					"150409ARKADropChance", "0"));
			드상드랍_할로윈_찬스 = Integer.parseInt(altSettings.getProperty(
					"HWDropChance", "0"));
			케플리샤드랍_찬스 = Integer.parseInt(altSettings.getProperty(
					"1219EventDropChance", "0"));
			케플리샤_Event = Boolean.parseBoolean(altSettings.getProperty(
					"1219Event", "false"));
			...
			각반드랍_찬스 = Integer.parseInt(altSettings.getProperty(
					"150101EventDropChance", "0"));
			각반_Event = Boolean.parseBoolean(altSettings.getProperty(
					"150101Event", "false"));

			룸티스드랍_찬스 = Integer.parseInt(altSettings.getProperty(
					"150310EventDropChance", "0"));
			룸티스_Event = Boolean.parseBoolean(altSettings.getProperty(
					"150310Event", "false"));

			강화상자드랍_찬스 = Integer.parseInt(altSettings.getProperty(
					"150319EventDropChance", "0"));
			강화상자_Event = Boolean.parseBoolean(altSettings.getProperty(
					"150319Event", "false"));

			세뱃돈드랍_찬스 = Integer.parseInt(altSettings.getProperty(
					"150216EventDropChance", "0"));
			세뱃돈_Event = Boolean.parseBoolean(altSettings.getProperty(
					"150216Event", "false"));

			리퍼선물_Event = Boolean.parseBoolean(altSettings.getProperty(
					"150108Event", "false"));
			리퍼2드랍_찬스 = Integer.parseInt(altSettings.getProperty(
					"150108EventDropChance", "0"));

			수련자무기밸런스수치 = Integer.parseInt(altSettings.getProperty(
					"aWeaponBalance", "0"));
			수련자갑옷밸런스수치 = Integer.parseInt(altSettings.getProperty(
					"aArmorBalance", "0"));
			warmember = Integer.parseInt(altSettings.getProperty("warmember", "0"));

			리퍼드랍_찬스 = Integer.parseInt(altSettings.getProperty(
					"1225EventDropChance", "0"));
			리퍼_Event = Boolean.parseBoolean(altSettings.getProperty(
					"1225Event", "false"));
			Dragon_3DAY_Event = Boolean.parseBoolean(altSettings.getProperty(
					"Dragon3DayEvent", "false"));

			Dragon_1DAY_Event = Boolean.parseBoolean(altSettings.getProperty(
					"Dragon1DayEvent", "false"));

			초파드랍_그렘린_찬스 = Integer.parseInt(altSettings.getProperty(
					"ChopaChance", "0"));
			Chopa_Event = Boolean.parseBoolean(altSettings.getProperty(
					"ChopaEvent", "false"));

			Dragon_14_12_12_Event = Boolean.parseBoolean(altSettings
					.getProperty("Dragon141212event", "false"));

			할로윈_Event = Boolean.parseBoolean(altSettings.getProperty(
					"HW_Event", "false"));

			EVENT_각인해제인장_찬스 = Integer.parseInt(altSettings.getProperty(
					"GakinEventChance", "0"));

			EVENT_루피주먹주머니_찬스 = Integer.parseInt(altSettings.getProperty(
					"RuphyBraveEventChance", "0"));

			EVENT_SEHO_BOOMB_CHANCE = Integer.parseInt(altSettings.getProperty(
					"Event_box_chance", "0"));

			DRAGON_2DAY_EVENT = Boolean.parseBoolean(altSettings.getProperty(
					"Dragon2DayEvent", "false"));

			DANTES_2DAY_EVENT = Boolean.parseBoolean(altSettings.getProperty(
					"Dantes2DayEvent", "false"));

			Orim_EVENT = Boolean.parseBoolean(altSettings.getProperty(
					"OrimEvent", "false"));

			RuphyBrave_3DAY_EVENT = Boolean.parseBoolean(altSettings
					.getProperty("RuphyBrave3DayEvent", "false"));

			Tam_Ok = Boolean.parseBoolean(altSettings.getProperty("TamOK",
					"false"));
			Tam_Time = Integer.parseInt(altSettings
					.getProperty("TamTime", "15"));
			Tam_Count = Integer.parseInt(altSettings.getProperty("TamCount",
					"600"));
			...
			D_Reset_Time = Integer.parseInt(altSettings.getProperty(
					"DungeonResetTime", "9"));
		...
		try {
			...
			botSettings.load(is);
			is.close();
			Robot_Gaho = Boolean.parseBoolean(botSettings.getProperty(
					"Robotgaho", "false"));
			로봇요정활1 = Integer.parseInt(botSettings.getProperty(
					"robotbow1", "12314"));
			로봇요정활2 = Integer.parseInt(botSettings.getProperty(
					"robotbow2", "12314"));
			로봇요정활3 = Integer.parseInt(botSettings.getProperty(
					"robotbow3", "12314"));
			로봇요정활4 = Integer.parseInt(botSettings.getProperty(
					"robotbow4", "12314"));
			로봇요정활5 = Integer.parseInt(botSettings.getProperty(
					"robotbow5", "12314"));
			로봇칼1 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd1", "12283"));
			로봇칼2 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd2", "12283"));
			로봇칼3 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd3", "12283"));
			로봇칼4 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd4", "12283"));
			로봇칼5 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd5", "12283"));
			로봇칼6 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd6", "12283"));
			로봇칼7 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd7", "12283"));
			로봇칼8 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd8", "12283"));
			로봇칼9 = Integer.parseInt(botSettings.getProperty(
					"robotsowrd9", "12283"));			
			인형타입활 = Integer.parseInt(botSettings.getProperty(
					"robotdollbowtype", "22"));
			인형타입칼1 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordtype1", "21"));
			인형타입칼2 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordtype2", "26"));
			인형타입칼3 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordtype3", "26"));
			인형타입칼4 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordtype4", "26"));
			인형타입칼5 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordtype5", "26"));
			인형타입칼6 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordtype6", "26"));
			인형타입경험치 = Integer.parseInt(botSettings.getProperty(
					"robotdolltypeexp", "26"));
			인형타입활엔피씨아이디 = Integer.parseInt(botSettings.getProperty(
					"robotdollbownpcid", "41915"));
			인형타입칼엔피씨아이디1 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordnpcid1", "4500161"));
			인형타입칼엔피씨아이디2 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordnpcid2", "45000161"));
			인형타입칼엔피씨아이디3 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordnpcid3", "4500161"));
			인형타입칼엔피씨아이디4 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordnpcid4", "4500161"));
			인형타입칼엔피씨아이디5 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordnpcid5", "4500161"));
			인형타입칼엔피씨아이디6 = Integer.parseInt(botSettings.getProperty(
					"robotdollswordnpcid6", "4500161"));
			경험치인형엔피씨아이디 = Integer.parseInt(botSettings.getProperty(
					"robotdollexpnpcid", "4500161"));
			봇시작레벨 = Integer.parseInt(botSettings.getProperty(
					"botstartLev", "56"));
			봇기술적중 = Integer.parseInt(botSettings.getProperty(
					"bottechnichit", "5"));
			봇정령적중 = Integer.parseInt(botSettings.getProperty(
					"botspirithit", "0"));
			봇용언적중 = Integer.parseInt(botSettings.getProperty(
					"botdragonhit", "0"));
			봇공포적중 = Integer.parseInt(botSettings.getProperty(
					"botfearhit", "0"));
			봇기술내성 = Integer.parseInt(botSettings.getProperty(
					"bottechnictollance", "30"));
			봇정령내성 = Integer.parseInt(botSettings.getProperty(
					"botspirittollance", "10"));
			봇용언내성 = Integer.parseInt(botSettings.getProperty(
					"botdragontollance", "10"));
			봇공포내성 = Integer.parseInt(botSettings.getProperty(
					"botfeartollance", "10"));
			요정활기본타격치PC = Integer.parseInt(botSettings.getProperty(
					"bowbasepc", "10"));
			요정활랜덤타격치PC = Integer.parseInt(botSettings.getProperty(
					"bowrndpc", "10"));
			기사기본타격치PC = Integer.parseInt(botSettings.getProperty(
					"knightbasepc", "10"));
			기사랜덤타격치PC = Integer.parseInt(botSettings.getProperty(
					"knightrndpc", "10"));
			전사기본타격치PC = Integer.parseInt(botSettings.getProperty(
					"warriorbasepc", "10"));
			전사랜덤타격치PC = Integer.parseInt(botSettings.getProperty(
					"warriorrndpc", "10"));
			다엘기본타격치PC = Integer.parseInt(botSettings.getProperty(
					"darkelfbasepc", "10"));
			다엘랜덤타격치PC = Integer.parseInt(botSettings.getProperty(
					"darkelfrndpc", "10"));
			용기사기본타격치PC = Integer.parseInt(botSettings.getProperty(
					"dragonebasepc", "10"));
			용기사랜덤타격치PC = Integer.parseInt(botSettings.getProperty(
					"dragonerndpc", "10"));
			군검요기본타격치PC = Integer.parseInt(botSettings.getProperty(
					"crownbasepc", "10"));
			군검요랜덤타격치PC = Integer.parseInt(botSettings.getProperty(
					"crownrndpc", "10"));	
			요정활기본타격치NPC = Integer.parseInt(botSettings.getProperty(
					"bowbasenpc", "10"));
			요정활랜덤타격치NPC = Integer.parseInt(botSettings.getProperty(
					"bowrndnpc", "10"));
			기사기본타격치NPC = Integer.parseInt(botSettings.getProperty(
					"knightbasenpc", "10"));
			기사랜덤타격치NPC = Integer.parseInt(botSettings.getProperty(
					"knightrndnpc", "10"));
			전사기본타격치NPC = Integer.parseInt(botSettings.getProperty(
					"warriorbasenpc", "10"));
			전사랜덤타격치NPC = Integer.parseInt(botSettings.getProperty(
					"warriorrndnpc", "10"));
			다엘기본타격치NPC = Integer.parseInt(botSettings.getProperty(
					"darkelfbasenpc", "10"));
			다엘랜덤타격치NPC = Integer.parseInt(botSettings.getProperty(
					"darkelfrndnpc", "10"));
			용기사기본타격치NPC = Integer.parseInt(botSettings.getProperty(
					"dragonebasenpc", "10"));
			용기사랜덤타격치NPC = Integer.parseInt(botSettings.getProperty(
					"dragonerndnpc", "10"));
			군검요기본타격치NPC = Integer.parseInt(botSettings.getProperty(
					"crownbasenpc", "10"));
			군검요랜덤타격치NPC = Integer.parseInt(botSettings.getProperty(
					"crownrndnpc", "10"));
			봇물약기본회복량 = Integer.parseInt(botSettings.getProperty(
					"botposionbase", "45"));
			봇물약랜덤회복량 = Integer.parseInt(botSettings.getProperty(
					"botposionrnd", "65"));
			봇요정싸이클론 = Boolean.parseBoolean(botSettings.getProperty(
					"botelfcyclone", "false"));
			봇요정악장 = Boolean.parseBoolean(botSettings.getProperty(
					"botnightmare", "false"));
			봇전사태풍 = Boolean.parseBoolean(botSettings.getProperty(
					"bottypoon", "false"));
			봇군주엠파 = Boolean.parseBoolean(botSettings.getProperty(
					"botcrownempire", "false"));
			봇게일 = Boolean.parseBoolean(botSettings.getProperty(
					"botgalse", "false"));
			봇똥물 = Boolean.parseBoolean(botSettings.getProperty(
					"botpolute", "false"));
			로봇막피 = Boolean.parseBoolean(botSettings.getProperty(
					"botattckpc", "false"));
		...
		try {
			...
			MAXLEVEL = Integer.parseInt(charSettings.getProperty("LimitLevel",
					"99"));
	...
	private static void validate() {
		if (!IntRange.includes(Config.ALT_ITEM_DELETION_RANGE, 0, 5)) {
			throw new IllegalStateException(
					"ItemDeletionRange의 값이 설정 가능 범위외입니다. ");
		}

		if (!IntRange.includes(Config.ALT_ITEM_DELETION_TIME, 1, 35791)) {
			throw new IllegalStateException(
					"ItemDeletionTime의 값이 설정 가능 범위외입니다. ");
		}
	}
	 ...
	 * String 년2; if(년 < 10){ 년2 = "0"+년; }else{ 년2 = Integer.toString(년); } int
	 * 월 = Month(); String 월2=null; if(월 < 10){ 월2 = "0"+월; }else{ 월2 =
	 * Integer.toString(월); } int 일 = Date(); String 일2 = null; if(일 < 10){ 일2 =
	 * "0"+일; }else{ 일2 = Integer.toString(일); } return 년2+월2+일2; }catch
	 * (Exception e){}
	 * 
	 * return "000000"; }
	 */
