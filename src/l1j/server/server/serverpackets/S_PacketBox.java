...
public class S_PacketBox extends ServerBasePacket {
	...
	public S_PacketBox(int subCode, String name, int mapid, int x, int y,
			int Mid) {
		writeC(108);
		writeC(subCode);
		switch (subCode) {
	...
	public S_PacketBox(L1ItemInstance item, int subcode) {
		...
		if (!((item.getItemId() >= 437010 && item.getItemId() <= 437013)
				...
				|| item.getItemId() == 1437010 || item.getItemId() == 1437011 || item.getItemId() == 60291 || item.getItemId() == 888819
				...
				|| item.getItemId() == 60398 || item.getItemId() == 9096
				|| item.getItemId() == 600369
	...
	public S_PacketBox(int subCode, int type, int time) {
		...
		switch (subCode) {
		case ICON_POLYMORPH: //변신아이콘 패킷 변경으로!
			writeH(type); // 시간
			writeC(time); // 타입(1 보통변신 , 2 지배자 변신)
			break;
	...
	public S_PacketBox(int subCode, L1ItemInstance item) {
		...
		case 172: // 인첸변경
			...
			writeD(0);
			writeD(0);
			writeD(item.getItem().isTradable() ? 7 : item.getBless() >= 128 ? 3
					: 2);
			writeD(0);
			break;
		}
	...
	public S_PacketBox(int subCode, L1PcInstance pc, L1ItemInstance item, boolean bool) {
		writeC(Opcodes.S_EVENT);
		writeC(subCode);
		switch (subCode) {
		case WEAPON_RANGE: {
			...
			if (polyId == 3784 || polyId == 13152 || polyId == 13153
					...
					|| polyId == 16014 || polyId == 17541 || polyId == 17515 || polyId == 17531
				...
				if (bool) {
					...
					if (길이 == 2 && (polyId == 7967 || polyId == 10874 || polyId == 7846 || polyId == 7848 || polyId == 8719)) {
						길이 = 1;
					}
					if (길이 == 2 && polyId >= 11328 && polyId <= 11448 	&& !(polyId >= 11408 && polyId <= 11421)) {
						if (polyId == 11330 || polyId == 11344 || polyId == 11351 || polyId == 11368 || polyId == 11376 || polyId == 11447
							||polyId == 16008 || polyId == 16056)
							길이 = 2;
						else
							길이 = 1;
					}
			...
			if (길이 == 17 || 길이 == 15) {
				...
				if (bool) {
					...
					if (pc.getWeapon().getItem().getType() == 6 || pc.getWeapon().getItem().getType() == 15) {
						착용 = 2;
					}
					...
					if (pc.getWeapon().getItem().getType() == 12) {
						착용 = 4;
						마법사용 = 0;
					}
				}
	...
	public S_PacketBox(int subCode, L1PcInstance pc) {
		...
		/*case AINHASAD:
			...
			int value2 = pc.getAinhasadbless();
			value /= 10000;
			writeD(value);// % 수치 1~200
			writeD(10000);
			writeD(0x00);
			writeD(value2);//축복소모효율
			break;*/
		case 유저빽스탭:
			writeH(pc.getX());
			writeH(pc.getY());
