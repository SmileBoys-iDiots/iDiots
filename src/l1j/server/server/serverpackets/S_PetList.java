...
public class S_PetList extends ServerBasePacket {
	...
	private void buildPacket(int npcObjId, L1PcInstance pc) {
		...
		/*
		 * if (amuletList.size() != 0) { writeC(Opcodes.S_FIXABLE_ITEM_LIST);
		 * writeD(0x00000046); // Price writeH(amuletList.size()); for
		 * (L1ItemInstance _item : amuletList) { writeD(_item.getId());
