	...
	public static S_CTPacket getSpellList(ArrayDeque<MJCTSpell> spQ){
		S_CTPacket pck = new S_CTPacket();
				...
				while(!spQ.isEmpty()){
					...
					int Length = 13 + pck.size7B(Item.getId()) + pck.size7B(Item.getItemId()) + pck.size7B(DescId) + pck.size7B(Type) +
							pck.size7B(Item.getCount()) + pck.size7B(sp.xicon) + sp.name.getBytes().length + os.getBytes().length;
