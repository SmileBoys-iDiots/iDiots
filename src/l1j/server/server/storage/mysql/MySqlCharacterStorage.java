...
public class MySqlCharacterStorage implements CharacterStorage {
	...
	public void storeCharacter(L1PcInstance pc) {
		...
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement(
					"UPDATE characters SET level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,BaseStr=?,Con=?,BaseCon=?,Dex=?,BaseDex=?,Cha=?,BaseCha=?,Intel=?,BaseIntel=?,Wis=?,BaseWis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?, sealScrollTime=?, sealScrollCount=?,PKcount=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,HellTime=?,Banned=?,Karma=?,Age=?,LastPk=?,DeleteTime=?,ReturnStat=?,Ainhasad_Exp=?,Ainhasad_DP=?,Logout_time=?,HuntPrice=?, HuntCount=?, HuntText=?, sealScrollTime=?, sealScrollCount=?, PC_Kill=?, PC_Death=?, bloodtime=?, bloodtime2=?,DragonEMETime=?, DragonEMETime2=?, bloodtime3=?, RingAddSlot=?, MaanDelay=?, Clan_Memo=?, DragonPUPLE=?, Bookmark_max=?, Clan_Join_Date=?, EarringAddSlot=? , MSTime=?, MSDay=?, FirstBlood=?, TOPAZTime=?, TamEndTime=?, GOTime=?, GODay=?, DCTime=?, DCDay=?, ElfAttrResetCount=?, QUEST_70=?, QUEST_83=?  WHERE objid=?");
			pstm.setInt(++i, pc.getLevel());
			pstm.setInt(++i, pc.getHighLevel());
			pstm.setInt(++i, pc.getExp());
			pstm.setInt(++i, pc.getBaseMaxHp());
