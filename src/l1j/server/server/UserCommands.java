...
public class UserCommands {
		...
		try {
			...
			if (target != null) {
				...
				L1World.getInstance().broadcastPacketToAll(
						new S_SystemMessage("\\aD[ 수배자 ]:  " + target.getName() + "  ]"));
				L1World.getInstance().broadcastPacketToAll(
						new S_SystemMessage("\\aD[ 이유 ]: " + story + "  "));
	...
	private void quize(L1PcInstance pc, String param) {
		try {
			...
			pc.sendPackets(new S_SystemMessage("키워드가 [" + quize
					+ "]으로 입력되었습니다. 키워드는 다시 확인과 변경이 불가능하니 유의하시기 바랍니다."));
		} catch (Exception e) {
	...
	private static final int[] allBuffSkill = { L1SkillId.FIRE_SHIELD,
