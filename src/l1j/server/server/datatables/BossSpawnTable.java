				...
				if (rs.getString("cycle_type").equals(name)) {
					...
					for (L1Object object : L1World.getInstance().getObject()) {
						if (object instanceof L1NpcInstance) {
							...
							if (npcId == 45488
									&& npc.getNpcTemplate().get_npcId() == 45488
									&& (npc.getMapId() == 9 || npc.getMapId() == 10)
									&& npc.getMapId() != 666) {
								alreadySpawnCount++;
							} else if (npcId == 45601
									&& npc.getNpcTemplate().get_npcId() == 45601
									&& (npc.getMapId() == 11
											|| npc.getMapId() == 12 || npc
											.getMapId() == 13)
									&& npc.getMapId() != 666) {
					...
					if (maxSpawnCount <= alreadySpawnCount)
						continue;
					...
					if (groupId > 0)
						L1MobGroupSpawn.getInstance().doSpawn(npc, groupId,
								rs.getBoolean("respawn_screen"), false);

					if (npcId == 45573 && npc.getMapId() == 2) {
						for (L1PcInstance _pc : L1World.getInstance()
								.getAllPlayers()) {
							if (_pc.getMapId() == 2) {
								L1Teleport.teleport(_pc, 32664, 32797,
										(short) 2, 0, true);
							}
						}
					}
					if (npcId == 46142 && npc.getMapId() == 73
						...
						for (L1PcInstance pc1 : L1World.getInstance()
								.getAllPlayers()) {
							if (pc1.getMapId() >= 72 && pc1.getMapId() <= 74) {
								L1Teleport.teleport(pc1, 32840, 32833,
										(short) 72, pc1.getMoveState()
												.getHeading(), true);
							}
						}
					}

					// System.out.println("보스스폰"+npcId);
							...
							if (npc.getNpcTemplate().get_npcId() == npcId
								...
								if (rs.getInt("group_id") != 0) {
									...
									for (int i = 0, a = info.getNumOfMembers(); i < a; i++) {
										npc1 = (L1NpcInstance) L1World
												.getInstance().findObject(
														info.getIndexMember(i)
																.getId());
										npc1.groupDeleteMe();
									}
								} else {
