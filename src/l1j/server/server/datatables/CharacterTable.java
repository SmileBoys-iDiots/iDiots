...
public class CharacterTable {
	...
	public static boolean somakname(String name) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("SELECT id FROM npc_gamble_spawnlist WHERE name=?");
			pstm.setString(1, name);
			rs = pstm.executeQuery();
			result = rs.next();
			if (!result) {
				pstm = con
						.prepareStatement("SELECT id FROM npc_gamble_support_spawnlist WHERE name=?");
				pstm.setString(1, name);
				rs = pstm.executeQuery();
				result = rs.next();
			}
		} catch (SQLException e) {
			// _log.warning("could not check existing charname:" +
			// e.getMessage());
		} finally {
			SQLUtil.close(rs);
			SQLUtil.close(pstm);
