...
public class NpcTable {
	...
	private void loadNpcData() {
		...
		try {
			...
			while (rs.next()) {
				...
				npc.set__MagicBarrier(rs.getBoolean("Magic_Barrier"));
	...
	public L1NpcInstance newNpcInstance(int id) throws ClassNotFoundException, IllegalAccessException, InstantiationException,
		...
		if (s.equals("Gamble")) {
			con = Class.forName("l1j.server.GameSystem.Gamble." + s + "Instance").getConstructors()[0];
		} else {
			con = Class.forName("l1j.server.server.model.Instance." + s + "Instance").getConstructors()[0];
		}
		if (con == null) {
			throw new IllegalArgumentException(String.format("NpcTemplate: %d not create", id));
		}
	...
	public boolean findNpcShopName(String name) {
		for (L1Npc npc : _npcs.values()) {
			if (npc.getImpl().equalsIgnoreCase("L1NpcShop") && npc.get_name().equals(name)) {
				return true;
			}
		}
		return false;
