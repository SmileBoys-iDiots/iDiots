	...
	public static void reload() {
		...
		try {
			...
			while (rs.next()) {
				...
				spawnDat = SpawnTable.getInstance().getTemplate(spawnid);
				if (spawnDat == null) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (template1 == null) {
					...
					spawnDat.setNpcid(rs.getInt("npc_templateid"));
					 ...
					 * System.out.println("2228 테이블에서읽음"); }
					 */
					// start the spawning
					spawnDat.init();
