...
public class C_SelectCharacter extends ClientBasePacket {
	...
	class login implements Runnable {
		...
		public void run() {
			try {
				if (name.equalsIgnoreCase("") || client == null /*
																 * || client.close
																 */)
					return;
	...
	private void login_server(String charName, LineageClient client) {
		try {
			...
			if (pc.getAbility().getCon() > 75 || pc.getAbility().getStr() > 75 || pc.getAbility().getDex() > 75
					|| pc.getAbility().getCha() > 75 || pc.getAbility().getInt() > 75
					|| pc.getAbility().getWis() > 75) {
				System.out.println("한가지 스탯 70이상 절단 : " + charName);
				client.kick();
				client.close();
				return;
			}
			...
			if (pc.getEarringSlotLevel() > 0) {
				...
				pc.sendPackets(
						new S_ReturnedStat(S_ReturnedStat.RING_RUNE_SLOT_CLOSE, S_ReturnedStat.SUBTYPE_RING, 16));
			}
...
//				}
			...
			if (pc.getNetConnection().getAccount().getBuff_STR() != null) {
				if (sysTime <= pc.getNetConnection().getAccount().getBuff_STR().getTime()) {
					...
					pc.sendPackets(new S_NewUI("완력", (long) bufftime), true);
			...
			if (pc.getNetConnection().getAccount().getBuff_DEX() != null) {
				if (sysTime <= pc.getNetConnection().getAccount().getBuff_DEX().getTime()) {
					...
					pc.sendPackets(new S_NewUI("민첩", (long) bufftime), true);
			...
			if (pc.getNetConnection().getAccount().getBuff_INT() != null) {
				if (sysTime <= pc.getNetConnection().getAccount().getBuff_INT().getTime()) {
					...
					pc.sendPackets(new S_NewUI("지식", (long) bufftime), true);
			...
			if (pc.getNetConnection().getAccount().getBuff_WIS() != null) {
				if (sysTime <= pc.getNetConnection().getAccount().getBuff_WIS().getTime()) {
					...
					pc.sendPackets(new S_NewUI("지혜", (long) bufftime), true);
			...
			//pc.sendPackets(new S_EventNotice(tamcount, tamcount, tamcount, false)); // 이벤트알람
			...
			*/
			...
			if (pc.getSealScrollTime() > 0) {
				if (pc.getSealScrollTime() < currentTime) {
					pc.sendPackets(new S_SystemMessage("봉인해제주문서 [" + pc.getSealScrollCount() + "]장이 지급되었습니다."));
					...
					pc.sendPackets(new S_SystemMessage(
							"해제주문서 발급일은[" + remainDay + "일] [" + remainHour + "시간] [" + remainMin + "분] 남았습니다."));
				}
			}
			// 테베가 열려있는지
			/*
			 * if (CrockSystem.getInstance().isOpen()) { // 연장 상태인지 if
			 * (CrockSystem.getInstance().isCrockIng()) { pc.sendPackets(new
			 * S_SystemMessage("시간의 균열이 열렸습니다. 균열의 힘이 약해져서 닫히는 시간이 연장되어 있는 상태입니다."), true);
			 * } else { pc.sendPackets(new S_SystemMessage("시간의 균열이 열렸습니다."), true); } }
			 */
			...
			/** 인스 던전 입장시에는 무조건 펫이랑 인형정보 삭제 한번 하도록 하자 */
			if(client.getInterServer()){
	...
	private int CheckMail(L1PcInstance pc){ 
		...
		try { 
			...
			SQLUtil.close(rs); SQLUtil.close(pstm1); SQLUtil.close(con); 
		} return count;
	}
			...
			while (rs.next()) {
				...
				try {
					...
					if (l1skills != null && pc.isCrown() && l1skills.getSkillLevel() == 16 && l1skills.getSkillId() == 121) {
                        pc.sendPackets(new S_CreateItem(145, 19, 0));
						pc.브레이브아바타 = true;
					}
					...
					if (l1skills != null && pc.isFencer() && l1skills.getSkillLevel() == 55 
							&& (l1skills.getSkillId() >= 550 && l1skills.getSkillId() <= 561)) {
					...
					} else if (l1skills != null && pc.isFencer() && l1skills.getSkillLevel() == 55 
							&&  l1skills.getSkillId() == 562){
						pc.Pantera_S = true;
						pc.sendPackets(new S_CreateItem(145, 36, 0));
					}
					
					if (l1skills != null && l1skills.getSkillId() > 0) {
						pc.sendPackets(new S_NewAddSkill(S_NewAddSkill.AddskillNew,pc,l1skills.getSkillId()), true);
					}
		...
		try {
			...
			while (rs.next()) {
				...
				case 999:// 드진 스킬아이디
					...
					break;
				/*
				 * case DRAGON_EME_1: if(pc.getDETime2() != null){ if(System.currentTimeMillis()
				 * > pc.getDETime2().getTime()){ pc.sendPackets(new
				 * S_PacketBox(S_PacketBox.DRAGON_EME, 0x01, 0));
				 * pc.getSkillEffectTimerSet().removeSkillEffect(L1SkillId. DRAGON_EME_1);
				 * }else{ long DETIME = pc.getDETime2().getTime()-System.currentTimeMillis() ;
				 * pc.getSkillEffectTimerSet ().removeSkillEffect(L1SkillId.DRAGON_EME_1);
				 * pc.getSkillEffectTimerSet ().setSkillEffect(L1SkillId.DRAGON_EME_1,
				 * (int)DETIME); pc.sendPackets(new S_PacketBox(S_PacketBox.DRAGON_EME, 0x01,
				 * (int)DETIME/1000)); try { pc.save(); } catch (Exception e) { // TODO
				 * Auto-generated catch block e.printStackTrace(); } } } break; case
				 * DRAGON_EME_2: if(pc.getDETime() != null){ if(System.currentTimeMillis() >
				 * pc.getDETime().getTime()){ pc.sendPackets(new
				 * S_PacketBox(S_PacketBox.DRAGON_EME, 0x02, 0));
				 * pc.getSkillEffectTimerSet().removeSkillEffect(L1SkillId. DRAGON_EME_2);
				 * }else{ long DETIME = pc.getDETime().getTime()-System.currentTimeMillis() ;
				 * pc.getSkillEffectTimerSet ().removeSkillEffect(L1SkillId.DRAGON_EME_2);
				 * pc.getSkillEffectTimerSet ().setSkillEffect(L1SkillId.DRAGON_EME_2,
				 * (int)DETIME); pc.sendPackets(new S_PacketBox(S_PacketBox.DRAGON_EME, 0x02,
				 * (int)DETIME/1000)); try { pc.save(); } catch (Exception e) { // TODO
				...
				case SHAPE_CHANGE:
					...
					}else L1PolyMorph.doPoly(pc, poly_id * -1, remaining_time, L1PolyMorph.MORPH_BY_Domination);
					continue;
				case DECAY_POTION:
