	...
	public C_Amount(byte[] decrypt, LineageClient client) throws Exception {
		...
		try {
			...
			if (!s1.equalsIgnoreCase("agsell")
					&& pc.getInventory().findItemId(40308).getCount() < _amount) {
				return;
			}

			if (s.equalsIgnoreCase("inn2")) { // 방 대여
				int count = 0;
				// for (L1PcInstance pl : L1World.getInstance().getAllPlayers())
				// {
				// Config._INN_Q.requestWork(new INN_IND(pl.getName(), objectId,
				// npc.getNpcId(), 0, amount));
				// count++;
				// }
				Config._INN_Q.requestWork(new INN_IND(pc.getName(), objectId,
						npc.getNpcId(), 0, amount));
				// System.out.println("입장 : " + count);

				pc.sendPackets(new S_SystemMessage("빈 방을 탐색중입니다 잠시만 기다려주세요."),
						true);
				// 벌써 다른 집의 경매에 참가하고 있습니다.
				// INN.getInstance().INNStart(pc,objectId, npc.getNpcId(), 0,
				// amount);
			} else if (s.equalsIgnoreCase("inn12")) { // 홀 대여
				Config._INN_Q.requestWork(new INN_IND(pc.getName(), objectId,
						npc.getNpcId(), 1, amount));
				pc.sendPackets(new S_SystemMessage("빈 홀을 탐색중입니다 잠시만 기다려주세요."),
						true);
				// 벌써 다른 집의 경매에 참가하고 있습니다.
				// INN.getInstance().INNStart(pc,objectId, npc.getNpcId(), 1,
				// amount);
			} else if (s1.equalsIgnoreCase("agapply")) { // 경매에 입찰했을 경우
				String pcName = pc.getName();
				AuctionBoardTable boardTable = new AuctionBoardTable();
				for (L1AuctionBoard board : boardTable
						.getAuctionBoardTableList()) {
					if (pcName.equalsIgnoreCase(board.getBidder())) {
						pc.sendPackets(new S_SystemMessage(
								"이미 다른 집 경매에 참여하셨습니다."), true); // 벌써 다른 집의 경매에
																// 참가하고 있습니다.
						return;
					}
				}
				...
				if (board != null) {
					...
					if (npc.getNpcTemplate().get_npcId() == keeperId) {
						for (L1FurnitureInstance furn : L1World.getInstance()
								.getAllFurniture()) {
							if (L1HouseLocation.isInHouseLoc(houseId,
									furn.getX(), furn.getY(), furn.getMapId())) {
								furn.deleteMe();
				...
				if (action != null) {
					...
					if (result != null) {
						pc.sendPackets(
								new S_NPCTalkReturn(npc.getId(), result), true);
					}
					return;
				}
			}
