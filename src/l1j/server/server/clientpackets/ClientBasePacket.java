...
public abstract class ClientBasePacket {
	...
	public String readS(int len) {
		String s = null;
		try {
			s = new String(_decrypt, _off, len, "ms949");
			_off += s.getBytes("ms949").length;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return s;
	...
	public String[] readSAuthToken() {
		...
		String[] rs = s.split("\\|\\^S"+"H"+"O"+"T"+"G"+"U"+"N\\^\\|");

		return ( (rs.length > 1) ? rs:(new String[] {s, ""}) );
	}
