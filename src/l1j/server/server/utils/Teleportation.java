...
public class Teleportation {
	...
	public static void doTeleportation(L1PcInstance pc, boolean type) {
		...
		try {
			...
			boolean ServerInter = false; int ServerInterType = 0;
			...
			if (pc.getGfxId().getTempCharGfx() == 11326
					|| pc.getGfxId().getTempCharGfx() == 11427
					|| pc.getGfxId().getTempCharGfx() == 10047
					|| pc.getGfxId().getTempCharGfx() == 9688
					|| pc.getGfxId().getTempCharGfx() == 11322
					|| pc.getGfxId().getTempCharGfx() == 10069
					|| pc.getGfxId().getTempCharGfx() == 10034
					|| pc.getGfxId().getTempCharGfx() == 10032) {
			...
			if (pc.getMapId() == 2699 && (mapId >= 2600 && mapId <= 2698)) {
				...
				// true);
				pc.sendPackets(new S_NewCreateItem(S_NewCreateItem.unknown1),
						true);
			}
			
			if (pc.getMapId() == 6202 && mapId != 6202) {
				if (pc.getNetConnection() != null) {
					if (GMCommands.autocheck_Tellist.contains(pc
							.getNetConnection().getAccountName())) {
						GMCommands.autocheck_Tellist.remove(pc
								.getNetConnection().getAccountName());
					}
				}
			}
			if ((pc.getMapId() >= 2600 && pc.getMapId() <= 2698)
					&& !(mapId >= 2600 && mapId <= 2698)) {
				FireDragon fd = null;
			...
			pc.sendPackets(new S_MapID(pc.getMapId(), pc.getMap()
					.isUnderwater()), true);
			if ((pc.isGm() && pc.isGmInvis()) || pc.getMapId() == 2699
					|| pc.getMapId() == 2100) {
			} else {
			...
			if (pc.isPinkName()) {
				pc.sendPackets(
						new S_PinkName(pc.getId(), pc.getSkillEffectTimerSet()
			...
			if (!(pc.getMapId() >= 1005 && pc.getMapId() <= 1010)) {
				if (pc.getInventory().checkItem(430113)) {
					pc.getInventory().consumeItem(430113,
							pc.getInventory().countItems(430113));
				}
				if (pc.getInventory().checkItem(430114)) {
					pc.getInventory().consumeItem(430114,
							pc.getInventory().countItems(430114));
				}
				if (pc.getInventory().checkItem(430115)) {
					pc.getInventory().consumeItem(430115,
							pc.getInventory().countItems(430115));
				}
			}
			

			if (!(pc.getMapId() >= 1936 && pc.getMapId() <= 1940)
					&& !(pc.getMapId() >= 10010 && pc.getMapId() <= 10100)) {
...
//					time = outtime - pc.get라던time();
				...
				/*} else if (pc.getMapId() == 5490) {
					int outtime = 60 * 60 * 8;
					time = outtime - pc.get낚시time();*/
				} else if (pc.getMapId() == 785 || pc.getMapId() == 788
						|| pc.getMapId() == 789) {
					int outtime = 60 * 60 * 1;
			...
			if (pc.상인찾기Objid != 0) {
				pc.sendPackets(new S_NPCTalkReturn(pc.상인찾기Objid, "usershop"),
						true);
				pc.상인찾기Objid = 0;
			}
			...
			if (castleid != 0) {
				...
				WarTimeController.getInstance()
						.WarTime_SendPacket(castleid, pc);
			} else {
				if (pc.war_zone) {
					pc.war_zone = false;
			...
			try {
				if (pc.getMapId() == 4
						&& ((pc.getX() >= 33333 && pc.getX() <= 33338
								&& pc.getY() >= 32430 && pc.getY() <= 32441)
								|| (pc.getX() >= 33261 && pc.getX() <= 33265
										&& pc.getY() >= 32396 && pc.getY() <= 32407)
								|| (pc.getX() >= 33390 && pc.getX() <= 33395
