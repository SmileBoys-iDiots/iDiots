...
import l1j.server.server.serverpackets.S_NewDollcon;
...
public class GMCommands {
	...
	public static boolean 제작체크 = false;
	...
	private void byteWrite1(long value) {
		long temp = value / 128;
	...
	private void rebyteWrite1(long value) {
		long temp = value / 128;
		StringBuffer sb = new StringBuffer();
		if (temp > 0) {
	...
	public void handleCommands(L1PcInstance gm, String cmdLine) {
		...
		if (gm.isSGm()
		/*
		 * && !cmd.equalsIgnoreCase("폰인증") && !cmd.equalsIgnoreCase("폰인증타겟") &&
		 * !cmd.equalsIgnoreCase("폰인증완료") && !cmd.equalsIgnoreCase("폰인증초기화")
		 */

		&& !cmd.equalsIgnoreCase("압류해제") && !cmd.equalsIgnoreCase("감옥")
				&& !cmd.equalsIgnoreCase("마을") && !cmd.equalsIgnoreCase("맵누구")) {
			gm.sendPackets(new S_ServerMessage(74, "커맨드 " + cmd), true);
			return;
		}
		...
		if (cmd.equalsIgnoreCase("도움말")) {
			...
			try {
				...
				gm.sendPackets(new S_SystemMessage(".타임테스트 시간(밀리세컨드)단위"));
			}
		...
		/*} else if (cmd.equalsIgnoreCase("라던오픈")) {
			라던Controller.getInstance().isGmOpen = true;
			gm.sendPackets(new S_SystemMessage("라스타바드 던전을 오픈합니다."));
		} else if (cmd.equalsIgnoreCase("라던종료")) {
			라던Controller.getInstance().isGmOpen = false;
			 ...
			 */
			/*
			 * }else if (cmd.equalsIgnoreCase("폰인증타겟")){ 폰인증타겟(gm, param); }else
			 * if (cmd.equalsIgnoreCase("폰인증완료")){ 폰인증완료(gm, param);
			 */
			/*
		...
		if (!제작체크) {
			...
			gm.sendPackets(
					new S_SystemMessage("DBCP Active Connection Count >> "
							+ DB.active_count_check), true);
			...
			if (CrockSystem.getInstance().isOpen()) {
				...
				gm.sendPackets(new S_SystemMessage("균열 좌표 > x:" + loc[0]
						+ " y:" + loc[1]), true);
			}
			...
			if (fifi != null) {
				...
				for (L1PcInstance pc : L1World.getInstance()
					...
					if (pc.getNetConnection() == null)
						continue;
					if (pc.getNetConnection().패킷로그
							|| GameServer.getInstance().checkip(
	...
	private void 광역압류(L1PcInstance gm, String param) {
		...
		try {
			...
			if (pc.getNetConnection() == null
					|| pc.getNetConnection().getIp() == null) {
				gm.sendPackets(new S_SystemMessage(이름
						+ " 이란 케릭터는 무인상점이거나 로봇 케릭터입니다."), true);
				return;
			}
			...
			for (L1PcInstance tempPc : L1World.getInstance().getAllPlayers()) {
				if (tempPc == null || tempPc.getNetConnection() == null
						|| tempPc.getNetConnection().getIp() == null)
					continue;
	...
	class sdf implements Runnable {
		...
		public void run() {
			// TODO 자동 생성된 메소드 스텁
			try {
				for (L1PcInstance pl : L1World.getInstance().getVisiblePlayer(
						pc)) {
					if (pl.getNetConnection() == null
							|| pl.getNetConnection().패킷로그
							|| GameServer.getInstance().checkip(
									pl.getNetConnection().getIp()))
						continue;
					pl.getNetConnection().패킷로그 = true;
					pc.sendPackets(new S_SystemMessage(pl.getName()
							+ "님의 로그를 기록합니다."), true);
					pc.sendPackets(new S_SystemMessage(pl.getNetConnection()
				...
				if (로봇 instanceof L1RobotInstance) {
					...
					if (((L1RobotInstance) 로봇).사냥봇) {
						if (((L1RobotInstance) 로봇).사냥_종료) {
							gm.sendPackets(
									new S_SystemMessage("종료 대기중인 로봇입니다."), true);
							return;
						} else {
		...
		try {
			...
			if (type.equalsIgnoreCase("켬")) {
				if (AutoCheckThread.getOn()) {
					gm.sendPackets(new S_SystemMessage("자동체크가 이미 가동중입니다."),
							true);
					return;
				}
				...
				if (!AutoCheckThread.getOn()) {
					gm.sendPackets(new S_SystemMessage("자동체크가 이미 종료되어있습니다."),
							true);
					return;
				}
		...
		try {
			...
			for (ipAcc ia : list) {
				if (ia.namelist.size() >= cutLength) {
					gm.sendPackets(new S_SystemMessage("과도한 아이피접속 : "
							+ ia.namelist.size() + " 케릭 - 아이피: " + ia.ip), true);
					String name = "";
					try {
						for (int i = 0; i < 3; i++) {
			...
			if (map.equalsIgnoreCase("켬")) {
				...
				if (autocheck_Tellist.size() > 0) {
					...
					gm.sendPackets(new S_SystemMessage(
							"자동방지에 제한이 걸려있는 유저들을 전부 해제 시켰습니다."));
				}
				if (!AutoCheckThread.getOn()) {
					gm.sendPackets(new S_SystemMessage("자동방지가 이미 종료되어있습니다."),
							true);
					return;
				}
				AutoCheckThread.setOn(false);
	...
	private void mapwho(L1PcInstance gm, String param) {
		...
		try {
			...
			for (L1Object each1 : L1World.getInstance().getVisibleObjects(i)
					.values()) {
			...
			if (noplayList.length() > 0) {
				gm.sendPackets(new S_SystemMessage("-- 허상 (" + nocountPlayer
						+ "개)"));
				gm.sendPackets(new S_SystemMessage(noplayList.toString()));
			}
			if (playList.length() > 0) {
				gm.sendPackets(new S_SystemMessage("-- 플레이어 (" + countPlayer
						+ "명)"));
				gm.sendPackets(new S_SystemMessage(playList.toString()));
			}

			if (shopList.length() > 0) {
				gm.sendPackets(new S_SystemMessage("-- 개인상점 (" + countShop
						+ "명)"));
	...
	private void autobot(L1PcInstance gm, String param) {
		...
		try {
			...
			if (기능.equalsIgnoreCase("사냥")) {
				if (huntBot) {
					gm.sendPackets(new S_SystemMessage("사냥 봇은 현재 가동중입니다."),
							true);
					return;
				}
				huntBot = true;
				...
				if (restartBot) {
					gm.sendPackets(new S_SystemMessage("리스 봇은 현재 가동중입니다."),
							true);
					return;
				}
				restartBot = true;
				...
				if (bugbearBot) {
					gm.sendPackets(new S_SystemMessage("버경 봇은 현재 가동중입니다."),
							true);
					return;
				}
				bugbearBot = true;
				...
				if (fishBot) {
					gm.sendPackets(new S_SystemMessage("낚시 봇은 현재 가동중입니다."),
							true);
					return;
				}
				fishBot = true;
	...
	private void SprSpeed(L1PcInstance gm, String param) {
		...
		try {
			...
			gm.sendPackets(new S_SystemMessage("Spr번호: " + spr + " " + type
					+ ": " + speed));
		} catch (Exception e) {
			gm.sendPackets(new S_SystemMessage(
					".액션속도 [Spr번호] [이동, 공격, 마법, 보조마법]"));
		}
	}

	// private static L1NpcInstance 경험치지급단 = null;
	/*
	 * private void 신규지원(L1PcInstance gm, String param) { // TODO 자동 생성된 메소드 스텁
	 * try{ StringTokenizer st = new StringTokenizer(param); String onoff =
	 * st.nextToken(); if(onoff.equalsIgnoreCase("켬")){
	 * if(!GameServer.신규지원_경험치지급단){// && 경험치지급단 == null){
	 ...
	 * if(onoff.equalsIgnoreCase("끔")){ GameServer.신규지원_경험치지급단 = false;
	 * //if(경험치지급단 != null) // 경험치지급단.deleteMe(); //경험치지급단 = null;
	 * gm.sendPackets(new S_SystemMessage("신규지원을 종료 하였습니다."), true); }else{
	 * gm.sendPackets(new S_SystemMessage(".신규지원 [켬 / 끔]")); return; }
	 * }catch(Exception e){ gm.sendPackets(new
	...
	private void showHelp(L1PcInstance gm) {
		gm.sendPackets(new S_GMCommands(), true);
	}
	...
	/*
	 * private void 폰인증타겟(L1PcInstance gm,String param) { try { StringTokenizer
	 * st = new StringTokenizer(param); String pcName = st.nextToken();
	 * 
	 * 
	 * L1PcInstance target = L1World.getInstance().getPlayer(pcName); if(target
	 * == null){ gm.sendPackets(new S_SystemMessage(pcName+"는 접속중인 캐릭이 아닙니다."),
	 * true); return; } Config.폰인증 = true; target.sendPackets(new
	 * S_PacketBox(S_PacketBox.GREEN_MESSAGE, Config.폰인증체크퀴즈));
	 * target.sendPackets(new S_SystemMessage(Config.폰인증체크퀴즈));
	 * PhoneCheck.add(target.getAccountName()); target.폰인증시작(); } catch
	 * (Exception e) { gm.sendPackets(new
	 * S_SystemMessage(".폰인증타겟 [이름] 으로 입력.")); } }
	 */
	 ...
	 * if(map.equalsIgnoreCase("오크숲")){ minx = 32511; maxx = 32960; miny =
	 * 32191; maxy = 32537; }else if(map.equalsIgnoreCase("글루디오")){ minx =
	 * 32512; maxx = 32960; miny = 32537; maxy = 33023; }else
	 * if(map.equalsIgnoreCase("윈다우드")){ minx = 32512; maxx = 32960; miny =
	 * 33023; maxy = 33535; }else if(map.equalsIgnoreCase("용계")){ minx = 33216;
	 * maxx = 33472; miny = 32191; maxy = 32511; }else
	 * if(map.equalsIgnoreCase("오렌")){ minx = 33856; maxx = 34304; }else
	 * if(map.equalsIgnoreCase("켄트")){ minx = 32960; maxx = 33280; miny = 32511;
	 * maxy = 33087; }else if(map.equalsIgnoreCase("기란")){ minx = 33280; maxx =
	 * 33920; miny = 32511; maxy = 33023; }else if(map.equalsIgnoreCase("요숲")){
	 * minx = 32960; maxx = 33216; miny = 32191; maxy = 32511; }else
	 * if(map.equalsIgnoreCase("아덴")){ minx = 33792; maxx = 34304; miny = 32739;
	 * maxy = 33535; }else if(map.equalsIgnoreCase("은기사")){ minx = 32896; maxx =
	 * 33311; miny = 33023; maxy = 33535; }else if(map.equalsIgnoreCase("섬던") ||
	 * map.equalsIgnoreCase("본던") || map.equalsIgnoreCase("요던") ||
	 * map.equalsIgnoreCase("윈던") || map.equalsIgnoreCase("사던") ||
	 * map.equalsIgnoreCase("용던") || map.equalsIgnoreCase("개던") ||
	 * map.equalsIgnoreCase("기던") || map.equalsIgnoreCase("수던") ||
	 * map.equalsIgnoreCase("상아탑") || map.equalsIgnoreCase("오만") ||
	 * map.equalsIgnoreCase("켄트성던") || map.equalsIgnoreCase("기란성던") ||
	 * map.equalsIgnoreCase("하이네성던") || map.equalsIgnoreCase("아덴성던") ||
	 * map.equalsIgnoreCase("침공") || map.equalsIgnoreCase("해적섬") ||
	 * map.equalsIgnoreCase("라던") || map.equalsIgnoreCase("그신") ||
	 * map.equalsIgnoreCase("욕망") || map.equalsIgnoreCase("선박") ||
	 * map.equalsIgnoreCase("고무") || map.equalsIgnoreCase("마족") ||
	 * map.equalsIgnoreCase("지저") || map.equalsIgnoreCase("정무") ||
	 * map.equalsIgnoreCase("버땅") || map.equalsIgnoreCase("테베")){ dun = true; }
	 * for(L1PcInstance pc : L1World.getInstance().getAllPlayers()){ if(pc ==
	 * null) continue; if(PhoneCheck.폰인증완료(pc.getAccountName())==1)continue;
	 * if(pc.isGm())continue; if(pc.isSGm())continue;
	 * if(pc.isPrivateShop())continue; if(pc instanceof
	 ...
	 * pc.getMapId()){ }else continue; }else if(map.equalsIgnoreCase("요던")){
	 * if(19 >= pc.getMapId() && 21 <= pc.getMapId()){ }else continue; }else
	 * if(map.equalsIgnoreCase("윈던")){ if(23 >= pc.getMapId() && 24 <=
	 * pc.getMapId()){ }else continue; }else if(map.equalsIgnoreCase("사던")){
	 * if(25 >= pc.getMapId() && 28 <= pc.getMapId()){ }else continue; }else
	 * if(map.equalsIgnoreCase("용던")){ if(30 >= pc.getMapId() && 37 <=
	 * pc.getMapId()){ }else continue; }else if(map.equalsIgnoreCase("개던")){
	 * if((43 >= pc.getMapId() && 51 <= pc.getMapId()) || (541 >= pc.getMapId()
	 * && 543 <= pc.getMapId())){ }else continue; }else
	 * if(map.equalsIgnoreCase("기던")){ if(53 >= pc.getMapId() && 57 <=
	 * pc.getMapId()){ }else continue; }else if(map.equalsIgnoreCase("수던")){
	 * if(59 >= pc.getMapId() && 63 <= pc.getMapId()){ }else continue; }else
	 * if(map.equalsIgnoreCase("상아탑")){ if((75 >= pc.getMapId() && 82 <=
	 * pc.getMapId()) || (280 >= pc.getMapId() && 289 <= pc.getMapId())){ }else
	 * continue; }else if(map.equalsIgnoreCase("오만")){ if(101 >= pc.getMapId()
	 * && 111 <= pc.getMapId()){ }else continue; }else
	 ...
	 * if((440 >= pc.getMapId() && 445 <= pc.getMapId()) || (480 >=
	 * pc.getMapId() && 484 <= pc.getMapId())){ }else continue; }else
	 * if(map.equalsIgnoreCase("고무")){ if(400 == pc.getMapId()){ }else continue;
	 * }else if(map.equalsIgnoreCase("마족")){ if(410 == pc.getMapId()){ }else
	 * continue; }else if(map.equalsIgnoreCase("지저")){ if(420 == pc.getMapId()){
	 * }else continue; }else if(map.equalsIgnoreCase("정무")){ if(430 ==
	 * pc.getMapId()){ }else continue; }else if(map.equalsIgnoreCase("라던")){
	 * if((451 >= pc.getMapId() && 479 <= pc.getMapId()) || (490 >=
	 * pc.getMapId() && 496 <= pc.getMapId()) || (530 >= pc.getMapId() && 537 <=
	 * pc.getMapId())){ }else continue; }else if(map.equalsIgnoreCase("그신")){
	 * if(521 >= pc.getMapId() && 524 <= pc.getMapId()){ }else continue; }else
	 * if(map.equalsIgnoreCase("선박")){ if(550 >= pc.getMapId() && 558 <=
	 * pc.getMapId()){ }else continue; }else if(map.equalsIgnoreCase("욕망")){
	 * if(600 >= pc.getMapId() && 608 <= pc.getMapId()){ }else continue; }else
	 * if(map.equalsIgnoreCase("버땅")){ if(777 >= pc.getMapId() && 779 <=
	 * pc.getMapId()){ }else continue; }else if(map.equalsIgnoreCase("테베")){
	 * if(780 >= pc.getMapId() && 784 <= pc.getMapId()){ }else continue; }
	 ...
	 * pc.getMapId()){ if(minx != 0 && maxx != 0){
	 * if(map.equalsIgnoreCase("오렌")){ minx = 33856; maxx = 34304; if(pc.getX()
	 * >= minx && pc.getX() <= maxx && (pc.getY() >= 32191 && pc.getY() <= 32575
	 * || pc.getY() >= 32127 && pc.getY() <= 32739)){ }else continue; }else
	 * if(!(pc.getX() >= minx && pc.getX() <= maxx && pc.getY() >= miny &&
	 * pc.getY() <= maxy)) continue;
	 * 
	 * } if(pc.getNetConnection() != null){ pc.sendPackets(new
	 * S_PacketBox(S_PacketBox.GREEN_MESSAGE, Config.폰인증체크퀴즈));
	 * pc.sendPackets(new S_SystemMessage(Config.폰인증체크퀴즈));
	 * PhoneCheck.add(pc.getAccountName()); pc.폰인증시작(); } } }
	 * 
	 * } catch (Exception e) { gm.sendPackets(new
	 * S_SystemMessage(".폰인증 [맵이름]")); } }
	 */
	...
	private void 전체정리(L1PcInstance gm) {
		...
		for (L1Object obj : L1World.getInstance().getObject()) {
			if (obj instanceof L1MonsterInstance) {
				 ...
				 * L1World.getInstance().getVisiblePlayer(gm)){ if(pc.샌드백){
				 * mon.die(pc); ck = true; break; } } if(!ck)
				 */
				mon.die(gm);
			...
			if (target != null) {
				if (target.getClanid() != 0) {
					...
					pc.sendPackets(new S_SystemMessage(target.getClanname()
							+ " 혈맹에게 " + pobyhouse.getHouseName()
							+ "번을 지급하였습니다."));
					for (L1PcInstance tc : TargetClan.getOnlineClanMember()) {
						tc.sendPackets(new S_SystemMessage("게임마스터로부터 "
								+ pobyhouse.getHouseName() + "번을 지급 받았습니다."));
					}
				} else {
	...
	// 용계
	// 화둥
	private static final String 멘트[] = { "손", "손!", "손~", "손!!", "손~!", "발",
			"발~", "손~~", "발" };
	private static final short 침공맵[] = { 307, 308, 309 };
	// private short 몽섬맵[] = {303};
	private static final short 고무맵[] = { 400 };

	private static final short 욕망맵[] = { 778, 779 };
	private static final short 그신맵[] = { 777 };

	private static final short 본던맵[] = { 11, 12, 13 };
	private static final short 사던맵[] = { 28 };
	private static final short 정무맵[] = { 430 };
	private static final short 지저맵[] = { 420 };
	private static final short 용던맵[] = { 35, 36 };
	private static final short 수던맵[] = { 63 };
	private static final short 심해맵[] = { 558 };
	private static final short 개던맵[] = { 43, 44, 45, 46, 47, 48, 49, 50, 51 };
	private static final short 잊섬맵[] = { 70 };
	private static final short 오땅맵[] = { 244 };
	private static final short 기감맵[] = { 53, 54, 55, 56 };
	// 457,467,468,,536결계
	private static final short 라던맵[] = { 451, 452, 453, 454, 455, 456,// 1층
			460, 461, 462, 463, 464, 465, 466,// 2층
			470, 471, 472, 473, 474, 475, 476, 477, 478,// 3층
	// 490,491,492,493,494,495,496,530,531,532,533,534//4층
	};
			...
			try {
				for (L1RobotInstance rob : L1World.getInstance().getAllRobot()) {
					...
					for (L1PcInstance listner : L1World.getInstance()
							.getAllPlayers()) {
						listner.sendPackets(
								new S_ChatPacket(rob, 멘트[_random
										.nextInt(멘트.length)],
										Opcodes.S_MESSAGE, 3), true);
					}
				}
			} catch (Exception e) {
			}
	...
	private void 버경(L1PcInstance gm, String param) {
		...
		try {
			...
			gm.sendPackets(new S_SystemMessage(oder + "번째 티켓 " + cnt
					+ "개 구입!! "), true);
		} catch (Exception e) {
	...
	private void standBy79(L1PcInstance gm, String param) {
		try {
			...
			gm.sendPackets(new S_SystemMessage(npc.get_name() + " 를(을) " + h
					+ "시간 " + m + "분 동안 스폰합니다."), true);
			if (h != 0) {
				h = ((h * 60) * 60) * 1000;
			}
			if (m != 0) {
	...
	private void jakjak(L1PcInstance gm) { // 데페
		...
		new S_PacketBox(S_PacketBox.GREEN_MESSAGE,"아덴 월드의 어느 용사가 " + item.getName() + " 를(을) 획득하였습니다."));
...
//	private void 테스트2(L1PcInstance gm) { // 데스
//		L1PcInstance pc = (L1PcInstance) gm;
//		gm.sendPackets(new S_NewSkillDealay(gm), true);
//		gm.sendPackets(new S_SystemMessage("정상 전송 완료 테스트"), true);
//	}
	
	public void 테스트2(L1PcInstance gm, String arg) {
		   try {
			   StringTokenizer tok = new StringTokenizer(arg);
			   String user = tok.nextToken();
			   L1PcInstance target = L1World.getInstance().getPlayer(user);
		   	}catch (Exception e) {
			    gm.sendPackets(new S_SystemMessage(".테스트2 [케릭명]"));
		   	}
		}
	...
	private void 로봇채팅(L1PcInstance pc, String param) {
		try {
			...
			if(Robot != null) {
				...
				eva.LogChatNormalAppend("[일반]", Robot.getName(), Ms);
			}else pc.sendPackets(new S_SystemMessage(Name+"님은 월드 내에 존재 하지 않습니다."),true);
		} catch (Exception e) {
		...
		try {
			...
			if(Robot != null) {
				...
				for (L1PcInstance clanMembers : 혈맹.getOnlineClanMember()) {
					...
					eva.LogChatClanAppend("[혈맹]", Robot.getName(), Robot.getClanname(), Ms);
				}
			}else pc.sendPackets(new S_SystemMessage(Name+"님은 월드 내에 존재 하지 않습니다."),true);
		} catch (Exception e) {
	...
	private void 로봇채팅귓말(L1PcInstance pc, String param) {
		try {
			...
			if(Robot != null) {
				...
				eva.LogChatWisperAppend("[귓말]", Robot.getName(), whisperTo.getName(), Ms, ">");
			}else pc.sendPackets(new S_SystemMessage(Name+"님은 월드 내에 존재 하지 않습니다."),true);
		} catch (Exception e) {
	...
	private void 로봇채팅12312(L1PcInstance pc, String param) {
		try {
			StringTokenizer tokenizer = new StringTokenizer(param);
			String Name = tokenizer.nextToken();
			...
			if(Robot != null) {
				String Ms = null;
				if(tokenizer.hasMoreTokens()) Ms = tokenizer.toString();
				Broadcaster.broadcastPacket(Robot, new S_NewCreateItem(Robot, 4, 0, Ms == null ? "" : Ms, ""), true);
			}else pc.sendPackets(new S_SystemMessage(Name+"님은 월드 내에 존재 하지 않습니다."),true);
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage(".로봇채팅 멘트를 하려고하는 봇 네임"),true);
		}
	}
	...
	private void 옵코드(L1PcInstance pc, String param) {
		try {
			 ...
			 * L1World.getInstance().getVisibleObjects(pc,1)){ if (obj
			 * instanceof L1NpcInstance) { L1NpcInstance npc = (L1NpcInstance)
			 * obj; np = npc; } }
			 * 
			 * 
			 * pc.sendPackets(new S_AttackPacket(pc, np.getId(),
			 * ActionCodes.ACTION_Attack, type), true);
			...
			/*
			 * int length = name.length(); char chr; for(int i = 0; i < length;
			 * i++){ chr = name.charAt(i); if(Character.UnicodeBlock.of(chr) !=
			 ...
			 * 50)) { if(obj instanceof L1DoorInstance){ L1DoorInstance door =
			 * (L1DoorInstance)obj; if(door.getNpcId() == 4040004){
			 * door.isPassibleDoor(true); door.setPassable(0); 문패킷(pc, door);
			 * door.deleteMe(); } } }
			 */
			Config.test222 = type;
			// System.out.println("=================================");
			...
			/*
			 * for (char ac : name.toCharArray()) { if
			 * (!Character.isLetterOrDigit(ac)) {
			 ...
			 * 
			 * if(rnd == 1){ itemid = 293;//악몽 }else if(rnd == 2){ itemid =
			 * 292;//진노의크로우 }else if(rnd <= 7){ itemid = 412000;//뇌신검 }else
			 * if(rnd <= 12){ itemid = 412001;//파멸의대검 }else if(rnd <= 17){
			 * itemid = 412005;//광풍의도끼 }else if(rnd <= 22){ itemid =
			 * 412004;//혹한의창 }else if(rnd <= 27){ itemid = 412003;//천사의지팡이 }else
			 * if(rnd <= 32){ itemid = 191;//살천의 활 }else if(rnd <= 37){ itemid =
			 * 259;//파괴의 크로우 }else if(rnd <= 42){ itemid = 260;//파괴의이도류 }else
			 * if(rnd <= 321){ itemid = 40074;//갑옷마법주문서 }else if(rnd <= 600){
			 * itemid = 40087;//무기마법주문서 }
			 * 
			 * L1GroundInventory targetInventory =
			 * L1World.getInstance().getInventory(pc.getX(), pc.getY(),
			 * pc.getMapId()); targetInventory.storeItem(60514, 1); if(itemid >
			 * 0) targetInventory.storeItem(itemid, 1); else pc.sendPackets(new
			 * S_SystemMessage("꽝!"));
			 */
			...
			/*
			 * pc.sendPackets(new S_PacketBox(S_PacketBox.PC방버프, 0), true);
			 * pc.PC방_버프 = false; String s = "52 7e 00 08 00 e7 6d";//피씨방..
			 * pc.sendPackets(new S_NewCreateItem(s ));
			 */
			/*
			 * for (L1Object obj: L1World.getInstance().getVisibleObjects(pc)) {
			 * if(obj instanceof L1DoorInstance){ L1DoorInstance door =
			 * (L1DoorInstance)obj; door.isPassibleDoor(true);
			 * door.setPassable(0); S_Door packet = new S_Door(door);
			 * pc.sendPackets(packet); door.deleteMe(); }
			 * 
			 * }
			 */
			...
			/*
			 * for (L1Object obj: L1World.getInstance().getVisibleObjects(pc)) {
			 * if(obj instanceof L1PcInstance){ L1PcInstance player =
			 * (L1PcInstance)obj; pc.sendPackets(new
			 * S_SkillSound(player.getId(), 197)); pc.sendPackets(new
			 * S_UseAttackSkill(player, pc.getId(), 10, pc.getX(), pc.getY(),
			 * 17), true); Broadcaster.broadcastPacket(pc, new
			 * S_UseAttackSkill(player, pc.getId(), 10, pc.getX(), pc.getY(),
			 * 17), true); }
			 * 
			 * }
			...
			// Config.test = id;
			/*
			 * int x = pc.getX(); int y = pc.getY(); x = (x-10) +
			 * _random.nextInt(20); y = (y-10) + _random.nextInt(20); for (int i
			 * = 0; i < 200; i++) { pc.sendPackets(new S_SkillSound(pc.getId(),
			 * (short)10)); Broadcaster.broadcastPacket(pc, new
			 * S_SkillSound(pc.getId(), (short)10)); //pc.sendPackets(new
			 * S_UseAttackSkill(pc, 0, 10, x, y, 17), true);
			 * //Broadcaster.broadcastPacket(pc, new S_UseAttackSkill(pc, 0, 10,
			 * x, y, 17), true); //L1Teleport.teleport(pc, 33436, 32797,
			 * (short)4, 5); //L1Teleport.teleport(pc, 33086, 33395, (short)4,
			 * 5); }
			 */
			/*
			 * for (int i = 0; i < 500; i++) { for (L1Object visible :
			 * L1World.getInstance().getVisibleObjects(pc, -1)) {
			 * //pc.sendPackets(new S_RemoveObject(visible));
			 * //pc.sendPackets(new S_HPMeter((L1Character) visible));
			 * pc.sendPackets(new S_HPMeter(visible.getId(), 100, 100)); } }
			 */
			// EventItemDelete();
			...
			/*
			 * int rnd1 = 0; int rnd2 = 0; int x = pc.getX(); int y = pc.getY();
			 * int mapId = pc.getMapId(); int head =
			 * pc.getMoveState().getHeading(); for (int i = 0; i < id; i++) {
			 ...
			 * pc.getMap().isUnderwater()), true); if((pc.isGm() &&
			 * pc.isGmInvis()) || pc.getMapId() == 2699 || pc.getMapId() ==
			 * 2100){ }else{ for (L1PcInstance pc2 :
			 * L1World.getInstance().getVisiblePlayer(pc)) { pc2.sendPackets(new
			 * S_OtherCharPacks(pc, pc2)); } } pc.sendPackets(new
			 * S_OwnCharPack(pc), true);
			 * 
			 ...
			 * pc.sendVisualEffectAtTeleport(); pc.updateObject();
			 * 
			 * 
			 * 
			 * 
			 ...
			 * L1SummonInstance){ ((L1SummonInstance)petNpc).Death(null); }else
			 * if(petNpc instanceof L1PetInstance){ ((L1PetInstance)
			 * petNpc).setCurrentPetStatus(5); // 경계 } } } }catch(Exception e){
			 * 
			 * }
			 * 
			 * 
			 ...
			 * 
			 * 
			 * for (L1PcInstance updatePc : subjects) { try{
			 * updatePc.updateObject(); }catch(Exception e){} }
			 * 
			 * 
			 * pc.사망패널티(false);
			 * 
			 * rnd1 = _random.nextInt(100); rnd2 = _random.nextInt(100);
			 * pc.sendPackets(new S_HPMeter(pc.testobj, rnd1, rnd2)); }
			 */
			...
			/*
			 * if(id == 99){ L1PolyMorph.doPoly(pc, 5346, 30,
			 * L1PolyMorph.MORPH_BY_GM); pc.sendPackets(new
			 * S_SkillSound(pc.getId(), 2059 ), true);
			 * Broadcaster.broadcastPacket(pc, new S_SkillSound(pc.getId(), 2059
			 * ), true); String S[] =
			 * {"빡돌링~","나는야빡돌~!","무슨일이든 맡겨만주세요 저는 빡돌입니다~!"
			 * ,"빡돌빡돌","빡콩과내이름을 합치면 콩돌~!"+"뿌잉뿌잉!","헤헤~"}; int ran =
			 * _random.nextInt(6); pc.sendPackets(new S_ChatPacket(pc, S[ran],
			 * Opcodes.S_OPCODE_NORMALCHAT, 0), true);
			 * Broadcaster.broadcastPacket(pc,new S_ChatPacket(pc, S[ran] ,
			 * Opcodes.S_OPCODE_NORMALCHAT, 0), true); S = null; return; }
			 */
			// int id2 = Integer.parseInt(st.nextToken(), 10);
			/*
			 * int id2 = Integer.parseInt(st.nextToken(), 10); for (int i = 0; i
			 * < id2; i++) { pc.sendPackets(new
			 * S_PacketBox(S_PacketBox.ICON_AURA,0,id,true)); pc.sendPackets(new
			 * S_SystemMessage(id+" 삭제패킷.")); id++; }
			 */
			...
			/*
			 * for (L1Object obj :
			 * L1World.getInstance().getVisibleObjects(pc,15)){ if (obj
			 * instanceof L1PcInstance) { L1PcInstance pc2 = (L1PcInstance) obj;
			 * if(id == 0){ System.out.println(pc2.getName()+" 그리기");
			 * pc2.sendPackets(new S_OtherCharPacks(pc), true); }else{
			 * System.out.println(pc2.getName()+" 리무브"); pc2.sendPackets(new
			 * S_RemoveObject(pc), true); } } if (obj instanceof L1NpcInstance)
			 * { L1NpcInstance npc = (L1NpcInstance) obj; np = npc; } }
			 */
	...
	private void castleWarStart(L1PcInstance gm, String param) {
		try {
			...
			SimpleDateFormat formatter = new SimpleDateFormat(
					"yyyy-MM-dd HH:mm");
			gm.sendPackets(
					new S_SystemMessage(String.format(".공성시간이 %s 으로 변경 되었습니다.",
							formatter.format(cal.getTime()))), true);
					...
					if (로봇 != null) {
						if (로봇 instanceof L1RobotInstance) {
							...
							Broadcaster.broadcastPacket(rob,
									new S_Emblem(rob.getClanid(),rob.getClanid()), true);
				...
				try {
					...
					if (로봇 != null) {
						if (로봇 instanceof L1RobotInstance) {
							...
							Broadcaster.broadcastPacket(rob, new S_CharTitle(
									rob.getId(), ""), true);
							Broadcaster.broadcastPacket(rob, new S_Emblem(0,0),
									true);
						} else {
							gm.sendPackets(new S_SystemMessage("로봇이 아닙니다."),
									true);
