		...
		try {
			while (true) {
				...
				if (L1World.getInstance().getAllPlayers().size() <= 0)
					continue;
                /**악영시간때만엔피씨생성**/
				L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "악마왕의 영토가 열렸습니다!")); 
				//L1SpawnUtil.spawn2(33430, 32797, (short) 4, 4100039, 0, 3800000, 0);//법진
				L1SpawnUtil.spawn2(33441, 32824, (short) 4, 777017, 0, 3800000, 0);//텔레포터 
				/**악영시간때만엔피씨생성**/
				isGmOpen = false;
				/** 오픈 메세지 * */
