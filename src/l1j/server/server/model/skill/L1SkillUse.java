...
public class L1SkillUse {
	...
	private int _AccelatorDecrease;
	...
	private static final int[] CAST_WITH_INVIS = { 1, 2, 3, 5, 8, 9, 12, 13, 14, 19, 21, 26, 31, 32, 35, 37, 42, 43, 44,
			48, 49, 52, 54, 55, 57, 60, 61, 63, 67, 68, 69, 72, 73, 75, 78, 79, REDUCTION_ARMOR, 마제스티, BOUNCE_ATTACK,
			...
			IllUSION_DIAMONDGOLEM, IllUSION_LICH, IllUSION_AVATAR, 인페르노, HALPHAS };

	// 카운터 매직으로 방어할수 없는 마법
	private static final int[] EXCEPT_COUNTER_MAGIC = { 1, 2, 3, 5, 8, 9, 12, 13, 14, 19, 21, 26, 31, 32, 35, 37, 42,
			43, 44, 48, 49, 52, 54, 55, 57, 60, 61, 63, 67, 68, 69, 72, 73, 75, 78, 79,FORCE_STUN,ETERNITY,
			AVENGER,DEMOLITION,SHOCK_STUN, 엠파이어, 파워그립, 데스페라도, PANTERA, PHANTOM, BLADE,
			REDUCTION_ARMOR, BOUNCE_ATTACK, SOLID_CARRIAGE, 마제스티, COUNTER_BARRIER, HALPHAS, 인페르노, 97, 98, 99, 100, 101, 102, 104,
			105, 106, 107, 109, 110, 111, ARMOR_BREAK, 113, 114, 115, 116, 117, 118, 129, 130, 131, 132, 134, 137, 138, 146, 147,
			148, 149, 150, 151, 155, 156, 158, 159, 161, 163, 164, 165, 166, 168, 169, 170, 171, 181, SOUL_OF_FLAME,
			...
			10026, 10027, 10028, 10029, 30060, 30000, 30078, 30079, 30011, 30081, 30082, 30083, 30080, 30084, 30010,
			9278, 9279, 30002, 30086, 100091, 100092 };
	...
	public boolean checkUseSkill(L1PcInstance player, int skillid, int target_id, int x, int y, String message,
		...
		/*
		 * || _skillId == CUBE_IGNITION || _skillId == CUBE_QUAKE || _skillId ==
		 * CUBE_SHOCK || _skillId == CUBE_BALANCE
		 */) {
			return true;
		}
	...
	private boolean isNormalSkillUsable() {
		if (_user instanceof L1PcInstance) {
			...
			if (pc.getInventory().calcWeightpercent() >= 83
					&& !pc.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.RANKING_BUFF_1)) { // 중량 오버이면 스킬을
				// 사용할 수 없다
			...
			if (pc.getMapId() == 5302 || pc.getMapId() == 5153 || pc.getMapId() == 5490) {// 배틀존
				if (_skillId == 69 || _skillId == 78 || _skillId == 71 || _skillId == 39 || _skillId == 20
						|| _skillId == 33 || _skillId == 67 || _skillId == 61 || _skillId == 75 || _skillId == 157
						|| _skillId == 44 || _skillId == 60) {
			...
			if (pc.getSkillEffectTimerSet().hasSkillEffect(SILENCE)
				...
				if (!(_skillId >= SHOCK_STUN && _skillId <= COUNTER_BARRIER) && !(_skillId >= 226 && _skillId <= 230) && !(_skillId == PRIME)  && !(_skillId == FORCE_STUN)
						&& !(_skillId == DEMOLITION) && !(_skillId == 엠파이어) && !(_skillId == HALPHAS) && !(_skillId == AVENGER) && !(_skillId >= 235 && _skillId <= 239)) {
					pc.sendPackets(new S_ServerMessage(285), true);
					return false;
				}
			}
	...
	private boolean isTarget(L1Character cha) throws Exception {
		...
		if ((cha.getSkillEffectTimerSet().hasSkillEffect(ICE_LANCE)) && (_skillId == ICE_LANCE || _skillId == SHOCK_STUN
				|| _skillId == 데스페라도 || _skillId == 엠파이어 || _skillId == 파워그립 || _skillId == FORCE_STUN 
				|| _skillId == ETERNITY || _skillId == DEMOLITION || _skillId == AVENGER || _skillId == PANTERA || _skillId == PHANTOM)) {
			return false;
		}
		...
		if (cha instanceof L1PcInstance) {
			...
			if (pc.getSkillEffectTimerSet().hasSkillEffect(ABSOLUTE_BARRIER)
				...
				/* || _skillId == DISEASE */ || _skillId == FOG_OF_SLEEPING || _skillId == SLOW
						|| _skillId == CANCELLATION || _skillId == SILENCE || _skillId == DECAY_POTION
						|| _skillId == MASS_TELEPORT || _skillId == DETECTION || _skillId == HORROR_OF_DEATH
						|| _skillId == COUNTER_DETECTION || _skillId == GUARD_BREAK || _skillId == ERASE_MAGIC
						|| _skillId == FREEZING_BREATH || _skillId == ENTANGLE || _skillId == FEAR
						|| _skillId == PHYSICAL_ENCHANT_DEX || _skillId == PHYSICAL_ENCHANT_STR
						|| _skillId == BLESS_WEAPON || _skillId == FIRE_SHIELD || _skillId == IMMUNE_TO_HARM
		...
		if (_calcType == PC_PC) {
			if (CharPosUtil.getZoneType(_player) == 1 || CharPosUtil.getZoneType(cha) == 1) {
				if (_skillId == L1SkillId.CURE_POISON || _skillId == L1SkillId.MIND_BREAK || _skillId == L1SkillId.PANIC
						...
						|| _skillId == L1SkillId.WEAKNESS || _skillId == L1SkillId.SILENCE
						|| _skillId == L1SkillId.FOG_OF_SLEEPING || _skillId == L1SkillId.DECAY_POTION
						|| _skillId == L1SkillId.EARTH_BIND || _skillId == L1SkillId.WIND_SHACKLE
						|| _skillId == L1SkillId.PANTERA || _skillId == L1SkillId.BLADE || _skillId == L1SkillId.PHANTOM
						|| _skillId == L1SkillId.ELEMENTAL_FALL_DOWN)
					_flg = false;
			}
		}
	...
	private void makeTargetList() {
		try {
			...
			// System.out.println(CharPosUtil.isAreaAttack(_user, _target.getX(),
			// _target.getY(),
			// _target.getMapId()));
			// System.out.println(CharPosUtil.isAreaAttack(_target,
			...
			if (_skill.getTarget().equals("buff") && _calcType == PC_PC
					&& (CharPosUtil.isAreaAttack(_user, _target.getX(), _target.getY(), _target.getMapId()) == false
							|| CharPosUtil.isAreaAttack(_target, _user.getX(), _user.getY(),
									_user.getMapId()) == false)) {
	...
	private boolean isAttrAgrees() {
		...
		if (!_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= 포커스웨이브 && _skillId <= 샌드스톰 && magicattr != 0)
			...
			return false;
		} else if (_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= 포커스웨이브 && _skillId <= 샌드스톰 && (magicattr == 1 || magicattr == 2))
				&& (_player.getElfAttr() == 21)) {
			return true;
		} else if (_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= 포커스웨이브 && _skillId <= 샌드스톰 && (magicattr == 2 || magicattr == 4))
				&& (_player.getElfAttr() == 24)) {
			return true;
		} else if (_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= 포커스웨이브 && _skillId <= 샌드스톰 && (magicattr == 2 || magicattr == 8))
				&& (_player.getElfAttr() == 28)) {
			return true;
		} else if (_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= 포커스웨이브 && _skillId <= 샌드스톰 && (magicattr == 1 || magicattr == 4))
				&& (_player.getElfAttr() == 41)) {
			return true;
		} else if (_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= 포커스웨이브 && _skillId <= 샌드스톰 && (magicattr == 4 || magicattr == 8))
				&& (_player.getElfAttr() == 48)) {
			return true;
		} else if (_player.글로리어스 && (_skill.getSkillLevel() >= 17 && _skill.getSkillLevel() <= 22 && _skillId >= 포커스웨이브 && _skillId <= 샌드스톰 && (magicattr == 1 || magicattr == 8))
				&& (_player.getElfAttr() == 81)) {
			return true;
		}
		return true;
	}
	...
	private boolean isHPMPConsume() {
		int orgMpConsume = _skill.getMpConsume();
		...
		if (_user instanceof L1NpcInstance) {
			...
			int pcint = _player.getAbility().getTotalInt();
			if (!(_skillId >= SHOCK_STUN && _skillId <= COUNTER_BARRIER) && !(_skillId >= 225 && _skillId <= 230) && !(_skillId == DEMOLITION) && !(_skillId == FORCE_STUN) && !(_skillId == HALPHAS) 
					&& !(_skillId >= JUDGEMENT && _skillId <= BLADE)) {

				// if(_skillId >= HEAL && _skillId <= ICE_SPIKE){
				/*
				 * if (_player.getBaseMagicDecreaseMp() > 0){ minusMP -=
				 * _player.getBaseMagicDecreaseMp(); }
				 */
				int maxConsume = _skill.getMpConsume();

				double balance = CalcStat.엠소모감소(_player.getAbility().getTotalInt()) * 0.01;
				// System.out.println(maxConsume);
	...
	private boolean isItemConsume() {
		...
		if (_player instanceof L1RobotInstance) return true;
	...
	private void addMagicList(L1Character cha, boolean repetition) {
		...
		if (_skillId == 데스힐 || _skillId == SHOCK_STUN || _skillId == 데스페라도 || _skillId == 파워그립 || _skillId == 엠파이어 || _skillId == DEMOLITION || _skillId == FORCE_STUN
				|| _skillId == ETERNITY || _skillId == PANTERA || _skillId == PHANTOM || _skillId == THUNDER_GRAB) {
		...
		if (_skillId == CURSE_PARALYZE || _skillId ==CURSE_PARALYZE2
			|| _skillId == MOB_CURSEPARALYZ_18 || _skillId == MOB_CURSEPARALYZ_19 || _skillId == 40013) {
	...
	private void sendIcon(L1PcInstance pc) {
		...
		case GLOWING_AURA:
			...
			break;
		case SHINING_AURA:
			pc.sendPackets(new S_SMPacketBox(S_SMPacketBox.RMSkillIconAura, 115, _getBuffIconDuration), true);
			break;
		case BRAVE_AURA:
		...
		case FIRE_SHIELD:
			...
			//pc.sendPackets(new S_NewSkillIcons(L1SkillId.FIRE_SHIELD, true, _getBuffIconDuration), true);
			break;
		case CLEAR_MIND:
		...
		case ADVANCE_SPIRIT:
			...
			break;
		case 기간틱:
			pc.sendPackets(new S_NewSkillIcons(L1SkillId.기간틱, true, 300));
			break;
...
//			break;
	...
	private void sendGrfx(boolean isSkillAction) {
		...
		if (_user instanceof L1PcInstance) {
			...
			if (_skillId == THUNDER_GRAB) {
				...
				if (!pc.썬더그랩브레이브) {
					if (_target instanceof L1PcInstance) {
						L1PcInstance target = (L1PcInstance) _target;
						/*
						 * target.sendPackets(new S_SkillSound(target.getId(), 6512));
						 */
			...
			/*
			 * case DESPERADO: // 공포 내성 if (_calcType == PC_PC || _calcType == NPC_PC) {
			 * probability -= _targetPc.getResistance
			 * 
			 * ().gethorror(); } break;
			 */
			...
			if (_skillId == SHOCK_STUN || _skillId == MOB_SHOCKSTUN_30 || _skillId == MOB_RANGESTUN_19
					|| _skillId == MOB_RANGESTUN_18 || _skillId == 엠파이어) {
			...
			if (_skill.getTarget().equals("attack") && _skillId != 18) {
				if (isPcSummonPet(_target)) {
					if (CharPosUtil.getZoneType(_player) == 1 || CharPosUtil.getZoneType(_target) == 1 || _player.skillismiss == true || _player.checkNonPvP(_player, _target)) {
					...
					if (_skillId == INVISIBILITY) {
						...
						if (_skillId == ARMOR_BREAK) {
							...
							if (_player.isInParty()) {
								for (L1PcInstance partymember : _player.getParty().getMembers()) {
									partymember.sendPackets(new S_SkillSound(targetid, castgfx), true);
								}
							} else {
						...
						_player.sendPackets(new S_SMPacketBox(S_SMPacketBox.RMSkillIconAura, 115, _getBuffIconDuration), true);
							...
							if (tempObj != null && tempObj instanceof L1PcInstance) {
								...
								if (tempPlayer.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_DRAGONPERL)) {
									int reminingtime = tempPlayer.getSkillEffectTimerSet()
											.getSkillEffectTimeSec(L1SkillId.STATUS_DRAGONPERL);
									tempPlayer.sendPackets(
											new S_PacketBox(S_PacketBox.DRAGONPERL, 8, (reminingtime / 4) - 2), true);
								}
								for (int skillNum = COOKING_BEGIN; skillNum <= COOKING_END; skillNum++) {
									if (tempPlayer.getSkillEffectTimerSet().hasSkillEffect(skillNum)) {
										...
										if (!ck) {
											...
											// System.out.println(skillNum+" > "+cookingType+" > "+time);
			...
			if (_skill.getTarget().equals("attack") && _skillId != 18) {
				if (_skill.getArea() == 0) {
					...
					if (castgfx == 0) {
						...
						/*
						 * if (_skillId == 10061) { ArrayList<L1PcInstance> pcs = new ArrayList<>(); for
						 * (L1PcInstance pc: L1World.getInstance().getVisiblePlayer(_user, 2)) {
						 * pcs.add(pc); }
						 * 
						 ...
						 * actionId, S_RangeSkill.TYPE_NODIR), true);
						 */
						// }else{
						Broadcaster.broadcastPacket(_user,
								new S_RangeSkill(_user, _target, cha, castgfx, actionId, S_RangeSkill.TYPE_DIR), true);
						// }
	...
	private static final int[][] repeatedSkills = {
			...
			{ FIRE_WEAPON, WIND_SHOT, STORM_EYE, BURNING_WEAPON, STORM_SHOT, 블로우어택 },
			{ SHIELD, /* SHADOW_ARMOR, FIRE_SHIELD,*/ /* EARTH_BLESS, */IRON_SKIN },
			{ HOLY_WALK, BLOOD_LUST, MOVING_ACCELERATION, 샌드스톰, 허리케인, STATUS_FRUIT, STATUS_BRAVE, STATUS_ELFBRAVE,
					FIRE_BLESS, 포커스웨이브 },
			{ HASTE, GREATER_HASTE, STATUS_HASTE }, { 프라이드, 기간틱, ADVANCE_SPIRIT },
			{ PHYSICAL_ENCHANT_DEX, DRESS_DEXTERITY }, { PHYSICAL_ENCHANT_STR, DRESS_MIGHTY },
			// { GLOWING_AURA, SHINING_AURA },
			{ /* SCALES_EARTH_DRAGON, SCALES_WATER_DRAGON, SCALES_FIRE_DRAGON */ },
			{ FEATHER_BUFF_A, FEATHER_BUFF_B, FEATHER_BUFF_C, FEATHER_BUFF_D }, /*{ IllUSION_OGRE, CUBE_IGNITION },*/
			/*{ IllUSION_DIAMONDGOLEM, CUBE_SHOCK },*/ /*{ CUBE_BALANCE, IllUSION_AVATAR },*/ /*{ IllUSION_LICH, CUBE_QUAKE },*/
...
//	public int getReuseDelayDcrease() {
//		int Dcrease = 0;
//		Dcrease += _skill.getReuseDelay();
//		if (Dcrease < 0) {
//			Dcrease = 0;
//		} else {
//			if(_player.isHaste()) {
//				double per = 30.000 / 100.000;
//				int decrease = (int) ((double) Dcrease * per);
//				Dcrease -= decrease; 
//			}else if(_player.isBloodLust() || _player.isSandstorm() || _player.isHurricane() || _player.isBrave()) {
//				double per = 30.000 / 100.000;
//				int decrease = (int) ((double) Dcrease * per);
//				Dcrease -= decrease;
//			}
...
//	}
	...
	private static final int[] BowGFX = { 138, 37, 3860, 3126, 3420, 2284, 3105, 3145, 3148, 3151, 3871, 4125, 2323,
			3892, 3895, 3898, 3901, 4917, 4918, 4919, 4950, 10870, 10869, 10871, 6140, 6145, 6150, 6155, 6269, 6272,
			6275, 6278, 6826, 6827, 6836, 6837, 6846, 6847, 6856, 6857, 6866, 6867, 6876, 6877, 6886, 6887, 6400, 5645,
			6399, 7039, 7040, 7041, 7140, 7144, 7148, 6160, 7152, 7156, 7160, 7164, 7139, 7143, 7147, 7151, 7155, 7159,
			7163, 7959, 7967, 7969, 7970, 9214, 8900, 8913, 9225, 9226, 8804, 8808, 8792, 8798, 8786, 8860, 8561, 8562,
			8719, 8112, 11498, 13140, 13388, 13380, 11122, 11126, 11331, 11342, 11352, 11353, 11362, 11363, 11369,
			11378, 11382, 11386, 11390, 11394, 11402, 11406, 11412, 11413, 11398, 10874, 7967, 7846, 7848, 8719, 13631,
			13635, 13723, 13725, 13346, 9615, 14927, 12314, 15848, 15814, 15528, 15830, 15831, 16002, 16074, 17535 };
	private static final int[] FouGFX = { 138, 37, 3860, 3126, 3420, 2284, 3105, 3145, 3148, 3151, 3871, 4125, 2323,
			...
			13380, 8792, 8798, 8786, 8860, 9362, 9363, 9364, 9365, 4769, 16008, 16056, 17541, 17545, 17549, 17515,
			17531 };
	...
	private void runSkill() {
		...
		if (_skillId == 엠파이어 || _skillId == SHOCK_STUN || _skillId == FORCE_STUN/*
														 * || _skillId == BONE_BREAK || _skillId == AM_BREAK || _skillId
														 * == SMASH
														 */
				&& _user instanceof L1PcInstance) {
			if (_target != _user)
			...
			for (Iterator<TargetStatus> iter = _targetList.iterator(); iter.hasNext();) {
				...
				if (_skill.getType() == L1Skills.TYPE_ATTACK && _user.getId() != cha.getId()) {
					...
					if (cha.getSkillEffectTimerSet().hasSkillEffect(10518)
							|| cha.getSkillEffectTimerSet().hasSkillEffect(데스힐) || cha.isPhantomDeathed()) {
				...
				/** 각성 스킬에 대한 정보 아우라키아(라는 스킬이 패시부가 잡혀있따면) */
				/*
				 * if (_skillId == L1SkillId.SCALES_EARTH_DRAGON || _skillId ==
				 * L1SkillId.SCALES_WATER_DRAGON || _skillId == L1SkillId.SCALES_FIRE_DRAGON ||
				 * _skillId == L1SkillId.SCALES_Lind_DRAGON) { L1PcInstance pc = (L1PcInstance)
				 * cha; pc.getWakeUpSkll(); }
				 */
				...
				if (cha.getSkillEffectTimerSet().hasSkillEffect(_skillId) && _skillId != 토마호크 && _skillId != BONE_BREAK
						...
						&& _skillId != 라이징 && _skillId != 엠파이어 && _skillId != 임팩트 && _skillId != FORCE_STUN && _skillId != DEMOLITION
						&& _skillId != ETERNITY && _skillId != PANTERA && _skillId != PHANTOM) {
				...
				case 그레이스아바타: {
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								if (paty.getSkillEffectTimerSet().hasSkillEffect(그레이스아바타)) {
									paty.getSkillEffectTimerSet().removeSkillEffect(그레이스아바타);
									paty.sendPackets(new S_NewSkillIcons(L1SkillId.그레이스아바타, false, 15));
								}
								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(그레이스아바타, _getBuffIconDuration * 1000);
								}
								int c = 5;// 기본확률
								if (paty.getLevel() > 80)
									c += (paty.getLevel() - 80);
								...
								paty.sendPackets(new S_NewSkillIcons(L1SkillId.그레이스아바타, true, 15));
								paty.sendPackets(new S_SkillSound(paty.getId(), 4434));
								paty.broadcastPacket(new S_SkillSound(paty.getId(), 4434));
							}
						}
						/** 파티원이 없다면 자기만 버프효과 */
					} else {
						...
						if (pc.getSkillEffectTimerSet().hasSkillEffect(그레이스아바타)) {
							pc.getSkillEffectTimerSet().removeSkillEffect(그레이스아바타);
							pc.sendPackets(new S_NewSkillIcons(L1SkillId.그레이스아바타, false, 15));
						}
						int c = 5;// 기본확률
						if (pc.getLevel() > 80)
							c += (pc.getLevel() - 80);
						...
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.그레이스아바타, true, 15));
				...
				case COUNTER_DETECTION: {
					...
					break;
				/*
				 * case JOY_OF_PAIN:{ int selldmg = _player.getMaxHp() - _player.getCurrentHp();
				 * dmg = selldmg/5; } break;
				 */
				case MIND_BREAK: {
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(CUBE_IGNITION,
											_getBuffIconDuration * 1000);
								}
								paty.addDmgup(4);
								paty.addHitup(4);
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(CUBE_SHOCK,
											_getBuffIconDuration * 1000);
								}
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(CUBE_QUAKE,
											_getBuffIconDuration * 1000);
								}
								paty.getAC().addAc(-8);
					...
					if (pc.isInParty()) {
						for (L1PcInstance paty : pc.getParty().getMembers()) {
							if (pc.getLocation().getTileLineDistance(paty.getLocation()) <= 14) {
								...
								/** 스킬사용자가 아니라면 버프 시간 추가 */
								if (pc != paty) {
									paty.getSkillEffectTimerSet().setSkillEffect(CUBE_BALANCE,
											_getBuffIconDuration * 1000);
								}
								paty.addDmgup(10);
...
//						healtime = CommonUtil.random(5, healtime);
						...
						pc.sendPackets(new S_NewSkillIcons(L1SkillId.데스힐, true, 8));
					}
				}
					break;
				case 디스트로이: {
				...
				case SHAPE_CHANGE:
					...
					if (cha instanceof L1PcInstance) {
						...
						/*
						 * int probability = 0; if(_targetNpc.getLevel()>=70){ probability = 0; } if
						 * (_calcType == PC_PC ){ if(_user.getId() == cha.getId()){ probability = 100; }
						 * }
						 * 
						 * if(probability > _random.nextInt(100)){
						 * 
						 * 
						 * L1PcInstance pc = (L1PcInstance) cha; L1PinkName.onAction(pc, _user);
						 * if(pc.getInventory().checkEquipped(20281)){ pc.sendPackets(new S_PolyHtml());
						 * pc.sendPackets(new S_ServerMessage(241, _user.getName())); }else{ int
						 * randomValue = 11; if(pc.getLevel() >= 15) randomValue += 9; if(pc.getLevel()
						 * >= 30) randomValue += 18; int polyId = 11328+_random.nextInt(randomValue);
						 * if(polyId >= 11358 && polyId <= 11361){ polyId = 11371+_random.nextInt(4);
						 * }else if(polyId >= 11362 && polyId <= 11365){ polyId =
						 * 11396+_random.nextInt(4); } L1PolyMorph.doPoly(pc, polyId, 7200,
						 * L1PolyMorph.MORPH_BY_ITEMMAGIC); if(_user.getId() == cha.getId()){ }else{
						 * pc.sendPackets(new S_ServerMessage(241, _user.getName())); } }
						 * 
						 * 
						 * }else{
				 ...
				 * _player.sendPackets(new S_SystemMessage("각성 업데이트로 당분간 사용할 수 없습니다."), true);
				 * 
				 * } break;
				 */
				case 38:
				...
				case THUNDER_GRAB: {
					...
					if (cha instanceof L1PcInstance) {
						...
						if (!pc1.썬더그랩브레이브) {
							...
							L1EffectSpawn.getInstance().spawnEffect(81182, _shockStunDuration, cha.getX(), cha.getY(),
									cha.getMapId());
				...
				case PRIME:{
					...
					for (L1PcInstance member : L1World.getInstance().getVisiblePlayer(pc, 13)) {
						if (pc.getClanid() != 0 && member.getClanid() == pc.getClanid()) {
								...
								member.sendPackets(new S_SkillSound(member.getId(), 18406));
								member.broadcastPacket(new S_SkillSound(member.getId(), 18406));
...
//							pc.sendPackets(new S_SPMR(pc), true);
//							pc.sendPackets(new S_NewSkillIcons(L1SkillId.PRIME, true, _getBuffIconDuration), true);
//							pc.sendPackets(new S_SkillSound(pc.getId(), 18406));
//							pc.broadcastPacket(new S_SkillSound(pc.getId(), 18406));
//							pc.sendPackets(new S_SystemMessage("프라임 : 근거리 대미지/명중 +3, 원거리 대미지/명중 +3, sp+2, 마법명중+2."));
//						}
					}
				}
				...
				case 엠파이어: {
					try {
						...
						L1EffectSpawn.getInstance().spawnEffect(81153, _shockStunDuration, cha.getX(), cha.getY(),
								cha.getMapId());
						if (cha instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) cha;
						...
						if (_target instanceof L1PcInstance) {
							...
							if(_player.phantom_D) {
								...
								if(ran > 50) {
									...
									L1EffectSpawn.getInstance().spawnEffect(81147, _shockStunDuration, _target.getX(), _target.getY(),
											_target.getMapId());
									...
									Broadcaster.broadcastPacket(_player, gfx, true);
									 _player.sendPackets(new S_SkillSound(_target.getId(), 18578), true);
									Broadcaster.broadcastPacket(_target, new S_SkillSound(_target.getId(), 18578), true);
									L1EffectSpawn.getInstance().spawnEffect(81148, _shockStunDuration, _target.getX(), _target.getY(),
											_target.getMapId());
								...
								Broadcaster.broadcastPacket(_player, gfx, true);
								 _player.sendPackets(new S_SkillSound(_target.getId(), 18578), true);
								Broadcaster.broadcastPacket(_target, new S_SkillSound(_target.getId(), 18578), true);
								L1EffectSpawn.getInstance().spawnEffect(81146, _shockStunDuration, _target.getX(), _target.getY(),
										_target.getMapId());
							...
							if(_player.phantom_D) {
								...
								if(ran > 70) {
									...
									Broadcaster.broadcastPacket(_player, gfx, true);
									 _player.sendPackets(new S_SkillSound(_target.getId(), 18580), true);
									Broadcaster.broadcastPacket(_target, new S_SkillSound(_target.getId(), 18580), true);
									L1EffectSpawn.getInstance().spawnEffect(81147, _shockStunDuration, npc.getX(), npc.getY(),
											npc.getMapId());
									...
									Broadcaster.broadcastPacket(_player, gfx, true);
									 _player.sendPackets(new S_SkillSound(_target.getId(), 18578), true);
									Broadcaster.broadcastPacket(_target, new S_SkillSound(_target.getId(), 18578), true);
									L1EffectSpawn.getInstance().spawnEffect(81148, _shockStunDuration, npc.getX(), npc.getY(),
											npc.getMapId());
								...
								Broadcaster.broadcastPacket(_player, gfx, true);
								 _player.sendPackets(new S_SkillSound(_target.getId(), 18578), true);
								Broadcaster.broadcastPacket(_target, new S_SkillSound(_target.getId(), 18578), true);
								L1EffectSpawn.getInstance().spawnEffect(81146, _shockStunDuration, npc.getX(), npc.getY(),
										npc.getMapId());
							}
						}
					} catch (Exception e) {
				...
				case ETERNITY: {
					try {
						...
						L1EffectSpawn.getInstance().spawnEffect(81149, _shockStunDuration, cha.getX(), cha.getY(),
								cha.getMapId());
						if (cha instanceof L1PcInstance) {
							L1PcInstance pc = (L1PcInstance) cha;
					...
					try {
...

						{
							if (diffLevel <= -10) {
								if(ran > 30) {
									_shockStunDuration = forcestunTimeArray1[random.nextInt(forcestunTimeArray1.length)];
									L1EffectSpawn.getInstance().spawnEffect(81151, _shockStunDuration, cha.getX(), cha.getY(),
											cha.getMapId());
								} else {
									_shockStunDuration = forcestunTimeArray2[random.nextInt(forcestunTimeArray2.length)];
									L1EffectSpawn.getInstance().spawnEffect(81152, _shockStunDuration, cha.getX(), cha.getY(),
											cha.getMapId());
								}
							} else if (diffLevel <= -8) {
								if(ran > 30) {
				...
				case 40003: // 불멸의리치, 냉혹의 아이리스 넉백스킬
					...
					for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(_user)) {
						...
						/*
						 * int heading = CharPosUtil.targetDirection(_user, pc.getX(), pc.getY());
						 * heading = checkObject(pc.getX(), pc.getY(), pc.getMapId(), heading);
						 * 
						 * if(heading == -1)continue; int tempx = HEADING_TABLE_X[heading]; int tempy =
						 * HEADING_TABLE_Y[heading];
						 * 
						 * if(Math.max(Math.abs(_user.getX() - (pc.getX() + tempx)),
						 * Math.abs(_user.getY() - (pc.getY() + tempy))) <= 1) continue; pc.dx=
						 * pc.getX() + tempx; pc.dy= pc.getY() + tempy; pc.dm=
						 * (short)pc.getLocation().getMapId(); pc.dh=pc.getMoveState().getHeading();
				...
				case 40029: // 옛늑인 끌어오기
					for (L1PcInstance pc : L1World.getInstance().getVisiblePlayer(_user)) {
						...
						short mapId = (short) newLocation.getMapId();
						/*
						 * int newX = _user.getX() + random.nextInt(2)+1; int newY = _user.getY() +
						 * random.nextInt(2)+1; short mapId = (short) _user.getMapId();
						 */
						if (pc instanceof L1RobotInstance) {
							L1RobotInstance rob = (L1RobotInstance) pc;
							L1Teleport.로봇텔(rob, newX, newY, (short) mapId, true);
				...
				case BONE_BREAK: {
					...
					L1EffectSpawn.getInstance().spawnEffect(4500500, _shockStunDuration, cha.getX(), cha.getY(),
							cha.getMapId());
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
				...
				case AM_BREAK: {
					/*
					 * if (cha instanceof L1PcInstance) { L1PcInstance pc = (L1PcInstance) cha;
					 * pc.addDmgup(-2); }
					 */
				...
				case CANCELLATION: {
					...
					for (int skillNum = SKILLS_BEGIN; skillNum <= SKILLS_END; skillNum++) {
						...
						if (skillNum == SHAPE_CHANGE) { //
							if (cha instanceof L1PcInstance) {
								L1PcInstance pc = (L1PcInstance) cha;
								if (pc.getGfxId().getTempCharGfx() >= 13715 && pc.getGfxId().getTempCharGfx() <= 13745 || pc.getGfxId().getTempCharGfx() == 15115) {
									continue;
				...
				case MANA_DRAIN: {
					if (!cha.isManaDrain) {
						...
						/*
						 * if(cha.getResistance().getMr()<=50){ drainMana -= drainMana*0.1; }else
						 * if(cha.getResistance().getMr()<=100){ drainMana -= drainMana*0.2; }else{
						 * drainMana -= drainMana*0.3; }
						 */
				...
				if (_calcType == PC_PC || _calcType == NPC_PC) { // 여기부터
					...
					case TELEPORT_TO_MOTHER: {
						...
						break;
					/*
					 * case CALL_CLAN: { L1PcInstance pc = (L1PcInstance) cha; if (pc.getMapId() ==
					 * 4 && ((pc.getX() >= 33332 && pc.getX() <= 33338 && pc.getY() >= 32433 &&
					 * pc.getY() <= 32439) || (pc.getX() >= 33259 && pc.getX() <= 33265 && pc.getY()
					 * >= 32399 && pc .getY() <= 32405) || (pc.getX() >= 33389 && pc.getX() <= 33395
					 * && pc.getY() >= 32341 && pc.getY() <= 32347))) { pc.sendPackets(new
					 * S_ServerMessage(647), true); return; }
					 * 
					 * if (pc.getMapId() == 4 && ((pc.getX() >= 33328 && pc.getX() <= 33344 &&
					 * pc.getY() >= 32427 && pc.getY() <= 32444) || (pc.getX() >= 33255 && pc.getX()
					 * <= 33272 && pc.getY() >= 32393 && pc .getY() <= 32412) ||
					 * 
					 * (pc.getX() >= 34194 && pc.getX() <= 34305 && pc.getY() >= 33324 && pc .getY()
					 * <= 33535) || // 황혼의산맥 (pc.getX() >= 33450 && pc.getX() <= 33470 && pc.getY()
					 * >= 32328 && pc .getY() <= 32344) || // 아덴의한국민
					 * 
					 * (pc.getX() >= 33385 && pc.getX() <= 33400 && pc.getY() >= 32336 && pc .getY()
					 * <= 32353) || (pc.getX() >= 33461 && pc.getX() <= 33534 && pc.getY() >= 33165
					 * && pc.getY() <= 33253))) { pc.sendPackets(new S_ServerMessage(647), true); }
					 * L1PcInstance clanPc = (L1PcInstance) L1World
					 * .getInstance().findObject(_targetID); if (clanPc != null) {
					 * clanPc.setTempID(pc.getId()); clanPc.sendPackets(new S_Message_YN(729, ""),
					 * true); } } break;
					 */
					/*
					 * case RUN_CLAN: { L1PcInstance pc = (L1PcInstance) cha; L1PcInstance clanPc =
					 * (L1PcInstance) L1World .getInstance().findObject(_targetID); if (clanPc !=
					 * null) { if (pc.getMap().isEscapable() || pc.isGm()) { if (pc.getMapId() == 4
					 * && ((pc.getX() >= 33332 && pc.getX() <= 33338 && pc.getY() >= 32433 && pc
					 * .getY() <= 32439) || (clanPc.getX() >= 33259 && clanPc.getX() <= 33265 &&
					 * clanPc.getY() >= 32399 && clanPc .getY() <= 32405) || (clanPc .getX() >=
					 * 33389 && clanPc.getX() <= 33395 && clanPc.getY() >= 32341 && clanPc .getY()
					 * <= 32347))) { pc.sendPackets(new S_ServerMessage(547), true); return; }
					 * boolean castle_area = L1CastleLocation .checkInAllWarArea(clanPc.getX(),
					 * clanPc.getY(), clanPc.getMapId()); if ((clanPc.getMapId() == 0 ||
					 * clanPc.getMapId() == 4 || clanPc .getMapId() == 304) && castle_area == false)
					 * { L1Teleport.teleport(pc, clanPc.getX(), clanPc.getY(), clanPc.getMapId(), 5,
					 * true, true); } else { pc.sendPackets(new S_ServerMessage(547), true); } }
					 * else { pc.sendPackets( new S_Paralysis( S_Paralysis.TYPE_TELEPORT_UNLOCK,
					 * false), true); pc.sendPackets(new S_ServerMessage(647), true); } } } break;
					 */
...
//						break;
					...
					case SHINING_AURA:{
						...
						pc.getAC().addAc(-8);
						pc.sendPackets(new S_SMPacketBox(S_SMPacketBox.RMSkillIconAura, 115, _getBuffIconDuration), true);
						pc.sendPackets(new S_SystemMessage("샤이닝 실드: ac-8."));
					}
						break;
					case BRAVE_AURA: {
						L1PcInstance pc = (L1PcInstance) cha;
					...
					case CLEAR_MIND: {
						...
						pc.sendPackets(new S_SystemMessage("클리어마인드: STR+1,DEX+1,INT+1 ."));
						/*
						 * pc.resetBaseMr(); pc.sendPackets(new S_SPMR(pc), true);
						 */
					}
						break;
					/*
					 * case RESIST_ELEMENTAL: { L1PcInstance pc = (L1PcInstance) cha;
					 * pc.getResistance().addAllNaturalResistance(10); pc.sendPackets(new
					 * S_OwnCharAttrDef(pc), true); } break;
					 */
					...
					case ELEMENTAL_PROTECTION: {
						...
						break;
					/*
					 * case INVISIBILITY: case BLIND_HIDING:{ L1PcInstance pc = (L1PcInstance) cha;
					 * 
					 * for (L1DollInstance doll : pc.getDollList()) { doll.deleteDoll();
					 * pc.sendPackets(new S_SkillIconGFX(56, 0), true); pc.sendPackets(new
					 * S_OwnCharStatus(pc), true); }
					 * 
					 * 
					 * pc.sendPackets(new S_Invis(pc.getId(), 1), true);
					 * Broadcaster.broadcastPacket(pc, new S_Invis(pc.getId(), 1), true);
					 * Broadcaster.broadcastPacket(pc, new S_RemoveObject(pc.getId()), true); }
					 * break;
					 */
					...
					case INVISIBILITY: {
						 ...
						 * if(pc.getParty().isMember(tar)){ tar.sendPackets(new S_OtherCharPacks(pc,
						 * tar, true)); } } }
						 */
					...
					case SCALES_EARTH_DRAGON: {
						...
						/*
						 * L1PcInstance pc = (L1PcInstance) cha; if(pc.getMapId() >= 9000 &&
						 * pc.getMapId() <= 9099){ pc.sendPackets(new S_ServerMessage(1170), true); //
						 * 이곳에서 변신할수 없습니다. return; } pc.addMaxHp(35); pc.getAC().addAc(-8);
						 * pc.sendPackets(new S_OwnCharAttrDef(pc), true); pc.sendPackets(new
						 * S_HPUpdate(pc.getCurrentHp(), pc.getMaxHp()), true); if (pc.isInParty()) {
						 * pc.getParty().updateMiniHP(pc); } pc.sendPackets(new
						 * S_MPUpdate(pc.getCurrentMp(), pc.getMaxMp()), true);
						 * pc.getGfxId().setTempCharGfx(9362); pc.sendPackets(new
						 * S_ChangeShape(pc.getId(), 9362), true); if (!pc.isGmInvis() &&
						 * !pc.isInvisble()) { Broadcaster.broadcastPacket(pc, new
						 * S_ChangeShape(pc.getId(), 9362), true); } pc.startMpDecreaseByScales();
						 * L1ItemInstance item = pc.getWeapon(); if(item != null){
						 * pc.getInventory().setEquipped(item, false);
						 * pc.getInventory().setEquipped(item, true); } pc.sendPackets(new
						 * S_SystemMessage("안타라스 각성 : 양손검 공격속도 특화"), true); pc.sendPackets(new
						 * S_SystemMessage("각성 업데이트로 당분간 각성마법(브레스등등)은 사용할 수 없습니다." ), true);
						 */
					}
						break;
					case SCALES_WATER_DRAGON: {
						...
						/*
						 * if(pc.getMapId() >= 9000 && pc.getMapId() <= 9099){ pc.sendPackets(new
						 * S_ServerMessage(1170), true); // 이곳에서 변신할수 없습니다. return; }
						 * pc.getResistance().addMr(15); pc.getResistance().addAllNaturalResistance(15);
						 * pc.sendPackets(new S_SPMR(pc), true); pc.sendPackets(new
						 * S_OwnCharAttrDef(pc), true); pc.getGfxId().setTempCharGfx(9365);
						 * pc.sendPackets(new S_ChangeShape(pc.getId(), 9365), true); if
						 * (!pc.isGmInvis() && !pc.isInvisble()) { Broadcaster.broadcastPacket(pc, new
						 * S_ChangeShape(pc.getId(), 9365), true); } L1ItemInstance item =
						 * pc.getWeapon(); if(item != null){ pc.getInventory().setEquipped(item, false);
						 * pc.getInventory().setEquipped(item, true); } pc.startMpDecreaseByScales();
						 * pc.sendPackets(new S_SystemMessage("파푸리온 각성 : 이동속도 특화"), true);
						 * pc.sendPackets(new S_SystemMessage("각성 업데이트로 당분간 각성마법(브레스등등)은 사용할 수 없습니다." ),
						 * true);
						 */
					}
						break;
					case SCALES_FIRE_DRAGON: {
						...
						/*
						 * if(pc.getMapId() >= 9000 && pc.getMapId() <= 9099){ pc.sendPackets(new
						 * S_ServerMessage(1170)); // 이곳에서 변신할수 없습니다. return; }
						 * pc.getAbility().addAddedStr((byte) 3); pc.getAbility().addAddedDex((byte) 3);
						 * pc.getAbility().addAddedCon((byte) 3); pc.getAbility().addAddedInt((byte) 3);
						 * pc.getAbility().addAddedWis((byte) 3); pc.getGfxId().setTempCharGfx(9363);
						 * pc.sendPackets(new S_ChangeShape(pc.getId(), 9363), true); if
						 * (!pc.isGmInvis() && !pc.isInvisble()) { Broadcaster.broadcastPacket(pc, new
						 * S_ChangeShape(pc.getId(), 9363), true); } L1ItemInstance item =
						 * pc.getWeapon(); if(item != null){ pc.getInventory().setEquipped(item, false);
						 * pc.getInventory().setEquipped(item, true); } pc.startMpDecreaseByScales();
						 * pc.sendPackets(new S_SystemMessage("발라카스 각성 : 도끼 공격속도 특화"), true);
						 * pc.sendPackets(new S_SystemMessage("각성 업데이트로 당분간 각성마법(브레스등등)은 사용할 수 없습니다." ),
						 * true);
						 */
					}
					...
					case VALA_MAAN: {
						...
						// pc.addDamageReductionByArmor(3);
						pc.addTechniqueTolerance(15); // 스턴내성3
					}
						break;
					case BIRTH_MAAN: {
						L1PcInstance pc = (L1PcInstance) cha;
				...
				/*
				 * if(cha instanceof L1PcInstance){ L1PcInstance pc = (L1PcInstance) cha; if
				 * (_skill.getType() == L1Skills.TYPE_HEAL){
				 * if(pc.getSkillEffectTimerSet().hasSkillEffect(10518)){ if(dmg > 0){ dmg *=
				 * -1; } } } }
				 */
	...
	private boolean isNotCancelable(int skillNum) {
		return skillNum == ENCHANT_WEAPON || skillNum == BLESSED_ARMOR || skillNum == ABSOLUTE_BARRIER
				|| skillNum == ADVANCE_SPIRIT || skillNum == 뫼비우스 || skillNum == SHOCK_STUN  || skillNum == PANTERA || skillNum == 기간틱
				|| skillNum == FORCE_STUN || skillNum == DEMOLITION || skillNum == ETERNITY || skillNum == PRIME
				|| skillNum == HALPHAS || skillNum == POTENTIAL || skillNum == PHANTOM
				|| skillNum == 파워그립 || skillNum == 데스페라도 || skillNum == 엠파이어 || skillNum == SCALES_Lind_DRAGON
				|| skillNum == STATUS_UNDERWATER_BREATH || skillNum == 인페르노 || skillNum == SHADOW_FANG
				|| skillNum == REDUCTION_ARMOR || skillNum == SOLID_CARRIAGE || skillNum == COUNTER_BARRIER
				|| skillNum == 인페르노 || skillNum == SHADOW_ARMOR || skillNum == ARMOR_BREAK || skillNum == DRESS_EVASION
				...
				|| skillNum == STATUS_DRAGONPERL || skillNum == 프라이드 || skillNum == 블로우어택 || skillNum == CUBE_IGNITION
				|| skillNum == CUBE_QUAKE || skillNum == CUBE_SHOCK || skillNum == CUBE_BALANCE;
	...
	private boolean isTargetFailure(L1Character cha) {
		...
		if ((_skillId == TURN_UNDEAD && (undeadType == 0 || undeadType == 2))
				...
				|| ((_skillId == ERASE_MAGIC || _skillId == SLOW || _skillId == MOB_SLOW_1 || _skillId == MOB_SLOW_18
						|| _skillId == MANA_DRAIN || _skillId == WIND_SHACKLE) && isErase == false)
	...
	private static final int[] stunTimehorror = { 1500, 1500,// 4
			2000, 2000, 2000, 2000,
			2500, 2500, 2500, 2500,// 2
	};
	private static final int[] stunTimehorror2 = { 1000, 1000, 1000, 1000, 1000, 1500, 1500, 1500, // 5
			1500, 1500, 1500, 1500, // 4
			2000, 2000, 2000, 2000,// 2
	};
	private static final int[] stunTimethunder = { 3000, 3000, 3000, 3000, 4000, 4000, 4000, 4000, 5000, 5000, 6000,
			6000// 2
	};
	...
	private static final int[] forcestunTimeArray1 = { 3500, 4000, 4500, 5000, 5000, 5000 };
	
	private static final int[] forcestunTimeArray2 = { 5500, 6000, 6500, 7000 };
