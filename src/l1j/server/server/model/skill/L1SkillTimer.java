	...
	public static void stopSkill(L1Character cha, int skillId) {
		...
		/*		case 앱솔루트블레이드: {
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_CreateItem(앱솔루트블레이드, 1));
			}
		}
		break;*/
		case 디스트로이: {
		...
		case L1SkillId.지식빙수:
			...
			if (cha instanceof L1PcInstance) {
				...
				pc.sendPackets(new S_OwnCharStatus2(pc), true);
				pc.sendPackets(
						new S_MPUpdate(pc.getCurrentMp(), pc.getMaxMp()), true);
			}
			break;
		case 메티스축복주문서:
			if (cha instanceof L1PcInstance) {
				...
				pc.addMaxMp(-30);
				pc.sendPackets(
						new S_HPUpdate(pc.getCurrentHp(), pc.getMaxHp()), true);
				pc.sendPackets(
						new S_MPUpdate(pc.getCurrentMp(), pc.getMaxMp()), true);
			}
			break;
		...
		case L1SkillId.파워그립:
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_Paralysis(S_Paralysis.TYPE_POWER_GRIP,
						false), true);
			}
			break;
		case 40037:
		...
		case 데스페라도:
			if (cha instanceof L1PcInstance) {
				...
				pc.sendPackets(new S_Paralysis(S_Paralysis.TYPE_DEATH_PERADO,
						false), true);
			} else if (cha instanceof L1MonsterInstance
					|| cha instanceof L1SummonInstance
					|| cha instanceof L1PetInstance) {
		...
		case STATUS_PINK_NAME:
			if (cha instanceof L1PcInstance) {
				...
				Broadcaster.broadcastPacket(npc,
						new S_PinkName(npc.getId(), 0), true);
...
class L1SkillTimerTimerImpl implements L1SkillTimer, Runnable {
	...
	public void run() {
		try {
			if (!on)
				return;
			if (_skillId == L1SkillId.EXP_POTION_cash
					|| _skillId == L1SkillId.EXP_POTION) {
			...
			if (_remainingTime <= 0) _cha.getSkillEffectTimerSet().removeSkillEffect(_skillId);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
