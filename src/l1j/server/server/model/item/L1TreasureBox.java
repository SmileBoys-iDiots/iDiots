...
public class L1TreasureBox {
		...
		if (getType().equals(TYPE.SPECIFIC)) {
			...
			if (!ck) {
				...
				for (Item each : getItems()) {
					...
					if (r < chance) {
							item = ItemTable.getInstance().createItem(each.getItemId());
							if (item != null && !open(pc)) {
								item.setCount(each.getCount());
								storeItem(pc, item);
								//메티스상자
								if((item.getItemId() == 1430509) || each.getItemId() == 600247){
						...
						if (item != null) {
							...
							if ((item.getItemId() >= 450008 && item.getItemId() <= 450013)
								...
								if (item.getItemId() == 266) {//
									int[] at = { 2, 5, 8, 11 };
									random = new Random(System.nanoTime());
									item.setAttrEnchantLevel(at[random
											.nextInt(4)]);
							...
							storeItem(pc, item);
							if (item.getItemId() == 40308) {
								L1Item tempItem = ItemTable.getInstance()
										.getTemplate(getBoxId());
								aden_store_databaseInsert(pc, tempItem);
							}
	...
	public int getDragonKeyCheck() {
		...
		for (L1FieldObjectInstance npc : L1World.getInstance().getAllField()) {
			if (npc.getNpcId() == 100011 || npc.getNpcId() == 4212015
	...
	private void aden_store_databaseInsert(L1PcInstance pc, L1Item box) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pst = con
					.prepareStatement("SELECT * FROM log_treasurebox_aden WHERE char_name = ? AND box_id = ?");
			...
			if (rs.next()) {
				...
				try {
					con2 = L1DatabaseFactory.getInstance().getConnection();
					pstm2 = con
							.prepareStatement("INSERT INTO log_treasurebox_aden SET char_name=?, box_id =?, box_name=?, count=?");
