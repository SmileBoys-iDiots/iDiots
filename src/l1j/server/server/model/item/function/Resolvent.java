...
public class Resolvent extends L1ItemInstance {
	...
	private void useResolvent(L1PcInstance pc, L1ItemInstance item,
		...
		/*
		 * if (rnd >= 1 && rnd <= 20) { crystalCount = 0; pc.sendPackets(new
		 * S_ServerMessage(158, item.getName()), true); // \f1%0이 증발하고 있지 않게
		 * 되었습니다. } else if (rnd >= 21 && rnd <= 95) { crystalCount *= 1; } else
		 * if (rnd >= 96 && rnd <= 100) { //crystalCount *= 1.5;
		...
		if (GMCommands.용해로그) {
			if (Config.접속채팅모니터() > 0) {
				for (L1PcInstance gm : Config.toArray접속채팅모니터()) {
					...
					gm.sendPackets(
							new S_SystemMessage("\\fY용해> " + pc.getName()
									+ " Item:" + item.getNumberedName(1)
									+ " 결정체:" + crystalCount), true);
				}
			}
		}
