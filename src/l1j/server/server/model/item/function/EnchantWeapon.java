	...
	public void clickItem(L1Character cha, ClientBasePacket packet) {
		if (cha instanceof L1PcInstance) {
			...
			if (weaponId == 7 || weaponId == 35 || weaponId == 48
					...
					|| weaponId == 175 || weaponId == 224 || weaponId == 7232 || weaponId == 1013) {
			...
			if (itemId == L1ItemId.여행무기주문서 || itemId == 60142) {
				if (weaponId == 7 || weaponId == 35 || weaponId == 48
						...
						|| weaponId == 7232 || weaponId == 224 || weaponId == 1013) {
					int enchant_level = l1iteminstance1.getEnchantLevel();
					if (enchant_level >= 6) {
						pc.sendPackets(new S_SystemMessage("아무일도 일어나지 않았습니다."),
								true);
		      ...
		      //아놀드인첸//
						if (weaponId == 90093 || weaponId == 90093 || weaponId == 90093
								|| weaponId == 90094 || weaponId == 90095 || weaponId == 90096
								|| weaponId == 90097 || weaponId == 90098 || weaponId == 90099
								|| weaponId == 90100) {
			...
			/*if (enchant_level >= Config.MAX_WEAPON1)
				if (weaponId >= 450008 && weaponId <= 450013) {
					pc.sendPackets(new S_SystemMessage("\\fW마족 무기는 현재 +"
							+ Config.MAX_WEAPON1 + "이상은 인챈할수 없습니다."), true);
					return;
				}*/
			...
			if (itemId == 60510) { // 장인의 무기 마법 주문서
				...
				if (enchant_level >= Config.MAX_WEAPON) {
					pc.sendPackets(new S_SystemMessage("무기는 9까지만 현재 인첸트 됩니다."),
							true);
					return;
				}
				if (safe_enchant == 0 || enchant_level != 9) {
					pc.sendPackets(new S_SystemMessage("이 아이템에 사용할 수 없습니다."),
							true);
					return;
				}
				if (enchant_level == 9) {
					...
					if (_random.nextInt(100) < Config.장인확률 + chance) {
						...
						pc.sendPackets(
								new S_ServerMessage(160, l1iteminstance1
										.getLogName(), "$245", "$248"), true);
					pc.getInventory().removeItem(useItem, 1);
				}
				...
				if (safe_enchant == 0){
					if(enchant_level >= Config.MAX_WEAPON1){
						...
						|| itemId == L1ItemId.WATER_ENCHANT_WEAPON_SCROLL
								&& itemId == L1ItemId.FIRE_ENCHANT_WEAPON_SCROLL))
						pc.sendPackets(new S_SystemMessage("\\fW안전인챈 0 무기는 현재 +"
								+ Config.MAX_WEAPON1 + "이상은 인챈할수 없습니다."), true);
						return;
					}
				} 
				...
				int enchant_chance_wepon1 = 0;

				if (safe_enchant == 0) {
				} else {
					if (itemId == 160510) {
						if (enchant_level <= 6)
							enchant_chance_wepon1 = 30;
						else if (enchant_level == 7)
							enchant_chance_wepon1 = 30;
						else if (enchant_level == 8)
							enchant_chance_wepon1 = 20;
						else if (enchant_level == 9)
							enchant_chance_wepon1 = 7;
						else if (enchant_level == 10)
							enchant_chance_wepon1 = 2;
						else if (enchant_level >= 11)
							enchant_chance_wepon1 = 1;
					} else {
						if (enchant_level <= 6)
							enchant_chance_wepon1 = 30;
						else if (enchant_level == 7)
							...
							enchant_chance_wepon1 = 20;
						else if (enchant_level == 9 || enchant_level == 10)
							enchant_chance_wepon1 = 2;
						else if (enchant_level >= 11)
							enchant_chance_wepon1 = 1;
					}
				}
				enchant_chance_wepon1 *= 100;
				enchant_chance_wepon1 *= (Config.ENCHANT_CHANCE_WEAPON == 0 ? 1
						: Config.ENCHANT_CHANCE_WEAPON);
				// 100%인첸주문서 //
				if (rnd < enchant_chance_wepon1) {
