...
public class Enchant extends L1ItemInstance {
	...
	public void SuccessEnchant(L1PcInstance pc, L1ItemInstance item, int i) {
			...
			if (item.getItem().getType() >= 8 && item.getItem().getType() <= 12) {
				if (newEnchantLvl >= 8) {
					L1World.getInstance().broadcastPacketToAll(
							new S_ServerMessage(4445, item.getLogName()));
				}
		...
		if (oldEnchantLvl >= 10
				&& newEnchantLvl < 10
				&& (attr_level == 4 || attr_level == 9 || attr_level == 14 || attr_level == 19)) {
			if (attr_level == 4)
			...
			pc.getInventory()
					.updateItem(item, L1PcInventory.COL_ATTRENCHANTLVL);
		...
		} else if (oldEnchantLvl >= 11
				&& newEnchantLvl < 11
				&& (attr_level == 5 || attr_level == 10 || attr_level == 15 || attr_level == 20)) {
			if (attr_level == 5)
		...
		if (newEnchantLvl > safe_enchant) {
			if (Config.인첸채팅모니터() > 0) {
				for (L1PcInstance gm : Config.toArray인첸채팅모니터()) {
					...
					gm.sendPackets(new S_SystemMessage("\\fV[" + pc.getName()
							+ "] " + item.getName() + " : " + oldEnchantLvl
							+ "->" + newEnchantLvl + " [인첸 성공]"), true);
				}
			}
			eva.LogEnchantAppend("성공:W", pc.getName(), oldEnchantLvl + "->"
					+ newEnchantLvl, item.getName(), item.getId());
		}
		if (item.getItem().getType2() == 2) {
			if (newEnchantLvl > safe_enchant) {
				eva.LogEnchantAppend("성공:A", pc.getName(), oldEnchantLvl + "->"
						+ newEnchantLvl, item.getName(), item.getId());
			}
		}
		...
		if (item.getItem().getType2() == 2) {
			if (item.isEquipped()) {
				...
				if (i2 == 500214 || i2 == 20011 || i2 == 20110 || i2 == 120011
						|| i2 == 9091 || i2 == 20117 || i2 == 420108
						|| i2 == 420109 || i2 == 420110 || i2 == 420111
						|| i2 == 425108 || i2 == 490008 || i2 == 490017
						|| i2 == 120194 || i2 == 1020110
						|| (i2 >= 21169 && i2 <= 21172)) { // 매직
					// 헤룸, 매직
					// 체인 메일
					pc.getResistance().addMr(i);
					pc.sendPackets(new S_SPMR(pc), true);
				}
				if (i2 == 20056 || i2 == 120056 || i2 == 9092 || i2 == 220056
						|| i2 == 425108 || i2 == 9084 || i2 == 20074
						|| i2 == 120074) { // 매직 클로크
	...
	public void FailureEnchant(L1PcInstance pc, L1ItemInstance item,
		...
		if (scrollid == L1ItemId.ORIM_ACCESSORY_ENCHANT_SCROLL) {
			...
			msg = (new StringBuilder()).append(pm1 + enchant_level).append(" ")
			...
			if (Config.인첸채팅모니터() > 0) {
				for (L1PcInstance gm : Config.toArray인첸채팅모니터()) {
					...
					gm.sendPackets(new S_SystemMessage("\\fV[" + pc.getName()
							+ "] +" + item.getEnchantLevel() + " "
							+ item.getName() + " [유물인첸 실패]"));
				}
			}
		}
		
		
		else if ((itemId >= 450022 && itemId <= 450025)
				|| (itemId >= 427110 && itemId <= 427112)) {
			...
			if (Config.인첸채팅모니터() > 0) {
				for (L1PcInstance gm : Config.toArray인첸채팅모니터()) {
					...
					gm.sendPackets(
							new S_SystemMessage("\\fV[" + pc.getName() + "] +"
									+ item.getEnchantLevel() + " "
									+ item.getName() + " [인첸 실패]"), true);
				}
			}
			if (itemType == 1) {
				eva.LogEnchantAppend("실패:W", pc.getName(),
						Integer.toString(item.getEnchantLevel()),
						item.getName(), item.getId());
			} else if (itemType == 2) {
				eva.LogEnchantAppend("실패:A", pc.getName(),
						Integer.toString(item.getEnchantLevel()),
						item.getName(), item.getId());
			}
		} 
			...
			if ((item.getItemId() >= 263 && item.getItemId() <= 265)
					|| item.getItemId() == 256 || item.getItemId() == 4500027
					|| item.getItemId() == 4500026)
				pc.sendPackets(new S_SystemMessage(
						"강렬하게 빛났지만 할로윈의 보호가 깃들어있어 증발 되지는 않았습니다."), true);
			else
				pc.sendPackets(new S_SystemMessage(
						"강렬하게 빛났지만 마족의 보호가 깃들어있어 증발 되지는 않았습니다."), true);
			...
			if (pc.getSecondWeapon() != null) {
				if (pc.getSecondWeapon().getId() == item.getId()) {
					pc.getInventory().setEquipped(pc.getSecondWeapon(), false,
							false, false, true);
				}
			}
			...
			case 0:
				if (chance <= 5) {
					pc.sendPackets(new S_SystemMessage("" + item.getLogName()
							+ "에 속성 저항력이 스며들었습니다."), true);
					item.setRegistLevel(1);
				} else {
					pc.sendPackets(new S_SystemMessage("" + item.getLogName()
							+ "에 속성 저항력이 스며들지 못하였습니다."), true);
				}
				break;
			case 1:
				if (chance <= 4) {
	...
	public void AttrEnchant(L1PcInstance pc, L1ItemInstance item, int item_id) { // 속성4단5단
		...
		if ((item.getEnchantLevel() < 9)
				&& (attr_level == 3 || attr_level == 8 || attr_level == 13 || attr_level == 18) && !(item.getItemId() == 61 || item.getItemId() == 12
						|| item.getItemId() == 86 || item.getItemId() == 134 || item.getItemId() == 30082 || item.getItemId() == 30080 || item.getItemId() == 30081
						|| item.getItemId() == 30083 || item.getItemId() == 450044 || item.getItemId() == 450045 || item.getItemId() == 450046 || item.getItemId() == 450047
						|| item.getItemId() == 450048 || item.getItemId() == 450049 || item.getItemId() == 450050 || item.getItemId() == 450051
						|| item.getItemId() == 30110 || item.getItemId() == 30111 || item.getItemId() == 30112)) {
			pc.sendPackets(new S_SystemMessage("아무일도 일어나지 않았습니다."), true); 
			return;
		} else if (item.getEnchantLevel() < 10
				&& (attr_level == 4 || attr_level == 9 || attr_level == 14 || attr_level == 19) && !(item.getItemId() == 61 || item.getItemId() == 12
						|| item.getItemId() == 86 || item.getItemId() == 134 || item.getItemId() == 30082 || item.getItemId() == 30080 || item.getItemId() == 30081
						|| item.getItemId() == 30083 || item.getItemId() == 450044 || item.getItemId() == 450045 || item.getItemId() == 450046 || item.getItemId() == 450047
						|| item.getItemId() == 450048 || item.getItemId() == 450049 || item.getItemId() == 450050 || item.getItemId() == 450051
						|| item.getItemId() == 30110 || item.getItemId() == 30111 || item.getItemId() == 30112)) {
		...
		if (item_id == L1ItemId.FIRE_ENCHANT_WEAPON_SCROLL || item_id == 60722) { // 불의 무기 강화 주문서
			if (attr_level == 0 || (attr_level >= 6)) {
				if (attr_level == 9 && attr_level == 10 && attr_level == 14 && attr_level == 15
						&& attr_level == 19 && attr_level == 20)
					pc.sendPackets(
							new S_ServerMessage(3297, item.getLogName()), true);
				else if (chance < 300) {
					pc.sendPackets(
							new S_ServerMessage(1410, item.getLogName()), true);
					item.setAttrEnchantLevel(1);
				} else {
					pc.sendPackets(
							new S_ServerMessage(1411, item.getLogName()), true);
				}
				...
				if (chance < 30) {
					...
					pc.sendPackets(
							new S_ServerMessage(3296, item.getLogName()), true);
				...
				return;
																		...
																		// 강화
																		// 주문서
			if ((attr_level >= 0 && attr_level <= 5)
					|| (attr_level >= 11)) {
				if (attr_level == 4 && attr_level == 5 && attr_level == 14 && attr_level == 15
						&& attr_level == 19 && attr_level == 20)
					pc.sendPackets(
							new S_ServerMessage(3297, item.getLogName()), true);
				else if (chance < 300) {
				...
				return;
																		...
																		// 강화
																		// 주문서
			if ((attr_level >= 0 && attr_level <= 10)
					|| (attr_level >= 16)) {
				if (attr_level == 4 && attr_level == 5 && attr_level == 9 && attr_level == 10
						&& attr_level == 19 && attr_level == 20)
					pc.sendPackets(
							new S_ServerMessage(3297, item.getLogName()), true);
				else if (chance < 300) {
		...
		if (item_id == L1ItemId.Add_ENCHANT_WEAPON_SCROLL) {
			{
				if (step_level == 0) {
					if (chance < 20) {
						...
						pc.sendPackets(new S_SystemMessage(
								"\\fW무기 1단계 강화  업그레이드 성공!"), true);
					} else {
						pc.sendPackets(new S_SystemMessage(
								"\\fY무기 1단계 강화  업그레이드 실패!"), true);
					}
				} else if (step_level == 1) {
					if (chance < 10) {
						item.setStepEnchantLevel(2);
						pc.sendPackets(new S_SystemMessage(
								"\\fW무기 2단계 강화  업그레이드 성공!"), true);
					} else {
						item.setStepEnchantLevel(1);
						pc.sendPackets(new S_SystemMessage(
								"\\fY무기 2단계 강화  업그레이드 실패!"), true);
					}
				} else if (step_level == 2) {
					if (chance < 5) {
						item.setStepEnchantLevel(3);
						pc.sendPackets(new S_SystemMessage(
								"\\fW무기 3단계 강화  업그레이드 성공!"), true);
					} else {
						item.setStepEnchantLevel(2);
						pc.sendPackets(new S_SystemMessage(
								"\\fY무기 3단계 강화  업그레이드 실패!"), true);
					}
				} else {
	...
	public void DragonArmorEnchant(L1PcInstance pc, L1ItemInstance item,
			int item_id) {
		int attr_level = item.getAttrEnchantLevel();
		// int safe_enchant = item.getItem().get_safeenchant();
		int chance = _random.nextInt(100) + 1;
		...
		if (item_id == 5000138) {
			if (attr_level == 0) {
				if (chance < 20) {
					...
					pc.sendPackets(new S_SystemMessage("데미지 리덕션 업그레이드 성공!!"),
							true);
				} else {
					pc.sendPackets(new S_SystemMessage("데미지 리덕션 업그레이드 실패!!"),
							true);
				}
		...
		if (item_id == 5000139) {
			if (attr_level == 0) {
				if (chance < 20) {
					...
					pc.sendPackets(new S_SystemMessage(
							"\\fW수룡의 갑옷 강화  업그레이드 성공!"), true);
				} else {
					pc.sendPackets(new S_SystemMessage(
							"\\fY수룡의 갑옷 강화  업그레이드 실패!"), true);
				}
		...
		if (item_id == 5000140) {
			if (attr_level == 0) {
				if (chance < 20) {
					...
					pc.sendPackets(new S_SystemMessage(
							"\\fW풍룡의 갑옷 강화  업그레이드 성공!"), true);
				} else {
					pc.sendPackets(new S_SystemMessage(
							"\\fY풍룡의 갑옷 강화  업그레이드 실패!"), true);
				}
		...
		if (item_id == 5000141) {
			if (attr_level == 0) {
				if (chance < 20) {
					...
					pc.sendPackets(new S_SystemMessage(
							"\\fW화룡의 갑옷 강화  업그레이드 성공!"), true);
				} else {
					pc.sendPackets(new S_SystemMessage(
							"\\fY화룡의 갑옷 강화  업그레이드 실패!"), true);
				}
