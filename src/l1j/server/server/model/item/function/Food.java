	...
	public void clickItem(L1Character cha, ClientBasePacket packet) {
		if (cha instanceof L1PcInstance) {
			...
			if (itemId == 60000) {
				pc.addMpr(2);
				pc.addHpr(2);
				pc.getSkillEffectTimerSet().setSkillEffect(60000, 900 * 1000);
				pc.sendPackets(new S_PacketBox(53, 54, 900), true);
			...
			case 60151:
				...
				if (pc.get_food() < 225) { // 100%
					...
					pc.set_food(pc.get_food()
							...
							* chargeCount);// 아이템마다의 영양으로 체워지게
					if (pc.get_food() >= 225) {
						pc.set_food(225);
						pc.setSurvivalCry(System.currentTimeMillis());
					}
					pc.sendPackets(
							new S_PacketBox(S_PacketBox.FOOD, pc.get_food()),
							true);
				...
				if (itemId == 40057) { // 후로팅아이육
					...
					pc.sendPackets(new S_ServerMessage(152), true); // \f1이상하게도
																	// 감각이 예민해진것
																	// 같습니다.
				}
			...
			} else if ((itemId >= 60072 && itemId <= 60075) || (itemId >= 60726 && itemId <= 60729) 
					|| (itemId >= 600350 && itemId <= 600353) || itemId == 60259 || itemId == 60260) {
				L1Cooking.useNewCookingItem(pc, useItem);
			}
			/*
			 * long sctime = 6000 - (System.currentTimeMillis() -
			 * pc.getSurvivalCry()) / 1000; if(sctime < 1) sctime = 1; else
			 * if(sctime > 6000) sctime = 6000; pc.sendPackets(new
			 * S_SurvivalCry((int) 1), true);
			 */
		}
	}
}
