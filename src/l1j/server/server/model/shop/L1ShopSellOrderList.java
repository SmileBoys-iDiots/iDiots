...
class L1ShopSellOrder {
	...
	public L1ShopSellOrder(L1AssessedItem item, int count, double dividend) {
		_item = item;
		_count = count;
		_dividend = dividend;
	}
...
public class L1ShopSellOrderList {
	...
	public void add(int itemObjectId, int count, L1PcInstance pc) {
		...
		if (item == null || itemObjectId != item.getId()) {
			bugok = 1;
			return;
		}
		...
		}else if (item.getItem().getItemId() == 40316) {
			dividend = DogFight.getInstance().getTicketPrice(item);
		}
