...
public class L1Shop {
	...
	public void sellItems(L1PcInstance pc, L1ShopBuyOrderList orderList) {
		...
		/**기사단의 주화*/
		if (getNpcId() == 77227 || getNpcId() == 77229 
				   || (getNpcId() >= 10 && getNpcId() <= 18)
				   || (getNpcId() >= 30 && getNpcId() <= 38)) {
	...
	private boolean 특화코인체크(L1PcInstance pc, L1ShopBuyOrderList orderList) {
		...
		if (!IntRange.includes(price, 0, 20000000)) {
			pc.sendPackets(new S_SystemMessage(
					"특화코인은 한번에 20000000개 이상 사용할수 없습니다."), true);
			return false;
		}
	...
	private boolean 주화체크(L1PcInstance pc, L1ShopBuyOrderList orderList) {
		...
		if (!IntRange.includes(price, 0, 20000000)) {
			pc.sendPackets(new S_SystemMessage(
					"주화는 한번에 20000000개 이상 사용할수 없습니다."), true);
			return false;
		}
	...
	private boolean 가넷2체크(L1PcInstance pc, L1ShopBuyOrderList orderList) {
		...
		if (!IntRange.includes(price, 0, 20000000)) {
			pc.sendPackets(new S_SystemMessage(
					"가넷은 한번에 20000000개 이상 사용할수 없습니다."), true);
			return false;
		}
	...
	/*
	 * private boolean ensureCoinSell(L1PcInstance pc, L1ShopBuyOrderList
	 * orderList) { int price = orderList.getTotalPrice(); // 오버플로우 체크 if
	 ...
	 * (!pc.getInventory().checkItem(438005, price)) {
	 * //System.out.println(price); // \f1아데나가 부족합니다. pc.sendPackets(new
	 * S_SystemMessage("코인이 부족합니다."), true);
	 * 
	 * return false; }
	 * 
	 * int currentWeight = pc.getInventory().getWeight() * 1000; if
	 * (currentWeight + orderList.getTotalWeight() > pc.getMaxWeight() * 1000) {
	 * pc.sendPackets(new S_ServerMessage(82), true); return false; } int
	 * totalCount = pc.getInventory().getSize(); L1Item temp = null; for
	 * (L1ShopBuyOrder order : orderList.getList()) { temp =
	 * order.getItem().getItem(); if (temp.isStackable()) { if
	 * (!pc.getInventory().checkItem(temp.getItemId())) { totalCount += 1; } }
	 * else { totalCount += 1; } } if (totalCount > 180) { pc.sendPackets(new
	 * S_ServerMessage(263)); return false; } if (price <= 0 || price >
	 * 2000000000) { pc.sendPackets(new S_Disconnect(), true); return false; }
	 * return true; }
	 */

	/*
	 * private void payTax(L1ShopBuyOrderList orderList) {
	 * payCastleTax(orderList); payTownTax(orderList); payDiadTax(orderList); }
	 */

	/*
	 * private void payCastleTax(L1ShopBuyOrderList orderList) { L1TaxCalculator
	 * calc = orderList.getTaxCalculator(); int price =
	 * orderList.getTotalPrice(); int castleId =
	 * L1CastleLocation.getCastleIdByNpcid(_npcId); int castleTax =
	 ...
	 * L1CastleLocation.DIAD_CASTLE_ID) { castleTax += nationalTax; nationalTax
	 * = 0; }
	 * 
	 * if (castleId != 0 && castleTax > 0) { L1Castle castle =
	 * CastleTable.getInstance().getCastleTable(castleId); synchronized (castle)
	 * { int money = castle.getPublicReadyMoney(); if (2000000000 > money) {
	 * money += castleTax; castle.setPublicReadyMoney(money);
	 * CastleTable.getInstance().updateCastle(castle); } } if (nationalTax > 0)
	 * { L1Castle aden =
	 * CastleTable.getInstance().getCastleTable(L1CastleLocation
	 * .ADEN_CASTLE_ID); synchronized (aden) { int money =
	 * aden.getPublicReadyMoney(); if (2000000000 > money) { money +=
	 * nationalTax; aden.setPublicReadyMoney(money);
	...
	private void payCastleTax(L1ShopBuyOrderList orderList, int castleId) {
		...
		if (castleId != 0 && castleTax > 0) {
			...
			/*
			 * if(castleId == 4){ castleTax *= 10; }else if(castleId == 1){
			 * castleTax *= 20; }
			 */
			synchronized (castle) {
	 ...
	 * CastleTable
	 * .getInstance().getCastleTable(L1CastleLocation.DIAD_CASTLE_ID);
	 ...
	 * L1TownLocation.getTownIdByNpcid(_npcId); if (town_id >= 1 && town_id <=
	 * 10) { TownTable.getInstance().addSalesMoney(town_id, price); } } }
	 */
	...
	public void buyItems주화(L1ShopSellOrderList orderList) {
		...
		if (itemExist != null
				&& (itemExist.getCount() + totalPrice) > 2000000000) {
			inv.getOwner().sendPackets(
					new S_SystemMessage("소지하고 있는 주화가 20억을 초과하게 됩니다.")); // \f1%0이%4%1%3%2
			return;
		}
		if (0 < totalPrice) {
	...
	public List<L1AssessedItem> assessTicketsDogFight(L1PcInventory inv) {
		...
		for (L1ShopItem item : _purchasingItems) {
			for (L1ItemInstance targetItem : inv.findItemsId(item.getItemId())) {
				...
				if(DogFight.getInstance().getRoundId() == targetItem.getSecondId() && 
				   DogFight.getInstance().getBugRaceStatus() != DogFight.STATUS_NONE){
				   continue;
				}
