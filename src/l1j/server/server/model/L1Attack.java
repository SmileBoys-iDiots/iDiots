...
public class L1Attack {
	...
	private boolean calcPcPcHit() {
		 ...
		 */if (attackerDice <= defenderDice) {
		 ...
		 if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(MIRROR_IMAGE)) {
			 절대미스확률 += 절대방어수치(_targetPc.getAC().getAc(),MIRROR_IMAGE);
		 }
		 if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(SCALES_Lind_DRAGON)) {
			 절대미스확률 += 절대방어수치(_targetPc.getAC().getAc(),SCALES_Lind_DRAGON);
		 }
		 if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(UNCANNY_DODGE)) {
			 절대미스확률 += 절대방어수치(_targetPc.getAC().getAc(),UNCANNY_DODGE);
		 }
			 ...
			 if (최종공격성공확률 > 0) {
				 ...
				 if (npcId >= 45912
					 ...
					 최종공격성공확률 = 0;
						 ...
						 || npcId == 45674111 || npcId == 100570111)) {
					 최종공격성공확률 = 0;
	...
	private boolean calcNpcPcHit() {
		...
		int attackerDice = attackerValue + _hitRate;
		 ...
		 if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(MIRROR_IMAGE)) {
			 attackerDice -= 절대방어수치(_targetPc.getAC().getAc(),MIRROR_IMAGE);
		 }
		 if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(SCALES_Lind_DRAGON)) {
			 attackerDice -= 절대방어수치(_targetPc.getAC().getAc(),SCALES_Lind_DRAGON);
		 }
		 if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(UNCANNY_DODGE)) {
			 attackerDice -= 절대방어수치(_targetPc.getAC().getAc(),UNCANNY_DODGE);
		 }
	...
	public int calcPcPcDamage(int adddmg) {
			...
			for (L1ItemInstance item : _targetPc.getInventory().getItems()) {
				if (item.isEquipped()) {
					if (item.getItemId() >= 420104
						...
						if (chance1 < 3) {
							_targetPc
									.setCurrentHp(_targetPc.getCurrentHp() + 5);
							_targetPc.sendPackets(
		...
		if (_weaponType != 0) { // 맨손이 아닐때
			...
			if (secondw) {
				...
				case 54:
					...
					break;
				case 30110 :
					dmg += WeaponSkill.getRepperSwordDamage(_pc, _targetPc,
							_weaponEnchant);
					break;
				case 30111 :
					dmg += WeaponSkill.getgrankainjudge(_pc, _targetPc,
							_weaponEnchant);
					break;
				case 30112 :
					dmg += WeaponSkill.ainhasadflash(_pc, _targetPc,
							_weaponEnchant);
					break;
				case 58:
				...
				case 291: // 제로스의지팡이
				 ...
				 case 203025:
			     case 203026:	
			          if (this._weaponEnchant <= 9) break;
				...
				case 290:
					...
					break; // 환상의 무기 (10검 이벤트)
				case 291: // 제로스의지팡이
					dmg += WeaponSkill.getZerosDamagePc(_pc, _target,
							_weaponEnchant);
					break;
				case 203025:
			    case 203026:	
		...
		if (_targetPc.getSkillEffectTimerSet().hasSkillEffect(COOKING_NEW_닭고기)
				|| _targetPc.getSkillEffectTimerSet().hasSkillEffect(
						...
						|| _targetPc.getSkillEffectTimerSet().hasSkillEffect(
								COOKING_NEW_칠면조)
								|| _targetPc.getSkillEffectTimerSet().hasSkillEffect(
										COOKING_NEW_한우) || _targetPc.getSkillEffectTimerSet().hasSkillEffect(COOKING_NEW_탐닭고기)
										|| _targetPc.getSkillEffectTimerSet().hasSkillEffect(
												COOKING_NEW_탐연어)
												|| _targetPc.getSkillEffectTimerSet().hasSkillEffect(
														COOKING_NEW_탐칠면조)
		...
		if (_pc != null) {
            ...
            }if (_pc.getInventory().checkEquipped(30091)){ // 가이아의 격노
		...
		if (_target != null) { //커츠의 휘장 치명타 데미지 추가
			for (L1ItemInstance item : _pc.getInventory().getItems()) {
				if (item.isEquipped()) {
					if (item.getItemId() == 231004
							|| item.getItemId() == 231006) {
							if (item.getEnchantLevel() == 6)
	...
	private int calcPcNpcDamage(int adddmg) {
		...
		if (_weaponType == 20) { // 활
			if (_weaponId == 190 || _weaponId == 100190 || _weaponId == 450029 || _weaponId == 30082
					|| _weaponId == 30091 || (_weaponId >= 11011 && _weaponId <= 11013)) { // 사이하의 활
				int add_dmg = 0;
				add_dmg = (int) (dmg + (2));
				dmg = add_dmg;
			} else if ( _weaponId == 7201){
		...
		if (_weaponType == 0) { // 맨손
			...
			if (secondw) {
				...
				case 54:
					...
					break;
				case 30110 :
					dmg += WeaponSkill.getRepperSwordDamage(_pc, _target,
							_weaponEnchant);
					break;
				case 30111 :
					dmg += WeaponSkill.getgrankainroar(_pc, _target,
							_weaponEnchant);
					break;
				case 30112 :
					dmg += WeaponSkill.ainhasadblow(_pc, _target,
							_weaponEnchant);
					break;
				case 58:
				...
				case 290:
					...
					break; // 환상의 무기 (10검 이벤트)
				case 291: // 제로스의지팡이
					dmg += WeaponSkill.getZerosDamageNpc(_pc, _target,
							_weaponEnchant);
					break;
				case 203025:
			    case 203026:	
		...
		if (_targetNpc instanceof L1PetInstance){
			...
			if(Pet.SkillCheck(L1SkillId.DogBlood)) dmg *= 0.75;
	...
	private int calcNpcPcDamage() {
		...
		L1ItemInstance 요정족방패 = _targetPc.getInventory().checkEquippedItem(
				222355);
		if (요정족방패 != null) {
		...
		if(_npc instanceof L1PetInstance){
			...
			if(흡혈[0] != 0){
				...
				if(흡혈[0] >= Random){
					...
					L1PcInstance PetMaster = (L1PcInstance) Pet.getMaster();
					if(PetMaster != null) PetMaster.sendPackets(new S_PetWindow(S_PetWindow.PatStatUpDate, Pet), true);
				}
			}
			흡혈 = null;
			...
			if(Pet.SkillCheck(L1SkillId.Fighting)){
				if(Pet.getFightingTarget() == null) Pet.setFightingTarget(_targetPc);
	...
	/*	if (_weaponType != 62 || _weaponType != 20 && _targetPc.getSkillEffectTimerSet().hasSkillEffect(ARMOR_BREAK)) { // 아머브레이크수정중
		...
		if(_npc instanceof L1PetInstance){
			...
			int[] 속성정보 = Pet.AttributeDmg();
			/** 조작 엔피씨라면 확율 5프로 증가 */
			if(Pet.isFabrication()) 속성정보[0] += 10;
			if (속성정보[0] >= Random) {
				double calcAttr = calcAttrResistance(Pet.getNpcTemplate().get_weakAttr());
				dmg += (int)(1.0 - calcAttr) * 속성정보[1];
				_속성공격임팩트 = 속성정보[2];
			...
			Random = _Random.nextInt(100) + 1;
			int[] 속성치명정보 = Pet.AttributeCriticalDmg();
			if(Pet.isFabrication()) 속성치명정보[0] += 10;
			if (속성치명정보 != null && 속성치명정보[0] >= Random) {
				double calcAttr = calcAttrResistance(Pet.getNpcTemplate().get_weakAttr());
				dmg += (int)(1.0 - calcAttr) * 속성치명정보[1];
				_속성치명임팩트 = 속성치명정보[2];
	...
	private double calcBuffDamage(double dmg) {
		if (_pc.getSkillEffectTimerSet().hasSkillEffect(BURNING_SPIRIT)) {
			if ((_random.nextInt(100) + 1) <= 33) {
		        ...
		        } else if (_pc.getSkillEffectTimerSet().hasSkillEffect(BRAVE_AURA) 
		        		&& (_weaponType != 20 && _weaponType != 62 && _weaponType1 != 17)) {
		        	if ((_random.nextInt(100) + 1) <= 33) {
		        		dmg *= 1.5;
		        		burning = true;
		        	}
		        } else if (_pc.getSkillEffectTimerSet().hasSkillEffect(ELEMENTAL_FIRE) 
		        		&& (_weaponType != 20 && _weaponType != 62 && _weaponType1 != 17)) {
		        	if ((_random.nextInt(100) + 1) <= 33) {
		        		dmg *= 1.5;
		        		burning = true;
		        	}
		        } else if (_pc.getSkillEffectTimerSet().hasSkillEffect(ENTANGLE) 
		        		&& (_weaponType != 20 && _weaponType != 62 && _weaponType1 != 17)) {
		        	if ((_random.nextInt(100) + 1) <= 33) {
		        		dmg *= 1.5;
		        		burning = true;
					...
					if ((_random.nextInt(100) + 1) <= chance) {
						if (_calcType == PC_PC) {
							_pc.sendPackets(new S_SkillSound(_targetPc.getId(),
									17223), true);
							Broadcaster.broadcastPacket(_pc, new S_SkillSound(
									_targetPc.getId(), 17223), true);
						} else if (_calcType == PC_NPC) {
							_pc.sendPackets(new S_SkillSound(
									_targetNpc.getId(), 17223), true);
							Broadcaster.broadcastPacket(_pc, new S_SkillSound(
									_targetNpc.getId(), 17223), true);
						}
						dmg *= 1.5;
					...
					if ((_random.nextInt(100) + 1) <= chance) {
						if (_calcType == PC_PC) {
							_pc.sendPackets(new S_SkillSound(_targetPc.getId(),
									17557), true);
							Broadcaster.broadcastPacket(_pc, new S_SkillSound(
									_targetPc.getId(), 17557), true);
						} else if (_calcType == PC_NPC) {
							_pc.sendPackets(new S_SkillSound(
									_targetNpc.getId(), 17557), true);
							Broadcaster.broadcastPacket(_pc, new S_SkillSound(
									_targetNpc.getId(), 17557), true);
						}
						dmg *= 1.4;
	...
	/*// ●●●● 플레이어의 대미지 강화 마법 ●●●●
		private double calcBuffDamage(double dmg) {
			...
			if (_pc.hasSkillEffect(BURNING_SPIRIT) || _pc.hasSkillEffect(BRAVE_AURA) || _pc.hasSkillEffect(L1SkillId.ENTANGLE)
			...
			}else if (_pc.hasSkillEffect(L1SkillId.블로우어택)){ //기사 블로우 어택
				int chance = 30; //기본 30퍼 80레벨에 35퍼 85레벨에 40퍼
				if(_pc.getLevel() > 75){
					chance = chance + (_pc.getLevel() - 75); //75레벨이상일때 1퍼씩 추가됨
				}
				...
				if ((_random.nextInt(100) + 1) <= chance) {
					...
					//_pc.sendPackets(new S_SystemMessage("블로우 어택 발동!"));
				}
			}
	...
	public void calcStaffOfMana() {
		...
		if (_weaponId == 1126 || _weaponId == 126 || _weaponId == 127
				|| _weaponId == 413103 || _weaponId == 100035 || _weaponId == 30101) {
	...
	public void calcDrainOfHp() {
		if (_weaponId == 412001 || _weaponId == 1412001 || _weaponId == 12
				|| _weaponId == 100032 || _weaponId == 450032
				|| _weaponId == 30088 || _weaponId == 30097 || _weaponId == 30089
				|| _weaponId == 31080 || _weaponId == 222202
				|| _weaponId == 222205) { // 파대,
	...
	private void actionPc() {
		...
		if (_pc.getCurrentWeapon() == 20 || _weaponType == 20) {
			if (_pc instanceof L1RobotInstance || _arrow != null) {
				...
				if (_pc.getGfxId().getTempCharGfx() == 7968
					...
					Broadcaster.broadcastPacket(_pc, new S_UseArrowSkill(_pc,
						...
						|| _pc.getGfxId().getTempCharGfx() == 16002) { // 헬바인
					...
					Broadcaster.broadcastPacketExceptTargetSight(_target,
						...
						|| _pc.getGfxId().getTempCharGfx() == 16074) { // 질리언
			...
			} else if (_weaponId == 190 || _weaponId == 100190
					|| _weaponId == 7201 || _weaponId == 450029
					|| _weaponId == 30082 || _weaponId == 31082 || _weaponId == 31091
					|| _weaponId == 222206) {
				...
				if (_크리티컬) {
					...
					if (_pc.getWeapon() != null) {
						if (_pc.getWeapon().getItem().getType() == 17) {
							...
							} else if (_pc.getWeapon().getItemId() == 6001 || _pc.getWeapon().getItemId() == 30081 || _pc.getWeapon().getItemId() == 31081
									|| _pc.getWeapon().getItemId() == 222207 || _pc.getWeapon().getItemId() == 30090 || _pc.getWeapon().getItemId() == 7238) {
	...
	public void actionCounterBarrier() {
		if (_calcType == PC_PC) {
			...
			Broadcaster.broadcastPacket(_pc,
					new S_SkillSound(_targetId, 17220), true);
	...
	public void actionMortalBody() {
		if (_calcType == PC_PC) {
			...
			_pc.sendPackets(new S_SkillSound(_targetId, 6519), true);
			Broadcaster.broadcastPacket(_pc, new S_SkillSound(_targetId, 6519), true);
		} else if (_calcType == NPC_PC) {
			int actId = 0;
			...
			Broadcaster.broadcastPacket(_npc, new S_SkillSound(_targetId, 6519), true);
		}
	}
	...
	private int 룸티스붉귀데미지감소() {
		...
		if (_calcType == NPC_PC || _calcType == PC_PC) {
			...
			L1ItemInstance item3 = _targetPc.getInventory().checkEquippedItem(
					21248);
			...
			L1ItemInstance item4 = _targetPc.getInventory().checkEquippedItem(
					21252);
			...
			L1ItemInstance item5 = _targetPc.getInventory().checkEquippedItem(
					30014);
	...
	private int 절대방어수치(int ac , int 스킬)
	{
		int 절대방어 = 0;
		
		if (ac <= -140)
			절대방어 += 스킬 == MIRROR_IMAGE? 20 : 스킬 == UNCANNY_DODGE ? 18: 8 ;
		else if (ac <= -130)
			절대방어 += 스킬 == MIRROR_IMAGE? 19 : 스킬 == UNCANNY_DODGE ? 17: 7 ;
		else if (ac <= -120)
			절대방어 += 스킬 == MIRROR_IMAGE? 18 : 스킬 == UNCANNY_DODGE ? 16: 6 ;
		else if (ac <= -110)
			절대방어 += 스킬 == MIRROR_IMAGE? 17 : 스킬 == UNCANNY_DODGE ? 15: 5 ;
		else if (ac <= -100)
			절대방어 += 스킬 == MIRROR_IMAGE? 16 : 스킬 == UNCANNY_DODGE ? 14: 4 ;
		else if (ac <= -99)
			절대방어 += 스킬 == MIRROR_IMAGE? 15 : 스킬 == UNCANNY_DODGE ? 12: 3 ;
		else if (ac <= -89)
			절대방어 += 스킬 == MIRROR_IMAGE? 14 : 스킬 == UNCANNY_DODGE ? 10: 2 ;
		else if (ac <= -79)
			절대방어 += 스킬 == MIRROR_IMAGE? 11 : 스킬 == UNCANNY_DODGE ? 8: 2 ;
		else if (ac <= -69)
			절대방어 += 스킬 == MIRROR_IMAGE? 7 : 스킬 == UNCANNY_DODGE ? 6: 1 ;
		else  
			return 0;
