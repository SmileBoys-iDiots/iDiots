...
public class L1EquipmentSlot {
	...
	private void setWeapon(L1ItemInstance weapon, boolean doubleweapon) {
		...
		/*if (weapon.getBless() >= 128 && (weapon.getItem().getType2() == 1 || weapon.getItem().getType2() == 2
				|| weapon.getItem().getType2() == 3 || weapon.getItem().getType2() == 4
				|| weapon.getItem().getType2() == 5 || weapon.getItem().getType2() == 6
				|| weapon.getItem().getType2() == 7 || weapon.getItem().getType2() == 8
				|| weapon.getItem().getType2() == 9 || weapon.getItem().getType2() == 10
				|| weapon.getItem().getType2() == 11 || weapon.getItem().getType2() == 12
				|| weapon.getItem().getType2() == 13 || weapon.getItem().getType2() == 14
				|| weapon.getItem().getType2() == 15 || weapon.getItem().getType2() == 16
				|| weapon.getItem().getType2() == 17 || weapon.getItem().getType2() == 18)) {// 봉인버프
			_owner.addBowDmgup(2);
			_owner.addDmgup(2);
			_owner.getAbility().addSp(2);
			_owner.sendPackets(new S_SPMR(_owner), true);
			_owner.sendPackets(new S_NewSkillIcon(L1SkillId.SEAL_BUFF, true));
		}*/
		...
		if (weapon.getSkill() != null && weapon.getSkill().getSkillId() != 0) {
			...
			case L1SkillId.HOLY_WEAPON:
				_owner.sendPackets(new S_PacketBox(S_PacketBox.SKILL_WEAPON_ICON, 2165, weapon.getSkill().getTime(),
						false, false));
				break;
			case L1SkillId.ENCHANT_WEAPON:
				_owner.sendPackets(new S_PacketBox(S_PacketBox.SKILL_WEAPON_ICON, 747, weapon.getSkill().getTime(),
						doubleweapon, false));
				break;
			case L1SkillId.SHADOW_FANG:
				_owner.sendPackets(new S_PacketBox(S_PacketBox.SKILL_WEAPON_ICON, 2951, weapon.getSkill().getTime(),
						false, false));
				break;
			default:
				break;
	...
	private void setArmor(L1ItemInstance armor, boolean loaded) {
		 ...
		 */
		if (itemId == 21259 || itemId == 1021259 || itemId == 21265 || itemId == 1021265 || itemId == 21266
				|| itemId == 1021266 || itemId == 30218) {
			switch (itemlvl) {
			case 7:
				_owner.addMaxHp(20);
				break;
		...
		/** 발라카스의 리뉴얼 3차 갑옷 **/
		if (itemId >= 420112 && itemId <= 420115) {
			switch (itemlvl) {
			case 0:
			case 1:
			case 2:
		...
		if (itemtype >= 8 && itemtype <= 12) {
			if (itemlvl > 0) {
				...
				if (itemId == 110051 || itemId == 110052 || itemId == 110053 // 썸타는무기류
						|| itemId == 110054 || itemId == 110055 || itemId == 110056 || itemId == 110057
						|| itemId == 110058) {
					switch (itemlvl) {
		...
		/*if (armor.getSkill() != null && armor.getSkill().getSkillId() != 0) {
			...
			case L1SkillId.BLESSED_ARMOR:
				_owner.sendPackets(
						new S_PacketBox(S_PacketBox.SKILL_WEAPON_ICON, 748, armor.getSkill().getTime(), false, false));
				break;
			default:
				break;
			}
	...
	private void removeWeapon(L1ItemInstance weapon, boolean doubleweapon) {
		...
		/*if (weapon.getBless() >= 128 && (weapon.getItem().getType2() == 1 || weapon.getItem().getType2() == 2
			...
			_owner.getAbility().addSp(-2);
			_owner.sendPackets(new S_SPMR(_owner), true);
			_owner.sendPackets(new S_NewSkillIcon(L1SkillId.SEAL_BUFF, false));
		}*/
	...
	private void removeArmor(L1ItemInstance armor) {
		...
		if ((itemtype >= 8 && itemtype <= 12) || itemtype == 16 || itemtype == 20 || itemtype == 21) { // 문장, 휘장 추가
			...
			if (itemId == 20016 || itemId == 20294 || itemId == 120016 || itemId == 220294) {
				...
				_owner.getAC().addAc(
						-(item.get_ac() - armor.getEnchantLevel() - armor.getAcByMagic() + armor.get_durability()));
			}
		}
		if (itemId == 420104 || itemId == 420105 || itemId == 420106 || itemId == 420107) {
		...
		if (itemId ==10167) {// 쿠거의 가더 나이트발드의 각반
			switch (itemlvl) {
			case 0:
			case 1:
			case 2:
		...
		/*if (armor.getSkill() != null && armor.getSkill().getSkillId() != 0) {
			switch (armor.getSkill().getSkillId()) {
			case L1SkillId.BLESSED_ARMOR:
				_owner.sendPackets(new S_PacketBox(S_PacketBox.SKILL_WEAPON_ICON, 748, 0, false, false));
				break;
			default:
				break;
			}
