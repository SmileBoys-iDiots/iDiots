...
public class L1TeleporterInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking != null) {
			...
			case 50001:
				...
				break;
			/*
			 * case 50014: if (player.isWizard()) { // 위저드 if
			 * (quest.get_step(L1Quest.QUEST_LEVEL30) == 1 &&
			 * !player.getInventory().checkItem(40579)) { // 안 데드의 뼈 htmlid =
			 * "dilong1"; } else { htmlid = "dilong3"; } } break;
			 */
			case 50016:
				...
				break;
			/*
			 * case 50031: if (player.isElf()) { // 에르프 if
			 * (quest.get_step(L1Quest.QUEST_LEVEL45) == 2) { if
			 * (!player.getInventory().checkItem(40602)) { // 블루 플룻 htmlid =
			 * "sepia1"; } } } break;
			 */
			/*
			 * case 50043: if (quest.get_step(L1Quest.QUEST_LEVEL50) ==
			 * L1Quest.QUEST_END) { htmlid = "ramuda2"; } else if
			 * (quest.get_step(L1Quest.QUEST_LEVEL50) == 1) { // 디가르딘 동의가 끝난 상태
			 * if (player.isCrown()) { // 군주 if (_isNowDely) { // 텔레포트 지연중
			 * htmlid = "ramuda4"; } else { htmlid = "ramudap1"; } } else { //
			 * 군주 이외 htmlid = "ramuda1"; } } else { htmlid = "ramuda3"; } break;
			 */
			case 50055:
				if (player.getLevel() >= 13)
					htmlid = "drist1";
			 ...
			 * 쟈이안트안트 변신 htmlid = "ants3"; } else if
			 * (player.getGfxId().getTempCharGfx() == 1039) {// 쟈이안트안트소르쟈 변신 if
			 * (player.isCrown()) { // 군주 if
			 * (quest.get_step(L1Quest.QUEST_LEVEL30) == 1) { if
			 * (player.getInventory().checkItem(40547)) { // 주민들의 유품 htmlid =
			 * "antsn"; } else { htmlid = "ants1"; } } else { // Step1 이외 htmlid
			 * = "antsn"; } } else { // 군주 이외 htmlid = "antsn"; } } break;
			 */
			/*
			 * case 70853: if (player.isElf()) { // 에르프 if
			 * (quest.get_step(L1Quest.QUEST_LEVEL30) == 1) { if
			 * (!player.getInventory().checkItem(40592)) { // 저주해진 정령서 Random
			 * random = new Random(System.nanoTime()); if (random.nextInt(100) <
			 * 50) { // 50%로 다크마르단젼 htmlid = "fairyp2"; } else { // 다크 에르프 지하 감옥
			 * htmlid = "fairyp1"; } random = null; } } } break;
			 */
			case 100218: // 도리아
				...
				break;
			/*
			 * case 100219: if(player.getLevel() < 2){ htmlid
			 * player.sendPackets(new S_NPCTalkReturn(objid, "adminrw3")); }else
			 * if(player.getLevel() < 3){
			 * player.setExp(ExpTable.getExpByLevel(3)); }else
			 * if(player.getLevel() < 5){ player.sendPackets(new
			 * S_NPCTalkReturn(objid, "adminrw1")); }else if(player.getLevel() <
			 * 6){ player.setExp(ExpTable.getExpByLevel(6));
			 * player.sendPackets(new S_NPCTalkReturn(objid, "newadmin2"));
			 * }else{ player.sendPackets(new S_NPCTalkReturn(objid,
			 * "newadmin2")); } break;
			 */
			case 50036: // 윌마 기란
			case 50020: // 스텐리 켄트
	...
	public void onFinalAction(L1PcInstance player, String action) {
		...
		if (action.equalsIgnoreCase("teleportURL")) {
			...
			switch (npcid) {
			case 100438: // 요숲 엘루나
				price = new String[] { "28", "28", "28", "69", "69", "69",
						"69", "103", "103", "115", "115", "345" };
				break;
			case 50015: // 말하는섬 루카
			...
			case 50039: // 웰던 레슬리
				price = new String[] { "72", "72", "174", "174", "261", "261",
						"261", "348", "348", "1160", "580", "580", "11165",
						"1155", "10000" };
				break;
			...
			case 50046: // 아덴 엘레리스
				price = new String[] { "70", "168", "168", "252", "252", "252",
						"336", "336", "420", "1260", "700", "700", "10360",
						"1155", "10000" };
				break;
			...
			case 50051: // 오렌키리우스
				price = new String[] { "75", "180", "270", "270", "360", "360",
						"360", "450", "450", "1350", "750", "750", "12000",
						"1155", "10000" };
				break;
			case 50054: // 윈다우드트레이
			...
			case 6000016: // 신녀 플로라
				...
				break;
			case 100573: // 린지
				price = new String[] { "82", "82", "82", "198", "198", "198",
						"198", "297", "297", "495", "495", "495", "1155",
						"12210" };
				break;
			default:
				price = new String[] { "" };
			...
			price = new String[] { "780", "780", "780", "780", "780", "1080",
					"1080", "1080" };
			...
			if (getNpcId() == 50056) { // 은말 텔녀
				...
				String html = "guide_7";
				String[] price = new String[] { "700", "700", "700", "700",
			...
			if (getNpcId() == 50036 || getNpcId() == 50020
					|| getNpcId() == 50024 || getNpcId() == 50054
					|| getNpcId() == 50066 || getNpcId() == 50039
					|| getNpcId() == 50051 || getNpcId() == 50044
					|| getNpcId() == 50046) {
	...
	private void doFinalAction(L1PcInstance player, String action) {
		...
		/*
		 * if (npcid == 50014) { // 디 론 if
		 * (!player.getInventory().checkItem(40581)) { // 안 데드의 키 isTeleport =
		 * false; htmlid = "dilongn"; } } else if (npcid == 50043) { // Lambda
		 * if (_isNowDely) { // 텔레포트 지연중 isTeleport = false; } } else if (npcid
		 * == 50625) { // 고대인(Lv50 퀘스트 고대의 공간 2 F) if (_isNowDely) { // 텔레포트 지연중
		 * isTeleport = false; } } if (isTeleport) { // 텔레포트 실행 try { //
		 * 뮤탄트안트단젼(군주 Lv30 퀘스트) if
		 * (action.equalsIgnoreCase("teleport mutant-dungen_la")) { // 3 매스 이내의
		 * Pc for (L1PcInstance otherPc :
		 * L1World.getInstance().getVisiblePlayer(player, 3)) { if
		 * (otherPc.getClanid() != 0 && otherPc.getClanid() ==
		 * player.getClanid() && otherPc.getId() != player.getId()) {
		 * L1Teleport.teleport(otherPc, 32740, 32800, (short) 217, 5, true); } }
		 * L1Teleport.teleport(player, 32740, 32800, (short) 217, 5, true); } //
		 * 시련의 지하 감옥(위저드 Lv30 퀘스트) else if
		 * (action.equalsIgnoreCase("teleport mage-quest-dungen_la")) {
		 * L1Teleport.teleport(player, 32791, 32788, (short) 201, 5, true); }
		 * else if (action.equalsIgnoreCase("teleport 29_la")) { // Lambda
		 * L1PcInstance kni = null; L1PcInstance elf = null; L1PcInstance wiz =
		 * null; // 3 매스 이내의 Pc L1Quest quest = null; for (L1PcInstance otherPc
		 * : L1World.getInstance() .getVisiblePlayer(player, 3)) { quest =
		 * otherPc.getQuest(); if (otherPc.isKnight() // 나이트 &&
		 * quest.get_step(L1Quest.QUEST_LEVEL50) == 1) { // 디가르딘 동의가 끝난 상태 if
		 * (kni == null) { kni = otherPc; } } else if (otherPc.isElf() // 요정 &&
		 * quest.get_step(L1Quest.QUEST_LEVEL50) == 1) { // 디가르딘 동의가 끝난 상태 if
		 * (elf == null) { elf = otherPc; } } else if (otherPc.isWizard() // 마법사
		 * && quest.get_step(L1Quest.QUEST_LEVEL50) == 1) { // 디가르딘 동의가 끝난 상태 if
		 * (wiz == null) { wiz = otherPc; } } } if (kni != null && elf != null
		 * && wiz != null) { // 전클래스 갖추어져 있다 L1Teleport.teleport(player, 32723,
		 * 32850, (short) 2000, 2, true); L1Teleport.teleport(kni, 32750, 32851,
		 * (short) 2000, 6, true); L1Teleport.teleport(elf, 32878, 32980,
		 * (short) 2000, 6, true); L1Teleport.teleport(wiz, 32876, 33003,
		 * (short) 2000, 0, true); TeleportDelyTimer timer = new
		 * TeleportDelyTimer(); GeneralThreadPool.getInstance().execute(timer);
		 * } } else if (action.equalsIgnoreCase("teleport barlog_la")) { //
		 * 고대인(Lv50 퀘스트 고대의 공간 2 F) L1Teleport.teleport(player, 32755, 32844,
		 * (short) 2002, 5, true); TeleportDelyTimer timer = new
		 * TeleportDelyTimer(); GeneralThreadPool.getInstance().execute(timer);
		 * 
		 * } else
		 */
		try {
			if (action.equalsIgnoreCase("D_giran") || action.equalsIgnoreCase("D_giran") || action.equalsIgnoreCase("D_giran")) { // 기란던전
				...
				try {
					...
					if (s1.equals("입장가능")) {// 입장가능
						...
						if (h > 0) {
							player.sendPackets(new S_ServerMessage(1525, h + "", m + ""));// %시간 %분남았습니다.
							// pc.sendPackets(new S_SystemMessage(pc,
							// "던전 체류 가능시간이 "+h+"시간 "+m+"분 남았습니다."), true);
						} else {
							// pc.sendPackets(new S_SystemMessage(pc,
							// "던전 체류 가능시간이 "+m+"분 남았습니다."), true);
							player.sendPackets(new S_ServerMessage(1527, "" + m + ""));// 분 남았다
						}
						...
						player.sendPackets(new S_ServerMessage(1522, "3"));// 5시간 모두  사용했다.
						return;
					} else if (s1.equals("초기화")) {// 초기화
						player.setgirantime(1);
						player.setgiranday(nowday);
						player.save();
						player.sendPackets(new S_ServerMessage(1526, "3"));// 시간 남았다.
						if (action.equalsIgnoreCase("D_giran")) {
							L1Teleport.teleport(player, 32809, 32794, (short) 54, 5, true, true, 5000);
						} else {
							L1Teleport.teleport(player, 32806 + gn1, 32732 + gn1, (short) 53, 5, true, true, 5000);
		...
		if (htmlid != null) { // 표시하는 html가 있는 경우
			S_NPCTalkReturn ntr = new S_NPCTalkReturn(objid, htmlid);
			player.sendPackets(ntr, true);
		}
		htmlid = null;
	}
