...
public class L1MerchantInstance extends L1NpcInstance {
	...
	public void onTalkAction(L1PcInstance player) {
		...
		if (talking == null && npcid != 100669 && npcid != 100219 && npcid != 7000096
				&& !(npcid >= 100403 && npcid <= 100406)) { // 수련장 관리인, 라던 클릭이동
															// 엔피씨
		...
		if (talking != null) {
			...
			case 100666:
				if (!player.isCrown() || player.getClanid() == 0) {
					player.sendPackets(new S_ServerMessage(2498), true); // 혈맹
																			// 군주만
																			// 사용
																			// 가능
					return;
				} else if (공성병기딜레이) {
					player.sendPackets(new S_ServerMessage(3680), true); // 재장전시간
																			// 필요
					return;
				} else if (!WarTimeController.getInstance().isNowWar(
						L1CastleLocation.KENT_CASTLE_ID)) {
					player.sendPackets(new S_ServerMessage(3683), true); // 공성시간에만
																			// 사용
																			// 가능
					return;
				} else if (player.getClan().getCastleId() != L1CastleLocation.KENT_CASTLE_ID
						&& npcid == 100666) {
					player.sendPackets(new S_ServerMessage(3682), true); // 성혈
																			// 군주만
																			// 사용가능
					return;
				} else {
					if (npcid == 100663) {
						...
						if (player.getClan().getCastleId() != 0 || !in_war) {
							player.sendPackets(new S_ServerMessage(3681), true); // 선포한
																					// 군주만
																					// 사용가능
							return;
						}
			...
			case 70838: // 네루파
				if (player.isCrown() || player.isKnight() || player.isWizard()
						|| player.isDragonknight() || player.isIllusionist() || player.isFencer()) {
					htmlid = "nerupam1";
			...
			case 81208: // 더러워진 브롭브
				if (player.getInventory().checkItem(41129) // 핏자국의 정수
					...
					htmlid = "minibrob04";
						...
						&& player.getInventory().checkItem(41137)) { // 카헬의 아집인
																		// 정수
					htmlid = "minibrob02";
				}
				break;
