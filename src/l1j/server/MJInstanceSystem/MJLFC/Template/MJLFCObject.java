...
public class MJLFCObject extends MJInstanceObject {
	...
	public void init() {
		...
		for (int i = 0; i < size; i++) {
			...
			MJInstanceSpace.teleportInstance(pc, (short) getCopyMapId(), _lType.getRedXStartup(),
					_lType.getRedYStartup());
		...
		for (int i = 0; i < size; i++) {
			...
			MJInstanceSpace.teleportInstance(pc, (short) getCopyMapId(), _lType.getBlueXStartup(),
					_lType.getBlueYStartup());
			pc.setDamageFromLfc(0);
		}
	...
	protected void waitCount() throws Exception {
		try {
			String sec = new StringBuilder().append(_lType.getReadySecond()).append("초 후 경기가 시작됩니다.").toString();
			...
			for (int i = _lType.getReadySecond(); i > 0; i--) {
				if (i <= 10) {
					...
					msg.append(i).append("초 후 경기가 시작됩니다.");
	...
	protected boolean checkSecond() {
		if (_time > 0) {
			...
			if (_time <= 10 && _time != 0) {
				String sec = String.format("%d초 후 경기가 종료됩니다.", _time);
	...
	protected L1NpcInstance spawnBoundary(int x, int y, int heading) {
		...
		if (boundary == null)
			_log.log(Level.SEVERE, "spawnBoundary");
		return boundary;
	}
	...
	protected L1NpcInstance spawnBox(int x, int y, int heading) {
		...
		if (box == null)
			_log.log(Level.SEVERE, "spawnBox");
		return box;
	}
	...
	protected void getBack() {
		...
		for (int i = 0; i < size; i++) {
			...
			MJInstanceSpace.teleportInstance(pc, (short) MJInstanceLoadManager.MIS_ERRBACK_MAPID,
					MJInstanceLoadManager.MIS_ERRBACK_X, MJInstanceLoadManager.MIS_ERRBACK_Y);
		}
