...
public class DreamsLaboratorySpawn {
	private static DreamsLaboratorySpawn _instance;

	public static DreamsLaboratorySpawn getInstance() {
		if (_instance == null) {
			_instance = new DreamsLaboratorySpawn();
		}
		return _instance;
	}

	private DreamsLaboratorySpawn() {}
	...
	public FastTable<L1NpcInstance> fillSpawnTable(int mapid, int type, L1NpcInstance npc, boolean countB) {
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		L1Npc l1npc = null;
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM spawnlist_dreams_temple");
			rs = pstm.executeQuery();
			while (rs.next()) {
				if (type != rs.getInt("type"))
					continue;
