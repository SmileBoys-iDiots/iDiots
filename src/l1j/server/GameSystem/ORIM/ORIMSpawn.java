...
public class ORIMSpawn {
	private static ORIMSpawn _instance;
	private Random random = new Random(System.nanoTime());

	public static ORIMSpawn getInstance() {
		if (_instance == null) {
			_instance = new ORIMSpawn();
		}
		return _instance;
	}
	...
	public void Spawn(L1PcInstance pc, int mapid, int type) {
		...
		try {
			...
			if (FD == null) {
				System.out.println("화둥인던오류 : " + pc.getName() + " 이 맵 " + mapid
						+ "에서 " + type + "번 스탭 진행중 오류");
				SQLUtil.close(rs);
				SQLUtil.close(pstm);
				SQLUtil.close(con);
