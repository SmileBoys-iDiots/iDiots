...
public class CrockSystem extends Thread {
	...
	private boolean opencheck(long evatime) {
		boolean result = false;
		long time = System.currentTimeMillis();
		long bosstime = evatime + (long) ((long) 60000 * (long) 60 * (long) 2)
				+ ((long) 60000 * (long) 30);
		long bossok = evatime + (long) ((long) 60000 * (long) 60 * (long) 4);
		if (time > evatime && time < bossok) {
			return true;
		}
		return result;
	}
