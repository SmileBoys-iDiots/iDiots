...
public class NavalWarfareController implements Runnable {
	...
	private static final String[] Sub_Level = { "첫번째", "두번째", "세번째" };
	...
	public byte stage = 1;
	private byte spawn_stage = 0;
	private byte drakeCount = 0;
	private byte haphiCount = 0;
	private byte cuttleCount = 0;
	private byte shootCount = 0;
	private byte damageCount = 0;
	private byte 보상미믹레벨 = 1;
	public int score = 0;
	...
	private void StagePacket(byte stage2) {
		// TODO 자동 생성된 메소드 스텁
		L1PcInstance[] list = getMemberArray();
		if (list != null && list.length > 0) {
			for (L1PcInstance pc : list) {
				if (pc == null)
					continue;
				pc.sendPackets(new S_PacketBox(S_PacketBox.ROUND_SHOW, stage2,
						12), true);
			}
		}
		list = null;
	...
	private void 올딘대사(L1NpcInstance npc, int whileCount) {
		// TODO 자동 생성된 메소드 스텁
		String chat = "";
		switch (whileCount) {
		case 2:
			...
			break;
		case 5:
			chat = "자네는 지금 오림에게 속고 있는거야.";
			break;
		case 8:
			chat = "그를 믿어서는 안돼!. 어서 이 배를 빠져 나가 말하는 섬으로 돌아가!";
			break;
		default:
			return;
		}
		if (chat.equalsIgnoreCase("") || npc == null || npc._destroyed
				|| npc.isDead())
			return;
		Broadcaster.broadcastPacket(npc, new S_NpcChatPacket(npc, chat), true);
	}
	...
	private void bosszoneReset() {
		bossMoveOn = false;
		bossMoveOn2 = false;
		bossMoveOn3 = false;
		for (L1Object obj : L1World.getInstance().getVisibleObjects(mapid)
				.values()) {
			if (obj == null || obj instanceof L1DollInstance
					|| obj instanceof L1SummonInstance
					|| obj instanceof L1PetInstance)
				continue;
			if ((obj.getX() >= 32668 && obj.getX() <= 32680
					&& obj.getY() >= 32783 && obj.getY() <= 32816)
					|| // 올딘선
					(obj.getX() >= 32791 && obj.getX() <= 32810
							&& obj.getY() >= 32842 && obj.getY() <= 32876) || // 해적선
					(obj.getX() >= 32728 && obj.getX() <= 32747
							&& obj.getY() >= 32841 && obj.getY() <= 32879)) { // 유령선
				if (obj instanceof L1NpcInstance) {
					L1NpcInstance npc = (L1NpcInstance) obj;
					npc.deleteMe();
				} else if (obj instanceof L1PcInstance) {
					L1Teleport.teleport((L1PcInstance) obj, 32795, 32803,
							(short) obj.getMapId(), 4, true);// 좌표 수정해야됨
				}
			}
		}
		for (L1ItemInstance obj : L1World.getInstance().getAllItem()) {
	...
	private void Effect_Blind() {
		L1PcInstance[] list = getMemberArray();
		if (list != null && list.length > 0) {
			for (L1PcInstance pc : list) {
				if (pc == null)
					continue;
				pc.sendPackets(new S_PacketBox(83, 4), true);
			}
		}
		list = null;
	}

	private void Effect_P() {
		L1PcInstance[] list = getMemberArray();
		if (list != null && list.length > 0) {
			for (L1PcInstance pc : list) {
				if (pc == null)
					continue;
				pc.sendPackets(new S_Sound(184), true);
				pc.sendPackets(new S_PacketBox(83, 1), true);
			}
		}
		list = null;
	}

	private void LocationEffect(int x, int y, int gfxid) {
		L1PcInstance[] list = getMemberArray();
		if (list != null && list.length > 0) {
			for (L1PcInstance pc : list) {
				if (pc == null)
					continue;
				pc.sendPackets(new S_EffectLocation(x, y, (short) gfxid), true);
			}
		}
		list = null;
	}
	...
	// 공격 당한 횟수가 증가함에 따라 중간 보스 및 보스가 질이 떨어짐
	// 공격72 피격36
	// 피격 3
	// 공격 7
	// 크리티컬 11
	...
	private void 배옆액션변경(int act) {
		for (L1NpcInstance npc : baseList) {
			if (npc != null && npc.getNpcId() == 100109) {
				npc.setActionStatus(act);
				L1PcInstance[] list = getMemberArray();
				if (list != null && list.length > 0) {
					for (L1PcInstance pc : list) {
						if (pc == null)
							continue;
						pc.sendPackets(new S_DoActionGFX(npc.getId(), act),
								true);
					}
				}
				break;
	...
	private int rankLoad() {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("SELECT * FROM navalwarfare_score order by score desc");
			rs = pstm.executeQuery();
			while (rs.next()) {
				i++;
				if (score == rs.getInt(2)
						&& _member.get(0).getName()
								.equalsIgnoreCase(rs.getString(3)))
					break;
			}
		} catch (SQLException e) {
			e.printStackTrace();
