	...
	public static boolean 클래스버프(L1RobotInstance bot) {
		...
		if (bot.isKnight()) {
			...
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(
				...
				new L1SkillUse().handleCommands(bot, L1SkillId.DRAGON_SKIN,
						bot.getId(), bot.getX(), bot.getY(), null, 0,
			...
			if (!bot.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.BURNING_SPIRIT)) {
				new L1SkillUse().handleCommands(bot, L1SkillId.BURNING_SPIRIT,bot.getId(), bot.getX(), bot.getY(), null, 320, L1SkillUse.TYPE_NORMAL);
				return true;
			}
		} else if (bot.isWizard()) {
