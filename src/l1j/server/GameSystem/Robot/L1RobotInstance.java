...
public class L1RobotInstance extends L1PcInstance {
	...
	public boolean Poison() {
		// TODO 자동 생성된 메소드 스텁
		if (getSkillEffectTimerSet().hasSkillEffect(71) == true) { // 디케이포션 상태
			return false;
		}
		if (getPoison() != null) {
			...
			for (L1Object obj : list){
				...
				for (L1PcInstance obj2 : list2){
					if (obj2 instanceof L1PcInstance) {
						...
						if (!saram.isPinkName() && Config.로봇막피 == false && (!(getMapId() >= 43 && getMapId() <= 50))){ 
							continue; 
						}
	...
	public boolean onTarget() {
		...
		if (!FirstSkill2 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isElf() && getCurrentWeapon() == 20) {
				skillId = L1SkillId.TRIPLE_ARROW;
		...
		if (!FirstSkill3 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isElf() && getCurrentWeapon() == 20) {
				skillId = L1SkillId.TRIPLE_ARROW;
	...
	public <L1Pcinstance> boolean onTarget2() {
		...
		if (!FirstSkill && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isElf() && getCurrentWeapon() == 20 && getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_ELFBRAVE) && Config.봇게일 == true) {
				skillId = L1SkillId.STRIKER_GALE;
			}  else if (isElf() && getCurrentWeapon() == 20 && getSkillEffectTimerSet().hasSkillEffect(L1SkillId.포커스웨이브) && Config.봇똥물 == true) {
				...
				skillId = L1SkillId.FEAR;
				skill_range = 1;
			}  else if (isWarrior()) {
				skillId = L1SkillId.파워그립;
				skill_range = 1;
			}  else if (isElf() && getCurrentWeapon() != 20) {
				skillId = L1SkillId.ELEMENTAL_FALL_DOWN;
				skill_range = 1;
			}  else if (isKnight()) {
				skillId = L1SkillId.SHOCK_STUN;
				skill_range = 1;
			}  else if (isCrown() && Config.봇군주엠파 == true) {
		...
		if (!FirstSkill4 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isElf() && getCurrentWeapon() == 20) {
				skillId = L1SkillId.TRIPLE_ARROW;
		...
		if (!FirstSkill5 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isWizard()) { //법사봇 디스
				skillId = L1SkillId.ICE_SPIKE;
		...
		if (!FirstSkill6 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isWizard()) { //법사봇 디스
				skillId = L1SkillId.ICE_SPIKE;
		...
		if (!FirstSkill7 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isWizard()) { //법사봇 디스
				skillId = L1SkillId.ICE_SPIKE;
		...
		if (!FirstSkill8 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isWizard()) { //법사봇 디스
				skillId = L1SkillId.ICE_SPIKE;
		...
		if (!FirstSkill9 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isWizard()) { //법사봇 디스
				skillId = L1SkillId.ICE_SPIKE;
		...
		if (!FirstSkill10 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isWizard()) { //법사봇 디스
				skillId = L1SkillId.ICE_SPIKE;
		...
		if (!FirstSkill11 && !isSkillDelay() && getCurrentMp() > 30) {
			int skillId = 0;
			int skill_range = 11;
			if (isDragonknight()) {
				skillId = L1SkillId.FOU_SLAYER;
		...
		if (_target2.getCurrentHp() > 1 && !isSkillDelay() && getCurrentMp() > 30) {
			...
			if (isDragonknight()) {
				 ...
				 * else if(drandom > 6 && isDarkelf()){ //다엘봇 더블
				 * Broadcaster.broadcastPacket(_target2, new S_SkillSound(_target2.getId(),
				 * 13417)); _target2.sendPackets(new S_SkillSound(_target2.getId(), 13417));
				 * _target2.receiveDamage(this, 200, false); }
				 */
		...
		if (CharPosUtil.isAttackPosition(this, target2.getX(), target2.getY(),target2.getMapId(), range) == true
				&& CharPosUtil.isAttackPosition(target2, getX(), getY(), getMapId(), range) == true) {// 기본 공격범위	
			if (door || !tail) {
				cnt++;
				if (cnt > 5) {
					_target2 = null;
			...
			public void run() {
				...
				try {
					if (L1RobotInstance.this.isDead()
							...
							|| L1RobotInstance.this.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.데스페라도)
...
//			
//			/*if (type != MOVE_SPEED) {
//				if (gfxid >= 11328 && gfxid <= 13635) {// 로봇공속수정
//					if (getLevel() >= 15)
//						interval -= 43;
//					if (getLevel() >= 30)
//						interval -= 43;
