...
public class PackageWarehouse extends Warehouse {
	...
	public synchronized boolean checkitem(int itemid, int count) {
		...
		try {
			...
			pstm = con
					.prepareStatement("SELECT count FROM character_package_warehouse WHERE account_name = ? AND item_id = ?");
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO character_package_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, second_id=?, round_id=?, ticket_id=?");
