...
public class ClanWarehouse extends Warehouse {
	...
	public synchronized void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("SELECT * FROM clan_warehouse WHERE id = ?");
			pstm.setInt(1, item.getId());
			rs = pstm.executeQuery();
			if (!rs.next()) {
				pstm = con
						.prepareStatement("INSERT INTO clan_warehouse SET id = ?, clan_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id= ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, end_time=?, CreaterName=?, demon_bongin=?");
				pstm.setInt(1, item.getId());
