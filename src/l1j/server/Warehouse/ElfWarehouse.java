...
public class ElfWarehouse extends Warehouse {
	...
	public synchronized boolean checkitem(int itemid, int count) {
		...
		try {
			...
			pstm = con
					.prepareStatement("SELECT count FROM character_elf_warehouse WHERE account_name = ? AND item_id = ?");
	...
	public void insertItem(L1ItemInstance item) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO character_elf_warehouse SET id = ?, account_name = ?, item_id = ?, item_name = ?, count = ?, is_equipped=0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, attr_enchantlvl = ?, bless = ?,  second_id=?, round_id=?, ticket_id=?, end_time=?, CreaterName=?, demon_bongin=?");
			...
			pstm.setString(18, item.getCreaterName());
			pstm.setInt(19, item.isDemonBongin() ? 1 : 0);
