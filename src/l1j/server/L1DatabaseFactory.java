...
public class L1DatabaseFactory {
	...
	private L1DatabaseFactory() throws SQLException {
		/*
		 * try { _source = new ComboPooledDataSource();
		 * _source.setDriverClass(_driver); _source.setJdbcUrl(_url);
		 * _source.setUser(_user); _source.setPassword(_password);
		 * 
		 * /* Test the connection
		 */
		/*
		 * _source.getConnection(). close(); } catch (SQLException x) {
		 * _log.fine("Database Connection FAILED"); // rethrow the exception
		 * throw x; } catch (Exception e) {
		 * _log.fine("Database Connection FAILED"); throw new
		 * SQLException("could not init DB connection:" + e); }
		 */
	}
	...
	public Connection getConnection() {
		 ...
		 * Connection con = null;
		 * 
		 * while (con == null) { try { con = _source.getConnection(); } catch
		 * (SQLException e) {
		 * _log.warning("L1DatabaseFactory: getConnection() failed, trying again "
		 * + e); } } return Config.DETECT_DB_RESOURCE_LEAKS ?
		 * LeakCheckedConnection . create(con) : con;
		 */
	}
}
