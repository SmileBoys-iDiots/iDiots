		...
		while (true) {
			try {
				...
				for (L1PcInstance _client : L1World.getInstance()
						...
						try {
							...
							if (Config.Dragon_3DAY_Event && Config.드다드랍진행중) {
								if (dtc >= 300) {
									...
									_client.sendPackets(
											new S_SystemMessage(
													"드다 이벤트 : 5시간 접속 보상 [드래곤의 다이아몬드] 획득."),
											true);
								} else {
							 ...
							 * _client.getSurvivalCry()) / 1000L; long minute =
							 * time / 60L; if (minute < 29L) { L1ItemInstance[]
							 * itemlist =
							 * _client.getInventory().findItemsId(60059);
							 * if(itemlist != null && itemlist.length > 0){
							 * for(L1ItemInstance tempitem : itemlist){
							 * if(tempitem.get_tempGfx() != 0) continue;
							 * tempitem.set_tempGfx(5355);
							 * _client.sendPackets(new
							 * S_DeleteInventoryItem(tempitem));
							 * _client.sendPackets(new S_AddItem(tempitem)); } }
							 * itemlist = null; } }
							 */
							...
							if (_client.getDollList().size() > 0) {
								for (L1DollInstance doll : _client
									...
									/*
									 * if(doll.getDollType() ==
									 * L1DollInstance.DOLLTYPE_남자_여자){
									 * if(_client.마법인형_남자여자_Count >= 15){
									 * _client.getInventory().storeItem(60472,
									 * 1); // 할로윈 호박씨 지급 _client.sendPackets(new
									 * S_ServerMessage(403, "크리스마스 쿠키 주머니"),
									 * true); _client.마법인형_남자여자_Count = 0; }else
									...
									if (doll.getDollType() == L1DollInstance.DOLLTYPE_그렘린) {
										if (_client.마법인형_그렘린_Count >= 15) {
											_client.getInventory().storeItem(
													9057, 1); // 할로윈 호박씨 지급
											_client.sendPackets(new S_SystemMessage(
													"붉은코카트리스 마법인형 소환 보상 : 붉은코카트리스 선물 상자 획득."));
											// _client.sendPackets(new
											// S_ServerMessage(403,
											// "그렘린의 선물 상자"), true);
					 ...
					 * L1Teleport.teleport(_client, 32928, 32864, (short) 6202,
					 * 5, true); } }
					 */
	...
	private class BuffStart implements Runnable {
		...
		private void buff(L1PcInstance pc) {
			...
			int[] allBuffSkill = { PHYSICAL_ENCHANT_DEX, PHYSICAL_ENCHANT_STR,
					BLESS_WEAPON, ADVANCE_SPIRIT, FIRE_SHIELD };
			if (pc.getLevel() <= 65) {
				try {
