...
//				}
				if (pc.getMapId() == 54
						|| pc.getMapId() == 15403 || pc.getMapId() == 15404) {// 기란던전&지배자 결계
					try {
				...
				if (pc.getMapId() == 785 || pc.getMapId() == 788
						|| pc.getMapId() == 789) { // 수렵이벤트
					try {
						수렵이벤트TimeCheck(pc);
					} catch (Exception e) {
					}
...
//				}
	...
	private void 할로윈(L1PcInstance player) {
		...
		try {
			...
			if (s1.equals("입장가능")) {// 입장가능
				...
				player.sendPackets(new S_PacketBox(S_PacketBox.TIME_COUNT,
						outtime - 1), true);
	...
	private void 몽섬(L1PcInstance player) {
		...
		if (player.getpc몽섬day() == null || player.get몽섬day() == null) {
			...
			if (maxcount <= 씨앗) {
				if (player.몽섬텔대기시간 <= 0) {
					L1Teleport.teleport(player, 33443, 32798, (short) 4, 5,
							true);
					return;
				} else {
					player.몽섬텔대기시간--;
			...
			try {
				...
				if (s.equals("입장가능")) {// 입장가능
					...
					player.sendPackets(new S_SystemMessage(
							"입장 시간 : 계정 입장 시간 3시간 30분 모두 사용"), true);
					L1Teleport.teleport(player, 33443, 32798, (short) 4, 5,
							true);
					return;
					...
					player.getNetConnection().getAccount().updateDGTime();
					player.sendPackets(new S_SystemMessage(
							"입장 시간 : 계정 입장 시간 3시간 30분 남음"), true);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
			...
			try {
				...
				if (s.equals("입장가능")) {// 입장가능
					...
					player.getNetConnection().getAccount().updateDGTime();
					player.sendPackets(new S_PacketBox(player,
							S_PacketBox.DG_TIME_RESTART), true);
					player.sendPackets(new S_PacketBox(S_PacketBox.TIME_COUNT,
							outtime - 1), true);
	...
	if (player.getgiranday() == null) {
		...
		L1Teleport
				.teleport(player, loc[0], loc[1], (short) loc[2], 5, true);
	} else {
		Timestamp nowday = new Timestamp(System.currentTimeMillis());
		try {
			int outtime = 60 * 60 * 2;
			int usetime = player.getravatime();
			String s = isPC입장가능여부(player.getgiranday(), outtime, usetime);
			if (s.equals("입장가능")) {// 입장가능
			} else if (s.equals("불가능")) {// 입장불가능
				player.sendPackets(new S_ServerMessage(1522, "2"));// 2시간 모두사용했다.
				int[] loc = Getback.GetBack_Location(player, true);
				L1Teleport.teleport(player, loc[0], loc[1], (short) loc[2],
						5, true);
				return;
			} else if (s.equals("초기화")) {// 초기화
		...
		if (player.get수상한천상계곡day() == null) {
			...
			try {
				...
				if (s.equals("입장가능")) {// 입장가능
					...
					// 사용했다.
					L1Teleport.teleport(player, 32779, 32832, (short) 622, 5,
							true);
					return;
				} else if (s.equals("초기화")) {// 초기화
		...
		if (player.get수렵이벤트day() == null) {
			...
			try {
				...
				if (s.equals("입장가능")) {// 입장가능
					...
					player.sendPackets(new S_ServerMessage(1522, "1"));// 5시간 모두
					// 사용했다.
					L1Teleport.teleport(player, 33432, 32796, (short) 4, 5,
							true);
					return;
				} else if (s.equals("초기화")) {// 초기화
	...
	/*private void 낚시(L1PcInstance player)
	  {
	    player.addchecktime();
	    ...
	    try {
	      int outtime = 60 * 60 * 8;
	      int usetime = player.get낚시time();
	      String s = isPC입장가능여부(player.get낚시day(), outtime, usetime);

	      if (!s.equals("입장가능")) {
	        if (s.equals("불가능")) {
	          player.sendPackets(new S_ServerMessage(1522, "8"));
	          ...
	          player.set낚시time(1);
	          player.set낚시day(nowday);
	          player.getNetConnection().getAccount().updateDGTime();
	          player.sendPackets(new S_PacketBox(153, outtime - 1), true);

	          player.sendPackets(new S_ServerMessage(1526, "8"));
	        }
	      }
	    } catch (Exception e) {
	      e.printStackTrace();
