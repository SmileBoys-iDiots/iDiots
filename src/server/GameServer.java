...
public class GameServer implements AcceptorHandler {
	...
	// public Tmfpem (aasdf indd , alin akjd){
	// try {
	// while (true) {
	// asdfawef
	// null;
	...
	//	DevilController.getInstance().start();
		...
		GeneralThreadPool.getInstance().schedule(
				FishingTimeController.getInstance(), 300);
	...
	//	Robot_Location.좌표설정();
			...
			// AutoCheckThread.getInstance();
			// IpCheckTable.getInstance();
			// IpPhoneCertificationTable.getInstance();
			//
			// 광고관련
			// ItemBayChatController.getInstance();
			// NurtureChatController.getInstance();
			// BadukiChatController.getInstance();

			L1MonsterInstance.리퍼시간체크 = new 감시자리퍼시간체크();
			GeneralThreadPool.getInstance().schedule(L1MonsterInstance.리퍼시간체크,
					1000);
			// L1BookMark.기란감옥텔체크();
			// PackageWarehouse.present(1,50, 9094, 0, 1);
			// PackageWarehouse.present("*", 9094, 0, 1);
			케릭샵테이블초기화();
		...
		if (Config.새로운패킷구조) {
			...
			System.out.println("사용중인 메모리 : " + SystemUtil.getUsedMemoryMB()
					+ "MB");

			System.out.println(":: 게임 서버가 " + Config.GAME_SERVER_PORT
					+ "번 포트를 이용해서 가동 되었습니다.  : Memory : "
					+ SystemUtil.getUsedMemoryMB() + " MB");

		}

		/*
		 * for( int i = 0; i < 10; ++i ){ _wst[i] = new SelectorThread(); }
		 * Acceptor wsc = new Acceptor(4000, _wst[0], this); wsc.startAccept();
		 * System.out.println(":: Web 서버가 4000번 포트를 이용해서 가동 되었습니다.  : Memory : "
		 * + SystemUtil.getUsedMemoryMB() + " MB");
		 */
	}
	...
	public void onAccept(Acceptor acceptor, final SocketChannel sc) {
		try {
			...
			if (!IpTable.getInstance().isBannedIp(ClientIp)) {
				...
				// System.out.println("접속 시행중 IP " + ClientIp);
				System.out.println(" ---  접속 시도 중 ---  IP : " + ClientIp
						+ "  Thread.C : " + Thread.activeCount()
						+ " UseMemory : " + SystemUtil.getUsedMemoryMB()
						+ " MB");
				++_ioThreadIndex;
	...
	/*
	 * 첫번째 쿼리 실행문 : TRUNCATE web_item_image 두번째 쿼리 실행문 : insert into
	 * web_item_image (item_name, invgfx) select name, invgfx from weapon union
	 * all select name, invgfx from armor union all select name, invgfx from
	 * etcitem
	 */
	...
	private void 웹이미지테이블넣기() {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("insert into web_item_image (item_name, invgfx) select name, invgfx from weapon   union all select name, invgfx from armor   union all select name, invgfx from etcitem");
			pstm.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public void 계정및인벤아덴체크() {
		...
		try {
			...
			pstm2 = con2
					.prepareStatement("SELECT * FROM accounts WHERE banned=0");
			rs2 = pstm2.executeQuery();
			while (rs2.next()) {
				String account = rs2.getString("login");
				Connection con = L1DatabaseFactory.getInstance()
						.getConnection();
	...
	public void 로그저장(StringBuilder sb) {
		 ...
		 * writer.write(sb.toString()); } catch (IOException e) {
		 * e.printStackTrace(); }
		 */
		// 저장
		try {
		...
		try {
			...
			while (rs2.next()) {
				...
				Connection con = L1DatabaseFactory.getInstance()
						.getConnection();
				PreparedStatement pstm1 = con
						.prepareStatement("SELECT login FROM accounts WHERE "
								+ qu + "=?");
				...
				if (count >= 20)
					f20.add(password + " Count: " + count + " > "
							+ sb.toString());
				else if (count >= 15)
					f15.add(password + " Count: " + count + " > "
							+ sb.toString());
				else if (count >= 10)
					f10.add(password + " Count: " + count + " > "
							+ sb.toString());
				else if (count >= 5)
					f5.add(password + " Count: " + count + " > "
							+ sb.toString());
				// }
				SQLUtil.close(rs);
				SQLUtil.close(pstm1);
			...
			System.out.println("총 : "
					+ (f20.size() + f15.size() + f10.size() + f5.size()));
	...
	public void 인벤아덴체크() {
		...
		try {
			...
			con2 = L1DatabaseFactory.getInstance().getConnection();
			pstm2 = con2
					.prepareStatement("SELECT * FROM character_items WHERE item_id=? order by count desc");
			...
			while (rs2.next()) {
				...
				while (rs.next()) {
					...
					if (count > 5200000) {
						System.out.println("계정:" + account + " 이름:" + name
								+ " 아데나:" + count);
						count2++;
					}
				}
		...
		try {
			...
			while (rs2.next()) {
				...
				Connection con = L1DatabaseFactory.getInstance()
						.getConnection();
				PreparedStatement pstm1 = con
						.prepareStatement("SELECT * FROM shop WHERE item_id=?");
	...
	public void 상점판매액체크() {
		...
		try {
			...
			while (rs.next()) {
				...
				int pack = rs.getInt("pack_count");
				// 테스트 NPC
				if (npcid == 4500162 || npcid == 4220019 || npcid == 4220700
						|| npcid == 8000055)
					continue;
				if (price == -1)
					continue;
				...
				Connection con1 = L1DatabaseFactory.getInstance()
						.getConnection();
				PreparedStatement pstm1 = con
						.prepareStatement("SELECT * FROM shop WHERE item_id=?");
				pstm1.setInt(1, itemid);
				...
				while (rs1.next()) {
					...
					if (totalprice < sell_price) {
						System.out.println(npcid + " > " + itemid + " > "
	...
	public void disconnectAllCharacters() {
		try {
			...
			try {
				if (!L1BugBearRace.getInstance()._goal) {
					for (L1LittleBugInstance b : L1BugBearRace.getInstance()._littleBug) {
						if (b == null)
							continue;
						L1BugBearRace.getInstance().race_divAdd(
								L1BugBearRace.getInstance().getRoundId(),
								b.getNumber(), 1);
					}
				}
			} catch (Exception e) {
			...
			try {
				for (L1FieldObjectInstance foi : L1World.getInstance()
						.getAllField()) {
					if (foi == null || foi._destroyed || foi.isDead()
							|| foi.Potal_Open_pcid == 0)
						continue;
					String AccountName = null;
					L1Object obj = L1World.getInstance().findObject(
							foi.Potal_Open_pcid);
					...
					SupplementaryService pwh = WarehouseManager.getInstance()
							.getSupplementaryService(AccountName);
			...
			for (L1PcInstance pc : players) {
				...
				if (pc.getNetConnection() != null) {
					try {
						...
						} else if (pc.getMapId() >= 2102
								&& pc.getMapId() <= 2151) { // 얼녀 인던
	...
	/*
	 * public LineageClient[] getallclient() { return clmap.toArray(new
	 * LineageClient[clmap.size()]); } private ArrayList<LineageClient> clmap =
	 * new ArrayList<LineageClient>(); public void addcl(LineageClient cl){
	 * clmap.add(cl); } public void removecl(LineageClient cl){
	 * if(clmap.contains(cl)){ clmap.remove(cl); } }
	 */
	private class ServerShutdownThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				/*
				 * for(L1RobotInstance bot :
				 * L1World.getInstance().getAllRobot()){ bot._스레드종료 = true; for
				 * (L1PcInstance pc :
				 * L1World.getInstance().getRecognizePlayer(bot)) {
				 * S_RemoveObject re = new S_RemoveObject(bot);
				 * pc.sendPackets(re, true);
				 * pc.getNearObjects().removeKnownObject(bot); }
				 * world.removeVisibleObject(bot); world.removeObject(bot);
				 * bot.getNearObjects().removeAllKnownObjects();
				 * bot.stopHalloweenRegeneration(); bot.stopPapuBlessing();
				 * bot.stopLindBlessing(); bot.stopHalloweenArmorBlessing();
				 * bot.stopAHRegeneration(); bot.stopHpRegenerationByDoll();
				 * bot.stopMpRegenerationByDoll(); bot.stopSHRegeneration();
				 * bot.stopMpDecreaseByScales(); bot.stopEtcMonitor();
				 * bot.setDead(true); }
				 */

				/*
				 * for(LineageClient cl: clmap){ //if(!cl.isClosed()){
				 * if(cl.getsbsize()>1){ cl.행동로그저장();
				 * if(cl.getActiveChar()!=null){ user++; }else
				 * if(cl.getAccount() != null){ account++; }else{ client++; }
				 * //} } }
				 */
				// System.out.println("패킷 녹화 종료. 저장완료.");
				// System.out.println("접속 ("+user+")개 / 계정 ("+account+")개 / 클라 ("+client+")개");
				while (0 < secondsCount) {
					if (secondsCount <= 30 || secondsCount == 40
							|| secondsCount == 50) {
						System.out.println("[GameServer] 게임이 " + secondsCount
								+ "초 후에 종료 됩니다. 게임을 중단해 주세요.");
						world.broadcastServerMessage("리니지가 " + secondsCount
								+ "초 후 중단됩니다. 안전한 곳에서 종료하여 주시기 바랍니다.");
					} else {
						if (secondsCount % 60 == 0) {
							System.out.println("[GameServer] 게임이 "
									+ secondsCount / 60 + "분 후에 종료 됩니다.");
							world.broadcastServerMessage("리니지가 " + secondsCount
									/ 60 + "분 후 중단됩니다. 안전한 곳에서 종료하여 주시기 바랍니다.");
						}
					}
					Thread.sleep(1000);
					secondsCount--;
				...
				System.out
						.println("[GameServer] 서버 종료가 중단되었습니다. 서버는 정상 가동중입니다.");
	...
	public void INNKeyDelete() {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("DELETE FROM character_items WHERE item_id = 49312 OR item_id = 40312");
			pstm.executeUpdate();
		} catch (SQLException e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
		} finally {
	...
	public static void DuplicationLoginCheck(String name, String msg) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("INSERT INTO duplication_login_check SET name=?, msg=?");
			...
			for (int i = 1; i <= 10; i++) {
				...
				if (!temp_name.equalsIgnoreCase("")) {
					sb.append(i + ". 판매자: " + temp_name + " 판매액수: " + temp_t
							+ "\n");
					for (log_shop_total lst : list
							.toArray(new log_shop_total[list.size()])) {
