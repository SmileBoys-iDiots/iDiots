...
public class WebClient {
	...
	public WebClient(byte[] abyte0) {
		...
		try {
			...
			case 상인찾기버튼:
				...
				L1NpcShopInstance targetNpc = L1World.getInstance().getNpcShop(
						name2);
				if (pc == null)
					return;
				...
				if (targetpc != null) {
					...
					pc.dh = calcheading(pc.dx, pc.dy, targetpc.getX(),
							targetpc.getY());
					...
					pc.dh = calcheading(pc.dx, pc.dy, targetNpc.getX(),
							targetNpc.getY());
			...
			case 혈맹가입신청버튼:
				...
				// System.out.println("[패킷1]\r\n"+printData(c_pcname.getBytes(),
				// c_pcname.getBytes().length));
				// System.out.println("[패킷2]\r\n"+printData(c_clanname.getBytes(),
				// c_clanname.getBytes().length));
				// System.out.println("1 : "+c_pcname + " 2 :"+c_clanname);
				L1PcInstance _pc = L1World.getInstance().getPlayer(c_pcname);
				if (_pc == null)
					return;// 123
				if (_pc.getSkillEffectTimerSet().hasSkillEffect(
						L1SkillId.가입신청딜레이)) {
					int time = _pc.getSkillEffectTimerSet()
							.getSkillEffectTimeSec(L1SkillId.가입신청딜레이);
					_pc.sendPackets(new S_SystemMessage("(" + time
							+ ") 초후 다시 클릭해주세요."));
					return;
				}
				...
				/*
				 * if (clan == null){ _pc.sendPackets(new
				 * S_SystemMessage(c_clanname+" 혈맹을 찾을 수 없습니다.")); return; }
				 */
				if (clan == null) {
					_pc.sendPackets(new S_NewCreateItem(
							S_NewCreateItem.CLAN_JOIN_MESSAGE, 4), true);
					return;
				}

				L1PcInstance _target = clan.getonline간부();
				/*
				 * if(_target == null){ _pc.sendPackets(new
				 * S_SystemMessage(c_clanname
				 * +" 혈맹에 가입 받을수 있는 케릭이 접속중이지 않습니다.")); return; }
				 */
				if (_target == null) {
					_pc.sendPackets(new S_NewCreateItem(
							S_NewCreateItem.CLAN_JOIN_MESSAGE, 11), true);
					return;
				}
	...
	private void 혈가입(L1PcInstance player, L1PcInstance target, L1Clan clan) {
		if (player.getClanid() != 0) { // 이미 크란에 가입이 끝난 상태
			if (player.isCrown()) {
				player.sendPackets(new S_NewCreateItem(
						S_NewCreateItem.CLAN_JOIN_MESSAGE, 13), true);
				return;
			} else {
				player.sendPackets(new S_NewCreateItem(
						S_NewCreateItem.CLAN_JOIN_MESSAGE, 9), true);
				return;
			}
		}
		...
		if (clan.getJoinSetting() == 0) {
			...
			player.sendPackets(new S_NewCreateItem(
					S_NewCreateItem.CLAN_JOIN_MESSAGE, 0), true);
			return;
		} else {// 승인가입
			target.setTempID(player.getId()); // 상대의 오브젝트 ID를 보존해 둔다
			player.getSkillEffectTimerSet().setSkillEffect(L1SkillId.가입신청딜레이,
					30000);
			// player.sendPackets(new
			// S_SystemMessage("["+clan.getClanName()+"] 혈맹에 가입신청을 하였습니다."));
			player.sendPackets(new S_NewCreateItem(
					S_NewCreateItem.CLAN_JOIN_MESSAGE, 1), true);
			if (target instanceof L1RobotInstance) {
				Random _rnd = new Random(System.nanoTime());
				GeneralThreadPool.getInstance().schedule(
						new join(target, player), 1000 + _rnd.nextInt(2000));
			} else {
